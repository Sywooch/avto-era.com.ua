...
{foreach $products as $key => $p}
    ...
    <li class="item-WL {if $p.stock == 0}not-avail{/if}">
        <a href="{shop_url('product/' . $p.url)}" class="frame-photo-title">
            <span class="d_b m-b_10"> <span class="title">{echo ShopCore::encode($p.name)}</span></span>
            ...
            <div class="funcs-buttons-WL-item">
                <div class="btn-remove-item-wl">
                    <button class="pointer"
                        ...
                        ><span class="icon_remove"></span><span class="text-el d_l_bl">{lang('Удалить', 'newLevel')}</span></button>
                </div>
                <div class="btn-move-item-wl">
                    <button class="pointer"
                        ...
                        data-always="true"
                        ><span class="icon_move"></span><span class="text-el d_l_bl">{lang('Переместить', 'newLevel')}</span>
                    </button>
                </div>
            </div>
        {/if}
