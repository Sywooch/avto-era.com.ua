...
{if $_GET['brand'] != "" || $_GET['p'] != "" || ($_GET['lp'] && $_GET['lp'] != $minPrice) || ($_GET['rp'] && $_GET['rp'] != $maxPrice)}
    <div class="frame-check-filter">
        <div class="inside-padd">
            <div class="title">Выбранные фильтры <span class="c_9">({echo count($products)})</span></div>
            ...
            <div class="foot-check-filter">
                <button type="button" onclick="location.href = '{site_url($CI->uri->uri_string())}'" class="btn-reset-filter pointer">
