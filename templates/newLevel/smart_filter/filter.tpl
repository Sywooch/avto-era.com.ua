            ...
            <div class="form-cost number">
                <div class="t-a_j">
                    ...
                    <div class="btn-not-avail v-a_t">
                        <button type="submit"><span class="text-el">ОК</span></button>
                    </div>
                </div>
            </div>
    ...
    {if count($brands) > 0}
        <div class="frame-group-checks">
            <div class="inside-padd">
                ...
                <ul class="filters-content" style="height: 300px; overflow-y: scroll; ">
                    {foreach $brands as $brand}
                        {if is_array($_GET['brand']) && in_array($brand->id, $_GET['brand'])}
                            ...
                            <div class="filters-content">
                                <ul>
                                    {foreach $prop->possibleValues as $item}
                                        {if is_array($_GET['p'][$prop->property_id]) && (in_array($item.value, $_GET['p'][$prop->property_id]) or in_array(htmlspecialchars_decode($item.value), $_GET['p'][$prop->property_id]))}
                                            {$check = 'checked="checked"'}
                                        {else:}
                                            {$check = ''}
                                        {/if}
                        ...
                        {else:}
                            <div class="lineForm">
                                <select name="p[{echo $prop->property_id}][]" id="p{echo $prop->property_id}" onchange="changeSelectFilter(this)">
                                    ...
                                    {foreach $prop->possibleValues as $item}
                                        {if is_array($_GET['p'][$prop->property_id]) && in_array($item.value, $_GET['p'][$prop->property_id])}
