...
function processBtnBuyCount(el) {
    ...
    el.find(genObj.btnBuy).each(function() {
        ...
        if (keys.indexOf(key) == -1) {
            ...
            $this.attr('title', $this.find(genObj.textEl).html())
...
/*additional front functions*/
function getCatSelect(el, sel){
    var $parent_frame = $("#"+sel).parents('.lineForm');
    $.ajax({
        type: 'post',
        url: '/shop/front_api/getCatSelectOptions/'+$(el).val()+'/'+sel,
        beforeSend: function(){
        },
        success: function(data){
            $parent_frame.empty().append(data);
            cuselInit(undefined, $('#'+sel));
            getPropSelect(el, 'diameter', 0);
        }
    });
    return false;
}
function getPropSelect(el, sel, no_input){
    var $parent_frame = $("#"+sel).parents('.lineForm');
    $.ajax({
        type: 'post',
        url: '/shop/front_api/getPropSelectOptions/'+$(el).val()+'/44/'+sel+'/'+no_input,
        beforeSend: function(){
        },
        success: function(data){
            $parent_frame.empty().append(data);
            cuselInit(undefined, $('#'+sel));
            setMainFilterFormAction();
        }
    });
    return false;
}
function setMainFilterFormAction(){
    //mainFilterForm
    var url = '';
    $('#mainFilterForm .datas').empty();
    if ($.exists('input[name=url_appointment]')){
        url = '/shop/category/'+$('input[name=url_appointment]').val();
        if ($.exists('input[name=url_season]') && url != ""){
            url += '/'+$('input[name=url_season]').val();
        }
        if ($('input[name=diameter]').val() != 0 && url != ""){
            $('#mainFilterForm .datas').append('<input type="hidden" name="p[44][]" value="'+$('input[name=diameter]').val()+'">');
        }
    }else{
        url = '';
    }
    ...
    if (url != ''){
        $('#mainFilterForm').attr('action', url);
        $('#mainFilterForm .btn-filter-bus button').attr('disabled',false);
    }else{
        $('#mainFilterForm').attr('action', "");
        $('#mainFilterForm .btn-filter-bus button').attr('disabled','disabled');
    }
}
