...
function changeDeliveryMethod(id) {
    ...
    $.get('/shop/cart_api/getPaymentsMethods/' + id, function(dataStr) {
        ...
        if (selectPayment)
            ...
            replaceStr = _.template('<div class="frame-radio"><% var i=0 %><% _.each(data, function(item) { %> <div class="frame-label"><span class = "niceRadio b_n"><input type = "radio" name = "paymentMethodId" value = "<%-item.id%>" <% if (i == 0){ %>checked = "checked"<% i++} %> /></span><div class="description"><div class = "name-count d_i-b"><span class = "text-el"><%-item.name%>&nbsp;</span></div><div class="help-block d_i-b"><%=item.description%></div></div></div> <% }) %></div>', {
                data: data
            });
        }
