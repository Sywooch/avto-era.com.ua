...
class Scaffolding {
	...
	function edit() {
		...
		// Run the query
		$query = $this->CI->db->get_where ( $this->current_table, array (
				$primary => $id 
		) );
	...
	function update() {
		...
		// Now do the query
		$this->CI->db->update ( $this->current_table, $_POST, array (
				$primary => $this->CI->uri->segment ( 4 ) 
		) );
	 ...
	 *
	 * @access public
	 * @return string HTML "delete confirm" page
	 */
	function delete() {
		...
		$data = array (
				...
				'no' => anchor ( array (
						$this->base_uri,
						'view' 
				), (! isset ( $this->lang ['scaff_no'] )) ? 'No' : $this->lang ['scaff_no'] ),
				'yes' => anchor ( array (
						$this->base_uri,
						'do_delete',
						$this->CI->uri->segment ( 4 ) 
				), (! isset ( $this->lang ['scaff_yes'] )) ? 'Yes' : $this->lang ['scaff_yes'] ) 
		);
	...
	function do_delete() {
		...
		header ( "Refresh:0;url=" . site_url ( array (
				$this->base_uri,
				'view' 
		) ) );
		exit ();
