<?php include ('/var/www/avto-era.com.ua/application/libraries/mabilis/functions/func.truncate.php');  ?>
<!-- ---------------------------------------------------Блок видалення---------------------------------------------------- -->
<div class="modal hide fade modal_del">
	...
	<div class="modal-footer">
		<a href="#" class="btn btn-primary"
			onclick="delete_function.deleteFunctionConfirm('<?php if(isset($ADMIN_URL)){ echo $ADMIN_URL; } ?>products/ajaxDeleteProducts')"><?php echo lang ('Delete','admin'); ?></a>
	...
	<div class="modal-body">
		<select name="" id="moveCategoryId" style="width: 285px;">
            <?php if(is_true_array($categories)){ foreach ($categories as $category){ ?>
                <option <?php if($category->getId() == $categoryId): ?>
				selected <?php endif; ?> value="<?php echo $category->getId()?>"><?php echo str_repeat ('-',$category->getLevel()); ?> <?php echo ShopCore::encode($category->getName())?></option>
            <?php }} ?>
        </select>
	</div>
	<div class="modal-footer">
		...
		<a href="#" class="btn"
			onclick="$('.modal_move_to_cat').modal('hide');"><?php echo lang ('Cancel','admin'); ?></a>
	</div>
</div>
...
<form
	action="<?php if(isset($ADMIN_URL)){ echo $ADMIN_URL; } ?>search/index/"
	id="filter_form" method="get">
	<section class="mini-layout">
		<div class="frame_title clearfix">
			<div class="pull-left">
				<span class="help-inline"></span> <span class="title"><?php if(isset($_GET['WithoutImages']) AND $_GET['WithoutImages'] == 1): ?><?php echo lang ('Products without images','admin'); ?><?php else:?><?php echo lang ('Products list','admin'); ?><?php endif; ?><?php if($totalProducts!=null): ?> (<?php echo $totalProducts?>)<?php endif; ?></span>
			</div>
			<div class="pull-right">
				<div class="d-i_b">

					<button title="<?php echo lang ('Filter','admin'); ?>"
						type="submit" class="btn btn-small">
						<i class="icon-filter"></i><?php echo lang ('Filter','admin'); ?></button>
					<a
						href="<?php if(isset($ADMIN_URL)){ echo $ADMIN_URL; } ?>search/index<?php if($_GET['WithoutImages'] == 1): ?>?WithoutImages=1<?php endif; ?>"
						title="<?php echo lang ('Reset filter','admin'); ?>" type="button"
						class="btn btn-small pjax"><i class="icon-refresh"></i><?php echo lang ('Cancel filter','admin'); ?></a>
					...
					<a class="btn btn-small btn-danger disabled action_on"
						...
						class="icon-trash icon-white"></i><?php echo lang ('Delete','admin'); ?></a>
					<a class="btn btn-small btn-success pjax"
						href="/admin/components/run/shop/products/create"><i
						class="icon-plus-sign icon-white"></i><?php echo lang ('Create','admin'); ?></a>
				</div>
			</div>
		</div>
		<div class="row-fluid">
				...
				<thead>
					...
					<tr class="head_body">
						<input type="hidden" name="WithoutImages"
							value="<?php echo $_GET['WithoutImages']; ?>" />
						<input type="hidden" name="orderMethod"
							value="<?php echo $_GET['orderMethod']; ?>" />
						<input type="hidden" name="order"
							value="<?php echo $_GET['order']; ?>" />
						<td class="t-a_c"></td>
						<td class="number">
							<div>
								<input name="filterID" type="text"
									value="<?php echo $_GET['filterID']; ?>" />
							</div>
						</td>
						<td><input type="text" name="text"
							value="<?php echo $_GET['text']; ?>" /></td>
                        ...
                        <?php if(is_true_array($categories)){ foreach ($categories as $category){ ?>
                            ...
                            <option
									...
									<?php if(isset($selected)){ echo $selected; } ?>><?php echo str_repeat ('-',$category->getLevel()); ?> <?php echo ShopCore::encode($category->getName())?></option>
                        <?php }} ?>
                    </select></td>
						<td><input type="text" name="number"
							value="<?php echo $_GET['number']; ?>" /></td>
						<td><select class="prodFilterSelect" name="Active">
								<option value=""><?php echo lang ('All','admin'); ?></option>
								<option value="true" <?php if($_GET['Active'] == 'true'): ?>
									selected="selected" <?php endif; ?>><?php echo lang ('Yes','admin'); ?></option>
								<option value="false" <?php if($_GET['Active'] == 'false'): ?>
									selected="selected" <?php endif; ?>><?php echo lang ('No','admin'); ?></option>
						</select></td>
						<td><select class="prodFilterSelect" name="s">
								<option value=""><?php echo lang ('All','admin'); ?></option>
								<option value="Hit" <?php if($_GET['s'] == 'Hit'): ?>
									selected="selected" <?php endif; ?>><?php echo lang ('Hit','admin'); ?></option>
								<option value="Hot" <?php if($_GET['s'] == 'Hot'): ?>
									selected="selected" <?php endif; ?>><?php echo lang ('New','admin'); ?></option>
								<option value="Action" <?php if($_GET['s'] == 'Action'): ?>
									selected="selected" <?php endif; ?>><?php echo lang ('Promotion','admin'); ?></option>
						</select></td>
						<td class="number"><label> <span class="pull-left"><span
									class="neigh_form_field help-inline"></span><?php echo lang ('From','admin'); ?>&nbsp;&nbsp;</span>
								<span class="o_h d_b"><input type="text" name="min_price"
									value="<?php echo $_GET['min_price']; ?>" /></span>
						</label> <label> <span class="pull-left"><span
									class="neigh_form_field help-inline"></span><?php echo lang ('To','admin'); ?>&nbsp;&nbsp;</span>
								<span class="o_h d_b"><input type="text" name="max_price"
									value="<?php echo $_GET['max_price']; ?>" /></span>
						</label></td>
                <?php echo form_csrf (); ?>
                </form>
					</tr>
				...
				<tbody>
                    <?php if(is_true_array($products)){ foreach ($products as $p){ ?>
						...
						<td class="share_alt"><a
							href="/shop/product/<?php echo $p->getUrl()?>" target="_blank"
							class="go_to_site pull-right btn btn-small" data-rel="tooltip"
							data-placement="top"
							data-original-title="<?php echo lang ('go to the website','admin'); ?>">
								<i class="icon-share-alt"></i>
						</a> <a
							href="/admin/components/run/shop/products/edit/<?php echo $p->getId()?><?php echo $_SESSION['ref_url']; ?>"
						...
						<td class="share_alt"><a
							href="/shop/category/<?php echo $p->getMainCategory()->getFullPath()?>"
							target="_blank" class="go_to_site pull-right btn btn-small"
							data-rel="tooltip" data-placement="top"
							data-original-title="<?php echo lang ('go to the website','admin'); ?>">
								<i class="icon-share-alt"></i>
						</a> <a
							href="<?php if(isset($ADMIN_URL)){ echo $ADMIN_URL; } ?>categories/edit/<?php echo $p->getMainCategory()->getId()?>"
							...
							<div class="frame_prod-on_off" data-rel="tooltip"
								...
								data-original-title="<?php echo lang ('show','admin'); ?>">
                                        <?php if($p->getActive() == true): ?>
                                            <span class="prod-on_off"
									data-id="<?php echo $p->getId()?>"></span>
                                        <?php else:?>
                                            <span
						...
						<td>
                                    <?php if($p->getHit() == true): ?>
                                        <button type="button"
								data-id="<?php echo $p->getId()?>"
								class="btn btn-small btn-primary active <?php if($p->getActive() != true): ?> disabled<?php endif; ?> setHit"
								data-rel="tooltip" data-placement="top"
								data-original-title="<?php echo lang ('hit','admin'); ?>">
								<i class="icon-fire"></i>
							</button>
                                        <?php else:?>
                                        <button type="button"
								data-id="<?php echo $p->getId()?>"
								class="btn btn-small <?php if($p->getActive() != true): ?> disabled<?php endif; ?> setHit"
								data-rel="tooltip" data-placement="top"
								data-original-title="<?php echo lang ('hit','admin'); ?>">
								<i class="icon-fire"></i>
							</button>
                                    ...
                                    <?php if($p->getHot() == true): ?>
                                        <button type="button"
								data-id="<?php echo $p->getId()?>"
								class="btn btn-small btn-primary active <?php if($p->getActive() != true): ?> disabled<?php endif; ?> setHot"
								data-rel="tooltip" data-placement="top"
								data-original-title="<?php echo lang ('novelty','admin'); ?>">
								<i class="icon-gift"></i>
							</button>
                                        <?php else:?>
                                        <button type="button"
								data-id="<?php echo $p->getId()?>"
								class="btn btn-small <?php if($p->getActive() != true): ?> disabled<?php endif; ?> setHot"
								data-rel="tooltip" data-placement="top"
								data-original-title="<?php echo lang ('novelty','admin'); ?>">
								<i class="icon-gift"></i>
							</button>
                                    ...
                                    <?php if($p->getAction() == true): ?>
                                        <button type="button"
								data-id="<?php echo $p->getId()?>"
								class="btn btn-small btn-primary active <?php if($p->getActive() != true): ?> disabled<?php endif; ?> setAction"
								data-rel="tooltip" data-placement="top"
								data-original-title="<?php echo lang ('promotion','admin'); ?>">
								<i class="icon-star"></i>
							</button>
                                        <?php else:?>
                                        <button type="button"
								data-id="<?php echo $p->getId()?>"
								class="btn btn-small <?php if($p->getActive() != true): ?> disabled<?php endif; ?> setAction"
								data-rel="tooltip" data-placement="top"
								data-original-title="<?php echo lang ('promotion','admin'); ?>">
								<i class="icon-star"></i>
							</button>
						...
						<td><span class="pull-right"><span
								class="neigh_form_field help-inline"></span>&nbsp;&nbsp;<?php echo ShopCore::app()->SCurrencyHelper->getSymbolById($variants[0]->getCurrency())?></span>
							<div class="p_r o_h frame_price number">
								...
								<button class="btn btn-small refresh_price"
									data-id="<?php echo $p->getId()?>" type="button">
							...
							<table>
								...
								<tbody>
									<tr>
										...
										<td class="share_alt"><a
											href="/shop/product/<?php echo $p->getUrl()?>"
											target="_blank" class="go_to_site pull-right btn btn-small"
											data-rel="tooltip" data-placement="top"
											data-original-title="<?php echo lang ('go to the website','admin'); ?>">
												<i class="icon-share-alt"></i>
										...
										<td>
											<div class="frame_prod-on_off" data-rel="tooltip"
												...
												data-original-title="<?php echo lang ('show','admin'); ?>">
                                                        <?php if($p->getActive() == true): ?>
                                                            <span
													class="prod-on_off" data-id="<?php echo $p->getId()?>"></span>
                                                        <?php else:?>
                                                            <span
													class="prod-on_off disable_tovar"
													data-id="<?php echo $p->getId()?>"></span>
										...
										<td>
                                                    <?php if($p->getHit() == true): ?>
                                                        <button
												type="button" data-id="<?php echo $p->getId()?>"
												class="btn btn-small btn-primary active <?php if($p->getActive() != true): ?> disabled<?php endif; ?> setHit"
												data-rel="tooltip" data-placement="top"
												data-original-title="<?php echo lang ('hit','admin'); ?>">
												<i class="icon-fire"></i>
										...
										<td><a href="#" class="t-d_n variants"><span class="js"><?php echo lang ('Variants','admin'); ?></span>
												<span class="f-s_14">↓</span></a></td>
									</tr>
												...
												<tbody class="sortable save_positions_variant"
													data-url="/admin/components/run/shop/search/save_positions_variant">
                                                            ...
                                                            <?php if(is_true_array($variants)){ foreach ($variants as $v){ ?>
                                                                ...
                                                                <tr
														data-title="<?php echo lang ('Drag product variant','admin'); ?>">
														...
														<td></td>
														<td><span class="pull-right"> <span
																class="neigh_form_field help-inline"></span>&nbsp;&nbsp;<?php echo ShopCore::app()->SCurrencyHelper->getSymbolById($v->getCurrency())?></span>
															<div class="p_r o_h frame_price number">
																<input type="text"
																...
																<button class="btn btn-small refresh_price"
																	...
																	variant-id="<?php echo $v->getId()?>" type="button">
																	<i class="icon-refresh"></i>
																</button>
															</div></td>
													</tr>
        ...
        <div class="clearfix">
            ...
            <div class="pagination pull-right"
                ...
                return false;">
                <?php if($_COOKIE['per_page'] == ShopCore::app()->SSettings->adminProductsPerPage): ?><option
						selected="selected" value="<?php echo $_COOKIE['per_page']?>"><?php echo $_COOKIE['per_page']?></option><?php endif; ?>
                <option <?php if($_COOKIE['per_page'] == 10): ?>
						selected="selected" <?php endif; ?> value="10">10</option>
					<option <?php if($_COOKIE['per_page'] == 20): ?>
						selected="selected" <?php endif; ?> value="20">20</option>
					<option <?php if($_COOKIE['per_page'] == 30): ?>
						selected="selected" <?php endif; ?> value="30">30</option>
					<option <?php if($_COOKIE['per_page'] == 40): ?>
						selected="selected" <?php endif; ?> value="40">40</option>
					<option <?php if($_COOKIE['per_page'] == 50): ?>
						selected="selected" <?php endif; ?> value="50">50</option>
					<option <?php if($_COOKIE['per_page'] == 60): ?>
						selected="selected" <?php endif; ?> value="60">60</option>
					<option <?php if($_COOKIE['per_page'] == 70): ?>
						selected="selected" <?php endif; ?> value="70">70</option>
					<option <?php if($_COOKIE['per_page'] == 80): ?>
						selected="selected" <?php endif; ?> value="80">80</option>
					<option <?php if($_COOKIE['per_page'] == 90): ?>
						selected="selected" <?php endif; ?> value="90">90</option>
					<option <?php if($_COOKIE['per_page'] == 100): ?>
						selected="selected" <?php endif; ?> value="100">100</option>
				</select>
			</div>
			<div class="pagination pull-right"
				style="margin-right: 10px; margin-top: 24px;"><?php echo lang ('At the products page','admin'); ?>:</div>
		</div>
		</div>
