            ...
            <div class="inside-padd clearfix">
                    ...
                    <div class="lineForm">
                        <select class="sort" id="sort" name="order">
                            ...
                            <?php if(is_true_array($sort)){ foreach ($sort as $s){ ?>
                                <option value="<?php echo $s['get']?>" <?php if($_GET['order']==$s['get']): ?>selected="selected"<?php endif; ?>><?php echo $s['name_front']?></option>
                            <?php }} ?>
                        </select>
                    </div>
                </div>
                        ...
                        <div class="lineForm">
                            ...
                            <select id="sort2" name="user_per_page">
                                <?php if(is_true_array($per_page_arr)){ foreach ($per_page_arr as $pp){ ?>
                                    <option <?php if($pp == $_GET['user_per_page']): ?>selected="selected"<?php endif; ?> value="<?php if(isset($pp)){ echo $pp; } ?>"><?php if(isset($pp)){ echo $pp; } ?></option>
                                <?php }} ?>
                            </select>
                        </div>
                    </div>
                ...
                <nav class="frame-catalog-view f-s_0 d_i-b">
                    ...
                    <ul class="tabs groups-buttons tabs-list-table" data-type="radio" data-elchange="#items-catalog-main" data-cookie="listtable">
                        <li class="btn-def <?php if($_COOKIE['listtable'] == 'table' || $_COOKIE['listtable'] == NULl): ?>active<?php endif; ?>">
                        ...
                        <li class="btn-def <?php if($_COOKIE['listtable'] == 'list'): ?>active<?php endif; ?>">
