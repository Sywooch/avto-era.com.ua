...
<section class="mini-layout">
	<div class="frame_title clearfix">
		<div class="pull-left">
			<span class="help-inline"></span> <span class="title"><?php echo ShopCore::encode($model->getName())?></span>
		</div>
		<div class="pull-right">
			<div class="d-i_b">
				<!-- $_SESSION['ref_url'] is session variable to save filter results to go back to it -->
				<a
					href="<?php if(isset($ADMIN_URL)){ echo $ADMIN_URL; } ?>search/index<?php echo $_SESSION['ref_url']; ?>"
					class="t-d_n m-r_15 pjax"><span class="f-s_14">←</span> <span
					class="t-d_u"><?php echo lang ('Back','admin'); ?></span></a>
				<button type="button"
					class="btn btn-small action_on formSubmit btn-primary"
					data-form="#image_upload_form" data-submit>
					<i class="icon-ok"></i><?php echo lang ('Save','admin'); ?></button>
				<button type="button" class="btn btn-small action_on formSubmit"
					data-form="#image_upload_form" data-action="close">
					<i class="icon-check"></i><?php echo lang ('Save and exit','admin'); ?></button>
                    <?php echo create_language_select($languages, $locale, "/admin/components/run/shop/products/edit/".$model->getId())?>
            </div>
		</div>
	</div>
	<form
		action="<?php if(isset($ADMIN_URL)){ echo $ADMIN_URL; } ?>products/edit/<?php echo $model->getId()?>/<?php if(isset($locale)){ echo $locale; } ?>"
			...
			<div class="clearfix">
				<div class="btn-group myTab m-t_20 pull-left"
                    ...
                    <?php if($moduleAdditions): ?>
                        <a href="#modules_additions"
						class="btn btn-small"><?php echo lang ('Modules additions', 'admin'); ?></a>
                    <?php endif; ?>
                </div>
				<div class="pull-right m-t_20">
					<a href="/shop/product/<?php echo $model->getUrl()?>"
						target="_blank" class="btn btn-small action_on"
						data-form="#image_upload_form" data-action="close"><i
						class="icon-share-alt m-r_5"></i><?php echo lang ('View page','admin'); ?></a>
					<a href=""></a>
				</div>
				<div class="pagination pagination-small pagination-centered">
					<ul>
						<li <?php if(!$prev_id): ?> class="disabled" <?php endif; ?>>
                            <?php if($prev_id): ?>
                                <a
							href="/admin/components/run/shop/products/edit/<?php echo $prev_id?><?php echo $_SESSION['ref_url']?>"
							class="pjax"><span>←</span> <?php echo lang ('Previous','admin'); ?></a>
                            <?php else:?>
						...
						<li <?php if(!$next_id): ?> class="disabled" <?php endif; ?>>
                            <?php if($next_id): ?>
                                <a
							href="/admin/components/run/shop/products/edit/<?php echo $next_id?><?php echo $_SESSION['ref_url']?>"
							class="pjax"><?php echo lang ('Next','admin'); ?> <span>→</span></a>
                            <?php else:?>
    ...
    <table>
		<tbody>
			<tr id="ProductVariantRow_">
				...
				<td class="number"><input type="text" name="variants[PriceInMain][]"
					value="" /></td>
                    ...
                    <td><select name="variants[currency][]">
							...
							<?php if($c->getMain()): ?> selected="selected" <?php endif; ?>><?php echo $c->getCode()?></option>
                            <?php }} ?>
                        </select></td>
                <?php endif; ?>
				...
				<td class="variantImage">
					<div class="control-group photo_album">
						<label class="control-label" style="display: none;"> <span
							class="btn btn-small p_r" data-url="file"> <i class="icon-camera"></i>
								<input type="file" class="newImage" name="image"
								title="<?php echo lang ('Main image','admin'); ?>"
						...
						<div class="controls">
							...
							<div class="btn-group f-s_0">
								...
								<button type="button" class="btn images_modal"
									data-rel="tooltip"
									data-title="Загрузить с интернета"
									data-original-title="">
