...
<div class="frame-inside page-product">
	<div class="container">
		<div
			class="clearfix item-product <?php if($model->firstVariant->getStock() == 0): ?>not-avail<?php endif; ?>">
			<div class="f-s_0 title-product">
				...
				<span class="frame-variant-name-code"> <span
					class="frame-variant-code"
					<?php if(!$model->firstVariant->getNumber()): ?>
					style="display: none;" <?php endif; ?>> <span class="c_9"> <span
							class="text-el">(</span><?php echo lang ('Код товара','newLevel'); ?>:
                            <span class="code">
                                <?php if($model->firstVariant->getNumber()): ?>
                                    <?php echo trim ($model->firstVariant->getNumber()); ?><span
								class="text-el">)</span>
                                <?php endif; ?>
                            </span>
					</span>
				</span> <span class="frame-variant-name"
						...
						<div class="lineForm">
							<select name="variant" id="variantSwitcher">
                                    <?php if(is_true_array($variants)){ foreach ($variants as $key => $productVariant){ ?>
                                        <option
									value="<?php echo $productVariant->getId()?>">
							...
							<div class="frame-prices-buy f-s_0 t-a_c">
								...
								<div class="frame-prices f-s_0 t-a_l">
                                    ...
                                    <?php if($model->firstVariant->toCurrency() > 0): ?>
                                        <span
										class="current-prices f-s_0"> <span class="price-new"> <span>
											...
											<span> (<span class="price addCurrPrice"><?php echo $model->firstVariant->toCurrency('Price',$NextCSId)?></span>
								...
								<div class="funcs-buttons">
                                    ...
                                    <?php if(is_true_array($variants)){ foreach ($variants as $key => $productVariant){ ?>
                                        ...
                                        <?php if($productVariant->getStock() > 0): ?>
                                            <div
										class="frame-count-buy variant_<?php echo $productVariant->getId()?> variant"
										<?php if($key != 0): ?> style="display: none" <?php endif; ?>>
										<div class="frame-count front-count">
											<div class="number"
												data-title="<?php echo lang ('Количество на складе','newLevel'); ?> <?php echo $productVariant->getstock()?>"
												data-prodid="<?php echo $model->getId()?>"
												data-varid="<?php echo $productVariant->getId()?>"
												data-rel="frameplusminus">
												...
												<input type="text" value="1" data-rel="plusminus"
													...
													data-min="1"
													data-max="<?php echo $productVariant->getstock()?>">
											</div>
										</div>
										<div class="btn-buy btn-buy-p">
											<button class="btnBuy infoBut" disabled="disabled"
												type="button"
												...
												<span class="icon_cleaner icon_cleaner_buy"></span> <span
													class="text-el"><?php echo lang ('В корзину'); ?></span>
											</button>
										</div>
									</div>
                                        <?php else:?>
										...
										<div
											class="variant_<?php echo $productVariant->getId()?> variant"
											<?php if($key != 0): ?> style="display: none" <?php endif; ?>>
											<div class="alert-exists"><?php echo lang ('Нет в наличии','newLevel'); ?></div>
											<div class="btn-not-avail">
												<button type="button" data-drop=".drop-report"
													...
													data-largeImage="<?php echo $productVariant->getlargePhoto()?>"
													class="infoBut">
													<span class="icon-but"></span> <span class="text-el"><?php echo lang ('Сообщить о появлении','newLevel'); ?></span>
												</button>
											</div>
										</div>
									</div>
								...
								<div class="frame-btn-compare">
									<div class="btn-compare"
										...
										<button class="toCompare pointer"
											data-prodid="<?php echo $model->getId()?>" type="button"
											...
											<span class="icon_compare"></span> <span
												class="text-el d_l_bl"><?php echo lang ('Добавиить к сравнению','newLevel'); ?></span>
										</button>
									</div>
								</div>
                                <?php if(is_true_array($variants)){ foreach ($variants as $key => $pv){ ?>
                                    <div
									class="frame-btn-wish variant_<?php echo $pv->getId()?> variant"
									<?php if($key != 0): ?> style="display: none" <?php endif; ?>
			...
			<div class="left-product">
				<!-- Start. Photo block-->
				<a rel="position: 'xBlock'" onclick="return false;"
					href="<?php echo $model->firstVariant->getLargePhoto()?>"
					class="frame-photo-title photoProduct cloud-zoom" id="photoProduct"
					title="<?php echo ShopCore::encode($model->getName())?>"
					data-drop="#photo" data-start="initDrop"> <span class="photo-block">
						<span class="helper"></span> <img
						src="<?php echo $model->firstVariant->getMainPhoto()?>"
						alt="<?php echo ShopCore::encode($model->getName())?>"
						title="<?php echo ShopCore::encode($model->getName())?> - <?php echo $model->getId()?>"
						class="vimgPr" />
                        <?php $discount = 0?>
                        <?php if($hasDiscounts): ?>
                ...
                <?php if($model->enable_comments && intval($Comments[$model->getId()]) !== 0): ?>
                    <div class="frame-star">
                        ...
                        <div class="d_i-b v-a_m">
						<button data-trigger="[data-href='#comment']" data-scroll="true"
							class="count-response pointer">
							<span class="text-el c_9">Всего:</span> <span
								class="d_l_red">
					...
					<div class="frame-thumbs carousel_js">
							...
							<ul class="items-thumbs items">
								...
								<li class="active"><a onclick="return false;"
									rel="useZoom: 'photoProduct'"
									href="<?php echo $model->firstVariant->getLargePhoto()?>"
									title="<?php echo ShopCore::encode($model->getName())?>"
									class="cloud-zoom-gallery" id="mainThumb"> <span
										class="photo-block"> <span class="helper"></span> <img
											src="<?php echo $model->firstVariant->getSmallPhoto()?>"
											alt="<?php echo ShopCore::encode($model->getName())?>"
											class="vimgPr" />
									</span>
								</a></li>
								<!-- End. main image-->
									...
									rel="useZoom: 'photoProduct'"
									href="<?php echo productImageUrl ('products/additional/'.$image->getImageName()); ?>"
									title="<?php echo ShopCore::encode($model->getName())?>"
									class="cloud-zoom-gallery"> <span class="photo-block"> <span
											class="helper"></span> <img
											src="<?php echo productImageUrl('products/additional/thumb_'.$image->getImageName())?>"
											alt="<?php echo ShopCore::encode($model->getName())?> - <?php echo ++$key?>" />
									</span>
								</a></li>
                                    <?php }} ?>
                                </ul>
