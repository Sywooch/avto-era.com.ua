...
class New_level extends MY_Controller {
	...
	public function OPI($model, $data = array(), $tpl = 'one_product_item') {
		\CMSFactory\assetManager::create ()->setData ( 'products', $model )->setData ( $data )->render ( $tpl, TRUE, FALSE );
	}
	...
	public function _install() {
		...
		$this->db->where ( 'identif', 'new_level' )->update ( 'components', array (
		...
		$this->db->where ( 'name', 'new_level' )->update ( 'components', array (
				'autoload' => '1',
				'enabled' => '1'
		) );
	}
