...
class New_level_model extends CI_Model {
	...
	public function getSettings() {
		$settings = $this->db->select ( 'settings' )->where ( 'identif', 'new_level' )->get ( 'components' )->row_array ();
	...
	public function setSettings($settings) {
		...
		return $this->db->where ( 'identif', 'new_level' )->update ( 'components', array (
	...
	public function getProperties() {
		...
		return $this->db->select ( 'shop_product_properties_i18n.id, shop_product_properties_i18n.name, mod_new_level_product_properties_types.type as type' )->join ( 'mod_new_level_product_properties_types', 'mod_new_level_product_properties_types.property_id=shop_product_properties_i18n.id', 'left' )->where ( 'shop_product_properties_i18n.locale', $locale )->get ( 'shop_product_properties_i18n' )->result_array ();
	}
	...
	public function setPropertyType($propety_id, $type) {
		$types = $this->db->select ( 'type' )->where ( 'property_id', $propety_id )->get ( 'mod_new_level_product_properties_types' )->row_array ();
		if (! $types) {
			$types = array ();
		} else {
		...
		if (! in_array ( $type, $types )) {
			...
			$this->db->where ( 'property_id', $propety_id )->update ( 'mod_new_level_product_properties_types', array (
					'type' => $types 
			) );
			if (! $this->db->affected_rows ()) {
				return $this->db->where ( 'property_id', $propety_id )->insert ( 'mod_new_level_product_properties_types', array (
						'property_id' => $propety_id,
						'type' => $types 
				) );
			}
		} else {
	...
	public function deletePropertyTypeFromSettings($del_type) {
		...
		$properties = $this->db->select ( 'property_id' )->get ( 'mod_new_level_product_properties_types' );
	...
	public function getPropertyTypes($property_id) {
		$result = $this->db->select ( 'type' )->where ( 'property_id', $property_id )->get ( 'mod_new_level_product_properties_types' )->row_array ();
		if ($result) {
	...
	public function saveCategories($categories_ids, $column) {
		...
		$this->db->where ( 'column', $column )->update ( 'mod_new_level_columns', array (
				'category_id' => serialize ( $categories_ids ) 
		) );
		if (! $column_exist) {
			return $this->db->insert ( 'mod_new_level_columns', array (
					'category_id' => serialize ( $categories_ids ),
					'column' => $column 
			) );
		}
	...
	public function clear_other_columns($categories_ids, $column) {
		...
		if ($query) {
			foreach ( $query->result_array () as $columns ) {
				...
				$this->db->where ( 'column', $columns ['column'] )->update ( 'mod_new_level_columns', array (
						'category_id' => serialize ( $categories ) 
				) );
			}
		} else {
			return FALSE;
	...
	public function deleteColumnFromSettings($del_column) {
		...
		$columns = $this->db->where ( 'column', $del_column )->delete ( 'mod_new_level_columns' );
	...
	public function editColumn($oldColumn, $newColumn) {
		...
		$this->db->where ( 'column', $oldColumn )->update ( 'mod_new_level_columns', array (
				'column' => $newColumn 
		) );
