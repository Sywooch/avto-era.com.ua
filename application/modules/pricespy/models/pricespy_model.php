...
class Pricespy_model extends CI_Model {
	...
	public function getSettings() {
		$settings = $this->db->select ( 'settings' )->where ( 'identif', 'pricespy' )->get ( 'components' )->row_array ();
	...
	public function setSpy($id, $varId, $productPrice) {
		return $this->db->set ( 'userId', $this->dx_auth->get_user_id () )->set ( 'productId', $id )->set ( 'productVariantId', $varId )->set ( 'productPrice', $productPrice )->set ( 'oldProductPrice', $productPrice )->set ( 'hash', random_string ( 'unique', 15 ) )->insert ( 'mod_price_spy' );
	}
}
