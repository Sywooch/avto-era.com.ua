...
class Pricespy extends MY_Controller {
	...
	public function index() {
		if ($this->dx_auth->is_logged_in ()) {
			\CMSFactory\assetManager::create ()->registerScript ( 'spy' );
			$this->renderUserSpys ();
		} else
			$this->core->error_404 ();
	}
	...
	public function priceUpdate($product) {
		...
		$CI = &get_instance ();
		$spys = $CI->db->from ( 'mod_price_spy' )->join ( 'shop_product_variants', 'mod_price_spy.productVariantId=shop_product_variants.id' )->join ( 'users', 'mod_price_spy.userId=users.id' )->join ( 'shop_products_i18n', 'shop_products_i18n.id=mod_price_spy.productId' )->where ( 'mod_price_spy.productId', $product [productId] )->get ()->result ();
	...
	public function init($model) {
		if ($this->dx_auth->is_logged_in ()) {
			...
			$products = $this->db->where_in ( 'productVariantId', $varId )->where ( 'userId', $this->dx_auth->get_user_id () )->get ( 'mod_price_spy' )->result_array ();
	...
	public function renderButton($id, $varId) {
		if ($this->dx_auth->is_logged_in ()) {
			...
			if ($this->isInSpy [$varId] == '')
				\CMSFactory\assetManager::create ()->setData ( 'data', $data )->setData ( 'value', lang ( 'Notify about price cut', 'pricespy' ) )->setData ( 'class', 'btn' )->render ( 'button', true );
			else
				\CMSFactory\assetManager::create ()->setData ( 'data', $data )->setData ( 'value', lang ( 'Already in tracking', 'pricespy' ) )->setData ( 'class', 'btn inSpy' )->render ( 'button', true );
		}
	}
	...
	private function renderUserSpys() {
		$products = $this->db->where ( 'userId', $this->dx_auth->get_user_id () )->join ( 'shop_product_variants', 'shop_product_variants.id=mod_price_spy.productVariantId' )->join ( 'shop_products_i18n', 'shop_products_i18n.id=mod_price_spy.productId' )->join ( 'shop_products', 'shop_products.id=mod_price_spy.productId' )->get ( 'mod_price_spy' )->result_array ();
		
		\CMSFactory\assetManager::create ()->setData ( 'products', $products )->render ( 'spys' );
	}
