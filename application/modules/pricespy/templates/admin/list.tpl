		...
		<tbody>
			...
			<tr id='{echo $product->hash}'>
				...
				<td><input type="submit"
					...
					onclick="unspy('{echo $product->hash}')" /></td>
				<td><a href="{shop_url($product->url)}"
					title="{echo $product->name}"> {echo $product->name} </a></td>
				<td>{echo round($product->productPrice,
					ShopCore::app()->SSettings->pricePrecision)}</td>
				<td>{echo round($product->oldProductPrice,
					ShopCore::app()->SSettings->pricePrecision)}</td>
				<td>{echo
					round(($product->productPrice-$product->oldProductPrice)*100/$product->oldProductPrice,
					2)} %</td>
			</tr>
			{/foreach}
		</tbody>
