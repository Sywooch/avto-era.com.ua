...
;
	...
	function _mainpage() {
		switch ($this->settings ['main_type']) {
			...
			case 'page' :
				...
				$this->template->assign ( 'content', $this->template->read ( $page_tpl, array (
						'page' => $page 
				) ) );
	...
	function _display_category($category = array()) {
		...
		if ($cnt > 0) {
			...
			$content = $this->template->read ( $cat_tpl, array (
					'pages' => $pages 
			) );
		} else {
			($hook = get_hook ( 'core_dispcat_no_pages' )) ? eval ( $hook ) : NULL;
			$content = $this->template->read ( $cat_tpl, array (
					'no_pages' => lang ( "In the category has no pages.", "core" ) 
			) );
		}
	...
	function error($text, $back = TRUE) {
		...
		$this->template->add_array ( array (
				'content' => $this->template->read ( 'error', array (
						'error_text' => $text,
						'back_button' => $back 
				) ) 
		) );
	...
	public function set_meta_tags($title = '', $keywords = '', $description = '', $page_number = '', $showsitename = 0, $category = '') {
		...
		if ($this->core_data ['data_type'] == 'main') {
			...
			if ($description != '' && $page_number > 1)
				$description = "$page_number - $description";
			...
			if ($this->core_data ['data_type'] == 'category') {
				if (is_numeric ( $this->uri->segment ( $this->uri->total_segments () ) )) {
					$offset = $this->uri->segment ( $this->uri->total_segments () );
					$seg = $this->uri->segment ( $this->uri->total_segments () - 1 );
					$pp = $this->cat_content [per_page];
					$pp = $offset / $pp + 1;
					$title = $pp . " - " . $title;
	...
	private function user_browser($agent) {
		...
		if ($browser == 'MSIE') {
			...
			return $browserIn = array (
					'0' => 'IE',
					'1' => $version 
			); // otherwise just return the IE and the version number
		}
		if ($browser == 'Firefox') {
			...
			if ($ff)
				return $browserIn = array (
						'0' => $ff [1],
						'1' => $ff [2] 
				); // if so, shows the number and version
		}
		...
		return $browserIn = array (
				'0' => $browser,
				'1' => $version 
		); // for the rest of the browser and return the version
	}
}
