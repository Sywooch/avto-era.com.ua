...
class Stat {
	...
	function createCampaign($sender, $text, $list_id, $datetime, $batch, $batchinterval, $sms_lifetime, $controlnumber) {
		return $this->gateway->execCommad ( 'createCampaign', array (
				'sender' => $sender,
				'text' => $text,
				'list_id' => $list_id,
				'datetime' => $datetime,
	...
	function sendSMS($sender, $text, $phone, $datetime, $sms_lifetime) {
		return $this->gateway->execCommad ( 'sendSMS', array (
				'sender' => $sender,
				'text' => $text,
				'phone' => $phone,
				'datetime' => $datetime,
	...
	function getCampaignInfo($id) {
		return $this->gateway->execCommad ( 'getCampaignInfo', array (
				'id' => $id 
		) );
	}
	...
	function getCampaignDeliveryStats($id, $datefrom = "") {
		return $this->gateway->execCommad ( 'getCampaignDeliveryStats', array (
				'id' => $id,
				'datefrom' => $datefrom 
		) );
	}
	...
	function cancelCampaign($id) {
		return $this->gateway->execCommad ( 'cancelCampaign', array (
				'id' => $id 
		) );
	}
	...
	function deleteCampaign($id) {
		return $this->gateway->execCommad ( 'deleteCampaign', array (
				'id' => $id 
		) );
	}
	...
	function checkCampaignPrice($sender, $text, $list_id) {
		return $this->gateway->execCommad ( "checkCampaignPrice", array (
				'sender' => $sender,
				'text' => $text,
				'list_id' => $list_id 
		) );
