...
class Admin extends BaseAdminController {
	...
	public function getSettings() {
		$settings = $this->db->select ( 'settings' )->where ( 'identif', 'translator' )->get ( 'components' )->row_array ();
	...
	public function setSettings($settings) {
		...
		return $this->db->where ( 'identif', 'translator' )->update ( 'components', array (
	...
	public function index($isExchange = FALSE) {
		...
		\CMSFactory\assetManager::create ()->registerScript ( 'admin' )->registerStyle ( 'admin' )->setData ( 'langs', $this->langs )->setData ( 'settings', $this->getSettings () )->setData ( 'locales', $locales_unique )->renderAdmin ( 'list' );
	...
	public function exchangeTranslation() {
		if ($_POST) {
			...
			if ($langExchanger && $langReceiver && $typeExchanger && $typeReceiver) {
				...
				$this->exchangePoArray = \CMSFactory\assetManager::create ()->setData ( 'po_settings', $this->po_settings )->setData ( 'po_array', $receiverPoArray )->setData ( 'paths', $this->paths )->setData ( 'page', 1 )->fetchAdminTemplate ( 'po_table' );
				return $this->index ( TRUE );
			}
		} else {
			$this->scanLangFiles ();
			\CMSFactory\assetManager::create ()->registerScript ( 'admin' )->registerStyle ( 'admin' )->setData ( 'langs', $this->langs )->renderAdmin ( 'exchange' );
		}
	}
					...
					if (is_dir ( $this->templates_path . $template . '/language/' . $template )) {
						...
						foreach ( $langs as $lang ) {
							if ($lang && $lang != '.' && $lang != '..' && $lang [0] != '.' && is_dir ( $this->templates_path . $template . '/language/' . $template . '/' . $lang ) && $this->isLocale ( $lang )) {
								$this->langs_templates [$lang] [] = array (
										'template' => $template 
								);
							}
						}
	...
	public function renderModulesNames($lang) {
		...
		$langs = $langs [$lang];
		return \CMSFactory\assetManager::create ()->setData ( 'langs', $langs )->fetchAdminTemplate ( 'modules_names' );
	}
	public function renderTemplatesNames($lang) {
		$langs = $this->session->userdata ( 'langs_templates' );
		$langs = $langs [$lang];
		return \CMSFactory\assetManager::create ()->setData ( 'langs', $langs )->fetchAdminTemplate ( 'templates_names' );
	}
	public function renderModulePoFile($module_template, $type, $lang, $offset = 0, $limit = 10) {
		...
		if ($translations != "no file") {
			...
			return \CMSFactory\assetManager::create ()->setData ( 'po_settings', $this->po_settings )->setData ( 'po_array', $translations )->setData ( 'paths', $this->paths )->setData ( 'page', $page )->setData ( 'rows_count', ceil ( count ( $translations ) / ($limit + 1) ) )->fetchAdminTemplate ( 'po_table' );
		} else {
			return 'no file';
		}
	}
	...
	public function translate() {
		...
		if ($po_array) {
			...
			return \CMSFactory\assetManager::create ()->setData ( 'po_array', $po_array )->setData ( 'page', 1 )->setData ( 'rows_count', ceil ( count ( $po_array ) / 11 ) )->fetchAdminTemplate ( 'po_table' );
		}
	}
	...
	public function makeCorrectPoPaths($module_template, $type, $lang) {
		...
		return \CMSFactory\assetManager::create ()->setData ( 'po_array', $currentLangs )->setData ( 'page', 1 )->setData ( 'rows_count', ceil ( count ( $currentLangs ) / 11 ) )->fetchAdminTemplate ( 'po_table' );
	}
	...
	public function update($module_template, $type, $lang) {
		...
		return \CMSFactory\assetManager::create ()->setData ( 'po_array', $result_array )->setData ( 'page', 1 )->setData ( 'rows_count', ceil ( count ( $result_array ) / 11 ) )->fetchAdminTemplate ( 'po_table' );
	}
