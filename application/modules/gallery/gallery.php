...
class Gallery extends MY_Controller {
	...
	function index() {
		...
		\CMSFactory\assetManager::create ()->setData ( $data )->render ( 'albums' );
	}
	...
	function album($id = 0) {
		...
		if ($album == FALSE) {
			...
			$this->core->set_meta_tags ( array (
					$album ['name']
			) );
				
			if ($album ['tpl_file']) {
				\CMSFactory\assetManager::create ()->setData ( $data )->render ( $album ['tpl_file'] );
			} else {
				\CMSFactory\assetManager::create ()->setData ( $data )->render ( 'album' );
			}
		}
	}
	function thumbnails($id = 0, $page = 0) {
		...
		\CMSFactory\assetManager::create ()->setData ( $data )->render ( 'thumbnails' );
	}
	function post_comment() {
		...
		if ($this->db->get_where ( 'gallery_images', array (
				'id' => $image_id
		) )->num_rows () > 0) {
