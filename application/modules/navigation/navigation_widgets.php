		...
		switch ($data_type) {
			...
			case 'brand' :
				if ($this->core->core_data ['id'] != null) {
					$ci = &get_instance ();
					$brand = $ci->db->select ( array (
							'url',
							'name'
					) )->where ( array (
							'shop_brands.id' => $this->core->core_data ['id'],
							'shop_brands_i18n.locale' => MY_Controller::getCurrentLocale ()
					) )->join ( 'shop_brands_i18n', 'shop_brands_i18n.id=shop_brands.id' )->limit ( 1 )->get ( 'shop_brands' )->row_array ();
					$navi_cats [] = array (
							'path_url' => 'shop/brand/',
			...
			case 'compare' :
				$navi_cats [] = array (
				'path_url' => 'shop/compare/',
				'name' => lang ( 'Compare', 'navigation' )
				);
				$tpl_data = array (
						'navi_cats' => $navi_cats
				);
			...
			case 'wish_list' :
				$navi_cats [] = array (
				'path_url' => 'shop/wish_list/',
				'name' => lang ( 'Wish List', 'navigation' )
				);
				$tpl_data = array (
						'navi_cats' => $navi_cats
				);
			...
			case 'profile' :
				$navi_cats [] = array (
				'path_url' => 'shop/profile/',
				'name' => lang ( 'Profile', 'navigation' )
				);
				$tpl_data = array (
						'navi_cats' => $navi_cats
				);
			...
			case 'search' :
				$navi_cats [] = array (
				'path_url' => 'shop/search/',
				'name' => lang ( 'Search', 'navigation' )
				);
				$tpl_data = array (
						'navi_cats' => $navi_cats
				);
			...
			case 'shop_category' :
				if ($this->core->core_data ['id'] != null && $this->core->core_data > 0) {
					// get category object
					$ci = &get_instance ();
					$shop_category = $ci->db->select ( array (
							'full_path_ids',
							'url',
							'name'
					) )->where ( array (
							'shop_category.id' => $this->core->core_data ['id'],
							'shop_category_i18n.locale' => MY_Controller::getCurrentLocale ()
					) )->join ( 'shop_category_i18n', 'shop_category_i18n.id=shop_category.id' )->limit ( 1 )->get ( 'shop_category' );
					if ($shop_category) {
			...
			case 'product' :
				if ($this->core->core_data ['id'] != null && $this->core->core_data ['id'] > 0) {
					$ci = &get_instance ();
					// get product model
					$product = $ci->db->select ( array (
							'name',
							'category_id'
					) )->where ( array (
							'shop_products.id' => $this->core->core_data ['id'],
							'locale' => MY_Controller::getCurrentLocale ()
					) )->join ( 'shop_products_i18n', 'shop_products_i18n.id=shop_products.id' )->get ( 'shop_products' );
					if ($product) {
						...
						if ($product ['category_id'] != null && $product ['category_id'] > 0) {
							$shop_category = $ci->db->select ( array (
									...
									'shop_category.id' => $product ['category_id'],
									'shop_category_i18n.locale' => MY_Controller::getCurrentLocale ()
							) )->join ( 'shop_category_i18n', 'shop_category_i18n.id=shop_category.id' )->limit ( 1 )->get ( 'shop_category' );
							if ($shop_category) {
								...
								$full_path_ids = unserialize ( $full_path_ids );
								if (is_array ( $full_path_ids ) && ! empty ( $full_path_ids )) {
									$result = $ci->db->select ( array (
											'full_path',
											'name'
									) )->where ( 'locale', MY_Controller::getCurrentLocale () )->where_in ( 'shop_category.id', $full_path_ids )->join ( 'shop_category_i18n', 'shop_category_i18n.id=shop_category.id' )->get ( 'shop_category' );
									if ($result) {
										$result = $result->result_array ();
										foreach ( $result as $key => $value ) {
											$result [$key] ['path_url'] = 'shop/category/' . $result [$key] ['full_path'];
										...
										$result [] = array (
												'path_url' => 'shop/category/' . $shop_category ['full_path'],
