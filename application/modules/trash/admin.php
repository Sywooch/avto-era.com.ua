...
class Admin extends BaseAdminController {
	...
	function create_trash() {
		...
		if (count ( $this->db->get_where ( 'components', array (
				'name' => 'shop' 
		) )->row () ) > 0) {
		...
		if ($_POST) {
			if ($this->form_validation->run ( $this ) == FALSE) {
				...
				switch ($this->input->post ( 'redirect_type' )) {
					...
					case "product" :
						$query = $this->db->get_where ( 'shop_products', array (
					...
					case "category" :
						$query = $this->db->get_where ( 'shop_category', array (
					...
					case "basecategory" :
						$query = $this->db->get_where ( 'category', array (
								'id' => $this->input->post ( 'category_base' ) 
						) );
						$url = $query->row ();
						$array = array (
	...
	function edit_trash($id) {
		$query = $this->db->get_where ( 'trash', array (
				'id' => $id 
		) );
		$this->template->add_array ( array (
				'trash' => $query->row () 
