...
class Search extends \Search\BaseSearch {
	...
	public function ac() {
		...
		if (mb_strlen ( $this->input->post ( 'queryString' ) ) >= 3) {
			...
			$res = $this->db->select ( 'DISTINCT shop_product_variants.product_id as product_id, name, url, shop_products.smallModImage, shop_product_variants.mainImage, shop_product_variants.price as price, shop_product_variants.id' )->join ( 'shop_products_i18n', 'shop_products.id = shop_products_i18n.id' )->join ( 'shop_product_variants', 'shop_products.id = shop_product_variants.product_id' )->like ( 'name', trim ( $_POST ['queryString'] ) )->or_like ( 'number', trim ( $_POST ['queryString'] ) )->where ( 'active', 1 )->where ( 'locale', \MY_Controller::getCurrentLocale () )->order_by ( 'shop_product_variants.position' )->group_by ( 'shop_products.id' )->limit ( 5 )->distinct ()->get ( 'shop_products' )->result_array ();
