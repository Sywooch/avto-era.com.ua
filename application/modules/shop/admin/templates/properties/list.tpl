<div class="container">
    ...
    <div class="modal hide fade modal_del">
        ...
        <div class="modal-footer">
            <a href="#" class="btn btn-primary" onclick="delete_function.deleteFunctionConfirm('{$ADMIN_URL}properties/delete')" >{lang('Delete','admin')}</a>
    ...
    <section class="mini-layout">
            ...
            <div class="pull-right">
                ...
                <div class="d-i_b">
                    <div class="d-i_b input-large">
                        <select name="CategoryId" class="catfilter" style="margin-bottom: 0px;">
                            <option value="0">- {lang('All','admin')} {lang('Categories','admin')} -</option>
                    ...
                    <button type="button" class="btn btn-small btn-danger disabled action_on" onclick="delete_function.deleteFunction()" id="del_sel_property"><i class="icon-trash icon-white"></i>{lang('Delete','admin')}</button>
                    <a class="btn btn-small btn-success pjax" href="/admin/components/run/shop/properties/create" ><i class="icon-plus-sign icon-white"></i>{lang('Create a property','admin')}</a>
                </div>
            </div>  
        </div>
        <div class="tab-content">
                ...
                <div class="row-fluid">
                    <form method="post" action="#" class="form-horizontal">
                        <table class="table table-striped table-bordered table-hover table-condensed content_big_td">
                            ...
                            <tbody class="sortable save_positions" data-url="/admin/components/run/shop/properties/save_positions">
                                {foreach $model as $p}
                                    ...
                                    <tr data-id="{echo $p->getId()}">
                                        ...
                                        <td>{echo $p->getId()}</td>
                                        <td>
                                            <a class="pjax" href="{$ADMIN_URL}properties/edit/{echo $p->getId()}/{echo $locale}" data-rel="tooltip" data-title="{lang('Editing','admin')}">{truncate(ShopCore::encode($p->getName()),100)}</a>
                                        </td>
                                        ...
                                        <td class="span1">
                                            <div class="frame_prod-on_off" data-rel="tooltip" data-placement="top" data-original-title="{lang('switch on','admin')}"  data-off="{lang('switch off','admin')}">
                                                <span class="prod-on_off prop_active {if $p->getActive() != 1}disable_tovar{/if}" data-id="{echo $p->getId()}"></span>
                                            </div>
                                        </td>
                                    </tr>
                                {/foreach}
            ...
            <div class="pagination pull-right" style="margin-right: 10px; margin-top: 24px;">{lang('At the properties page','admin')}:</div>
        </div>
