        ...
        <div class="content_big_td">
            <div class="clearfix">
                ...
                <div class="pull-right m-t_20">
                    <a href="/shop/product/{echo $model->getUrl()}" target="_blank" class="btn btn-small action_on" data-form="#image_upload_form" data-action="close"><i class="icon-share-alt m-r_5"></i>{lang('View page','admin')}</a>
                    <a href=""></a>
                </div>
                <div class="pagination pagination-small pagination-centered">
                    <ul>
                        <li {if !$prev_id}class="disabled"{/if}>
                            {if $prev_id}
                                <a href="/admin/components/run/shop/products/edit/{echo $prev_id}{echo $_SESSION['ref_url']}" class="pjax"><span>←</span> {lang('Previous','admin')}</a>
                            {else:}
                                <span><span>←</span> {lang('Previous','admin')}</span>
                            {/if}
                        </li>
                        <li {if !$next_id}class="disabled"{/if}>
                            {if $next_id}
                                <a href="/admin/components/run/shop/products/edit/{echo $next_id}{echo $_SESSION['ref_url']}" class="pjax">{lang('Next','admin')} <span>→</span></a>
                            {else:}
                                <span>{lang('Next','admin')} <span>→</span></span>
                            {/if}
                        </li>
                    </ul>
                </div>
