					...
					<div class="inside_padd">
						<div class="form-horizontal">
							<div data-frame>
								...
								<div class="span3">
									<span class="v-a_m m-r_5">{lang('Active','admin')}:</span> {if
									$model->getActive() == true} {$checked =
									'checked="checked"';$checkedP = '';$disableClass='';} {else:}
									{$checkedP = 'disable_tovar';$checked =
									'';$disableClass='disabled';} {/if}
									<div class="frame_prod-on_off v-a_m" data-rel="tooltip"
										data-placement="top"
										data-original-title="{lang('show','admin')}">
										<span data-page="tovar" class="prod-on_off {echo $checkedP;}"
											data-id="{echo $model->getId()}"></span> <input
								...
								<div class="control-group m-t_10">
									...
									<div class="controls">
										...
										<button type="button" data-id="{echo $model->getId()}"
											class="btn btn-small {echo $disableClass;} btn-primary active setHit"
										...
										{else:}
										<button type="button" data-id="{echo $model->getId()}"
											class="btn btn-small {echo $disableClass;} setHit"
											data-rel="tooltip" data-placement="top"
											data-original-title="{lang('hit','admin')}">
											<i class="icon-fire"></i> {lang('Hit','admin')}
										</button>
										{/if} {if $model->getHot() == true}
										<button type="button" data-id="{echo $model->getId()}"
											class="btn btn-small {echo $disableClass;} btn-primary active setHot"
											data-rel="tooltip" data-placement="top"
										...
										{else:}
										<button type="button" data-id="{echo $model->getId()}"
											class="btn btn-small {echo $disableClass;} setHot"
											data-rel="tooltip" data-placement="top"
											data-original-title="{lang('novelty','admin')}">
											<i class="icon-gift"></i> {lang('Novelty','admin')}
										</button>
										{/if} {if $model->getAction() == true}
										<button type="button" data-id="{echo $model->getId()}"
											class="btn btn-small {echo $disableClass;} btn-primary active setAction"
											data-rel="tooltip" data-placement="top"
										...
										{else:}
										<button type="button" data-id="{echo $model->getId()}"
											class="btn btn-small {echo $disableClass;} setAction"
											data-rel="tooltip" data-placement="top"
											data-original-title="{lang('promotion','admin')}">
											<i class="icon-star"></i> {lang('Promotion','admin')}
										</button>
								...
								<div class="variantsProduct">
									<table class="table table-bordered t-l_a">
										<thead>
											<tr>
												<th class="span3 vName">{echo $translatable_w}
													{lang('Product variant name','admin')}</th>
												<th class="span2">{echo
										...
										<tbody class="sortable save_positions_variant"
											...
											$model->getProductVariants(null,null, TRUE, $locale) as $v}

											<tr id="ProductVariantRow_{$i}">
												<td class="vName"><input name="idv" type="hidden"
													value="{echo $v->id}" /> {if $i>0}
													<button
													...
													<div class="o_h">
														...
														<input type="text" name="variants[Name][]"
															value="{echo ShopCore::encode($v->getName())}" />
													</div></td>
													...
													<div class="control-group">
														<label class="control-label" style="display: none;">
															<span class="btn btn-small p_r" data-url="file">
																...
																name="image{$i}" title="{lang('Main image','admin')}" />
																<input type="hidden" name="changeImage[]" value=""
																class="changeImage" /> <input type="hidden"
																name="variants[MainImageForDel][{echo $v->getId()}]"
																class="deleteImage" value="0" />
														</span>
														</label>
														<div class="controls photo_album">
															...
															<div class="btn-group f-s_0">
																...
																<button type="button" class="btn delete_image"
																	data-rel="tooltip"
																	data-title="{lang('Remove','admin')}">
																	<i class="icon-remove"></i>
																</button>
															</div>
															{if $v->getMainPhoto()} <img
																src="{echo $v->getMainPhoto()}" class="img-polaroid"
																style="width: 100px;"> {else:} <img
																src="{$THEME}images/select-picture.png"
																class="img-polaroid " style="width: 50px;"> {/if}
														</div>
													</div>
												</td>
											</tr>
										...
										<div class="controls">
											<select name="CategoryId" id="comment"> {foreach
												...
												<option {if $category->getLevel() ==
													...
													$model->getCategoryId() ==
													$category->getId()}selected="selected"{/if} value="{echo
									...
									<div class="control-group">
										...
										<div class="controls">
											<select name="Categories[]" multiple="multiple"
												...
												$category} {$selected=""} {if in_array($category->getId(),
												$productCategories) && $category->getId() !=
												$model->getCategoryId()} {$selected="selected='selected'"}
												{/if}
												<option {if $category->getLevel() ==
													0}style="font-weight: bold;"{/if} {$selected} value="{echo
						...
						<div class="form-horizontal">
							<div class="control-group">
								<label class="control-label" for="pName">{lang('Warehouses','admin')}:
									<button type="button" data-clone="wares" class="btn btn-small">
										<i class="icon-plus"></i>
									</button>
									<br />
								</label>
							...
							<div class="control-group">
								...
								<div class="controls">
									<select id="comments" class="input-mini" name="EnableComments">
										...
										<option {if !$model->getEnableComments()} selected
											{/if}value="0">{lang('No','admin')}</option>
									</select>
								</div>
							</div>
									...
									<div id="relatedProductsNames" style="margin-top: 10px;">
										...
										<div id="tpm_row{echo $shopKitProduct->getId()}">
											<span style="width: 70%; margin-left: 1%;" class="pull-left">
												<a
												href="{echo $ADMIN_URL .'products/edit/'.$shopKitProduct->getId()}">
													{echo ShopCore::encode($shopKitProduct->getName())} </a> <input
												type="hidden" name="RelatedProducts[]"
												value="{echo $shopKitProduct->getId()}">
											</span> <span style="width: 8%; margin-left: 1%;" class="pull-left">
												<button class="btn btn-small del_tmp_row" data-rel="tooltip"
													data-title="{lang('Delete','admin')}"
						...
						<div class="form-horizontal">
							...
							<div class="control-group">
								...
								<div class="controls">
									...
									<span class="o_h d_b"> <input type="text" name="Url"
										id="Url" value="{echo $model->getUrl()}" />
									</span>
								</div>
							</div>
							<div class="control-group">
									...
									<div class="control-group span6">
										<label class="control-label" for="additionalImage_{$i}">
											<span class="btn btn-small p_r" data-url="file"> <i
												...
												class="btn-small btn additional_image_file"
												id="additionalImage_{$i}" name="additionalImages{echo $i}">
												<input type="hidden" class='additional_image_url'
												id='add_img_urls_{$i}' name='add_img_urls_{$i}'>
										</span>
										</label>
										<div class="controls">
											{if isset($additionalImagePositions[$i])} <img
												src="/uploads/shop/products/origin/additional/{echo $additionalImagePositions[$i]->getImageName()}"
...
<div class="tab-pane" id="kits">
	<a style="float: right;" class="btn btn-mini btn-success pjax"
		...
		<i class="icon-plus-sign icon-white"></i>{lang('Create a
		set','admin')}
	</a> {if count($model->getKits($criteria))}
	<table
		class="table table-striped table-bordered table-hover table-condensed">
		<thead>
			<tr>
					...
					<table
						...
						<thead>
							<tr>
								...
								<th>{lang('Position','admin')}: {echo $kit->getPosition()}
									<button type="button" style="float: right"
										class="btn btn-mini kit_del" data-kid="{echo $kit->getId()}">
										<i title="{lang('Delete set','admin')}" class="icon-trash"></i>
									</button>
									<button type="button" style="float: right; margin-right: 2px;"
	...
	<div class="alert alert-info"
		style="margin-bottom: 18px; margin-top: 71px;">{lang('Kit List
		products are empty','admin')}</div>
	{/if}
</div>
