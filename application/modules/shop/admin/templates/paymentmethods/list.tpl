<div class="container">
	...
	<div class="modal hide fade modal_del">
		...
		<div class="modal-footer">
			<a href="#" class="btn btn-primary"
				onclick="delete_function.deleteFunctionConfirm('{$ADMIN_URL}paymentmethods/deleteAll')">{lang('Delete','admin')}</a>
	...
	<div id="delete_dialog"
		title="{lang('Removal of payment methods','admin')}"
		style="display: none">{lang('Delete method of
		payment?','admin')}</div>
	<!-- ---------------------------------------------------Блок видалення---------------------------------------------------- -->
	...
	<section class="mini-layout">
		<div class="frame_title clearfix">
			<div class="pull-left">
				<span class="help-inline"></span> <span class="title">{lang('Payment
					methods list','admin')}</span>
			</div>
			<div class="pull-right">
				<div class="d-i_b">
					<a class="btn btn-small btn-success pjax"
						href="/admin/components/run/shop/paymentmethods/create"><i
						class="icon-plus-sign icon-white"></i>{lang('Create a payment
						method','admin')}</a>
					<button type="button"
						class="btn btn-small btn-danger disabled action_on"
						id="del_sel_pm" onclick="delete_function.deleteFunction()">
		...
		<div class="tab-content">
			...
			<table
				...
				<thead>
					<tr>
						...
						<th class="span4">{lang('Currency name','admin')}</th>
						<th class="span4">{lang('Currency symbol','admin')}</th>
						<!--                                <th class="span4">{lang('Position','admin')}</th>-->
						<th class="span2 t-a_c">{lang('Active','admin')}</th>
					</tr>
				</thead>
				<tbody class="sortable save_positions"
					data-url="/admin/components/run/shop/paymentmethods/savePositions">
					...
					<tr>
						...
						<td><a class="pjax"
							href="{$ADMIN_URL}paymentmethods/edit/{echo $c->getId()}"
							data-rel="tooltip" data-title="{lang('Editing','admin')}">{echo
								ShopCore::encode($c->getName())}</a></td>
						<td><p>{echo
								ShopCore::encode($c->getCurrency()->getName())}</p></td>
						<td><p>{echo
								ShopCore::encode($c->getCurrency()->getSymbol())}</p></td>
						...
						<td class="t-a_c">
							...
							<div class="frame_prod-on_off" data-rel="tooltip"
								...
								onclick="change_status('{$ADMIN_URL}paymentmethods/change_payment_status/{echo $c->getId()}');">
								<span
									class="prod-on_off {if $c->getActive() == NULL}disable_tovar{/if}"></span>
							</div>
						</td>
					</tr>
					{/foreach}
			...
			<div class="alert alert-info"
				style="margin-bottom: 18px; margin-top: 18px;">{lang('Payment
				methods list is empty','admin')}</div>
			{/if}
		</div>
	</section>
</div>