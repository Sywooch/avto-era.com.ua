				...
				<td colspan="6">
					<div class="inside_padd">
						<div class="form-horizontal">
							<form id="paymentmethodsUpdate" id="paymentmethodsUpdate"
								method="post"
								active="{$ADMIN_URL}paymentmethods/edit/{$model->getId()}/{$locale}">
								<div class="span9">
									<div class="control-group">
										<label class="control-label" for="inputRecCount">{lang('Name','admin')}:{$translatable}</label>
										<div class="controls">
										...
										<div class="controls">
											<select name="CurrencyId" style="width: 280px;">
												...
												<option value="{echo $c->getId()}" {if $c->getId()
													== $model->getCurrencyId()}selected="selected"{/if}> {echo
								...
								<div class="span9">
									<div class="control-group">
										<label class="control-label" for="inputRecCount">{lang('System','admin')}
											{lang('Payments','admin')}:</label>
										<div class="controls">
											<select name="PaymentSystemName"
												onchange="loadPaymentSystemConfigForm(this.value, {echo $model->getId()});">
												<option value="0">{lang('No','admin')}</option> {foreach
												ShopCore::app()->SPaymentSystems->getList() as $key=>$val}
												<option value="{$key}" {if $model->getPaymentSystemName()
													== $key}selected="selected"{/if}>{echo
													encode($val.listName)}</option> {/foreach}
											</select>
										</div>
									</div>
									<div id="paymentSystemConfigureBox">
										{$paymentSystemForm}</div>
								</div>
						</div>
					</div>
					</div>
