	...
	<div class="modal hide fade modal_del">
		...
		<div class="modal-footer">
			<a href="#" class="btn btn-primary"
				onclick="delete_function.deleteFunctionConfirm('{$ADMIN_URL}notificationstatuses/deleteAll')">{lang('Delete','admin')}</a>
			<a href="#" class="btn" onclick="$('.modal').modal('hide');">{lang('Cancel','admin')}</a>
		</div>
	</div>


	<div id="delete_dialog" title="{lang('Removing status','admin')}"
		style="display: none">{lang('Remove status?','admin')}</div>
	<!-- ---------------------------------------------------Блок видалення---------------------------------------------------- -->
	<div method="post" action="#">
		<section class="mini-layout">
			<div class="frame_title clearfix">
				<div class="pull-left">
					<span class="help-inline"></span> <span class="title">{lang('View
						the status of appearance','admin')}</span>
				</div>
				<div class="pull-right">
					<div class="d-i_b">
						<a class="btn btn-small btn-success pjax"
							href="/admin/components/run/shop/notificationstatuses/create"><i
			...
			<div class="row-fluid">
				...
				<table
					...
					<tbody class="sortable  save_positions"
						data-url="/admin/components/run/shop/notificationstatuses/savePositions">
						{foreach $model as $c}
						<tr>
							...
							<td class="share_alt"><a
								href="{$ADMIN_URL}notificationstatuses/edit/{echo $c->getId()}"
								class="title">{echo ShopCore::encode($c->getName())}</a></td>
