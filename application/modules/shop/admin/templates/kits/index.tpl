    ...
    <section class="mini-layout">
        <div class="frame_title clearfix">
            <div class="pull-left">
                ...
                <span class="title">{lang('Product kits review','admin')}</span>
            </div>
            <div class="pull-right">
                <div class="d-i_b">
                    <button type="button" class="btn btn-small btn-danger disabled action_on" onclick="delete_function.deleteFunction()" id="del_sel_brand"><i class="icon-trash icon-white"></i>{lang('Delete','admin')}</button>
                            ...
                            <tbody class="sortable">
                                {foreach $model as $item}
                                    <tr>
                                        ...
                                        <td><a class="pjax" href="{$ADMIN_URL}kits/kit_edit/{echo $item->getId()}" data-rel="tooltip" data-title="{lang('Editing','admin')}">{echo $item->getId()}</a></td>
                                        <td class="span2"><p>{echo $item->getSProducts()->getId()}</p></td>
                                        <td>
                                            <div class="share_alt">
                                                <a class="pjax" href="{$ADMIN_URL}products/edit/{echo ShopCore::encode($item->getSProducts()->getId())}" data-rel="tooltip" data-title="{lang('Editing','admin')}">{echo ShopCore::encode($item->getSProducts()->getName())}</a>
                                        ...
                                        <td>
                                            ...
                                            {if $attachedProducts->count() == 1}
                                                ...
                                                <a href="{$ADMIN_URL}products/edit/{echo $attachedSProduct->getId()}" data-rel="tooltip" data-title="{lang('Editing','admin')}">{truncate(ShopCore::encode($attachedSProduct->getName()),100)}</a>
                                            {else:}
