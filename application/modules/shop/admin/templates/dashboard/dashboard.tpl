                ...
                {if count($model)>0}
                    <table class="table table-striped table-bordered table-hover table-condensed">
                        ...
                        <tbody>
                            ...
                            {foreach $model as $o}
                                <tr>
                                    <td><a href="{$ADMIN_URL}orders/edit/{echo $o->getId()}" data-title="{lang('Editing','admin')}" data-rel="tooltip">{echo $o->getId()} </a></td>
                                    <td><span class="badge">
                        ...
                        {if count($bestSellers)>0}
                            {foreach $bestSellers as $bestSeller}
                                <tr>
                                    ...
                                    <td><a href="{site_url("/shop/product") . '/' . $bestSeller->getId()}" target="_blank">{echo $bestSeller->getName()}</a></td>
                    ...
                    <tbody>
                        {foreach $newUsers as $newUser}
                            <tr>
                                ...
                                <td><a href="{$ADMIN_URL}users/edit/{echo $newUser->getId()}" data-title="{lang('Editing by','admin')}" data-rel="tooltip">{echo $newUser->getFullName()}</a></td>
            ...
            <table class="table table-striped table-bordered table-hover table-condensed">
                ...
                <tbody>
                    {foreach $lastCommentsArray as $comment}
                        <tr>
                            <td><a href="/admin/components/cp/comments/edit/{echo $comment.id}" data-title="{lang('Editing by','admin')}" data-rel="tooltip" class="pjax">{$comment.id}</a></td>
                            <td>
                                {if 0 == $comment.status}
                            ...
                            <td><a href="{$ADMIN_URL}users/edit/{$comment.user_id}" data-title="{lang('Editing by','admin')}" data-rel="tooltip">{$comment.user_name}</a></td>
                        </tr>
                    {/foreach}    
                </tbody>
            </table>
