<div class="container">
	{$ci = get_instance()} {$ci->load->model('dx_auth/users', 'users')}
	<form>
		<section class="mini-layout">
			<div class="frame_title clearfix">
				<div class="pull-left">
					<span class="help-inline"></span> <span class="title">{lang('View','admin')}
						{lang(' callbacks','admin')}</span>
				</div>
			...
			<div class="clearfix">
				<div class="btn-group myTab m-t_20 pull-left"
					...
					{foreach $callbackStatuses as $s} <a
						href="#callbacks_{echo $s->getId()}" class="btn btn-small">{echo
						$s->getText()}</a> {/foreach}
				</div>
			</div>
			{if sizeof($model)}
			<div class="tab-content">
					...
					<table
						...
						<thead>
							<tr>
								...
								<th title="{lang('Sort by name .','admin')}">{lang('User
									name','admin')}</th>
						...
						<tbody>
							{foreach $model as $c}
							<tr data-status="{echo $c->getStatusId()}"
								data-id="{echo $c->getId()}" class="simple_tr">
								...
								<td>{echo $c->getId()}</td>
								<td><a class="pjax"
									href="{$ADMIN_URL}callbacks/update/{echo $c->getId()}"
									data-rel="tooltip" data-title="{lang('Edit callback','admin')}">{echo
								...
								<td>
									<div>
										<select name="theme"
											...
											<option value="{echo $callbackTheme->getId()}"
												{if $callbackTheme->getId() == $c->getThemeId()}
												selected="selected" {/if}>{echo $callbackTheme->getText()}</option>
											{/foreach}
										</select>
									</div>
								</td>
								<td>
									<div>
										<select name="status"
											...
											<option value="{echo $callbackStatus->getId()}"
												{if $callbackStatus->getId() == $c->getStatusId()}
												selected="selected" {/if}>{echo $callbackStatus->getText()}</option>
											{/foreach}
										</select>
									</div>
								</td>
				...
				<div class="row-fluid tab-pane  " id="callbacks_{echo $s->getId()}">
					<table
						...
						<tbody>
							{foreach $model as $c} {if $c->getStatusId() == $s->getId()}
							<tr data-status="{echo $c->getStatusId()}"
								id="callback_{echo $c->getId()}" class="simple_tr">
								<td class="t-a_c"><span class="frame_label"> <span
										class="niceCheck" style="background-position: -46px 0px;">
											<input type="checkbox" name="ids" value="{echo $c->getId()}">
									</span>
								...
								<td><a class="pjax"
									...
									data-rel="tooltip" data-title="{lang('Edit callback','admin')}">
										{echo ShopCore::encode($c->getName())}</a></td>
								<td>{echo encode($c->getPhone())}</td>
								...
								<td>{echo date('H:i:s Y-m-d', $c->getDate())}</td>
								<td><a href="#" class="btn btn-small my_btn_s"
									onclick="callbacks.deleteOne({echo $c->getId()})"><i
										class="icon-trash"></i></a></td>
							</tr>
							{/if} {/foreach}
				...
				<div style="padding: 10px 10px 0 20px;">
					<div id="totalCallbacks">
						<b>{if $_GET.status != -1}{lang('All Callbacks ','admin')}
							:{else:}{lang('Found results','admin')}:{/if}</b> {$totalCallbacks}
					</div>
				</div>
			</div>
			{else:}
...
<div class="modal hide fade">
	...
	<div class="modal-footer">
		...
		<a href="#" onclick="callbacks.deleteMany();"
			class="btn btn-primary formSubmit" data-form="#deleteOrderStatus">{lang('Remove','admin')}</a>
	</div>
</div>