...
<form method="post" action="{$ADMIN_URL}brands/create"
	style="width: 100%">

	<div class="form_text">{{lang('Name','admin')}:</div>
	<div class="form_input">
		<input type="text" name="Name" value="" class="textbox_long" /> <span
			class="required">*</span>
	</div>
	...
	<div class="footer_panel" align="right">
		<input type="submit" id="footerButton" name="_add"
			value="{lang('Save','admin')}" class="active"
			onClick="ajaxShopForm(this);" /> <input type="submit"
			id="footerButton" name="_create"
			value="{lang('Save and create a new record','admin')}"
			onClick="ajaxShopForm(this);" /> <input type="submit"
			id="footerButton" name="_edit"
			value="{lang(' Save and edit','admin')}"
			onClick="ajaxShopForm(this);" />
	</div>
