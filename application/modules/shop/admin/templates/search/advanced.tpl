	...
	<form action="{$ADMIN_URL}search/index/" id="filter_form" method="get">
		...
		<section class="mini-layout">
			<div class="frame_title clearfix">
				<div class="pull-left">
					<span class="help-inline"></span> <span class="title">{lang('Search
						results','admin')}: "{$_GET['q']}"</span>
				</div>
			</div>
			<div class="row-fluid">
				...
				<div class="clearfix">
					<div class="btn-group myTab m-t_20 pull-left"
						...
						<a href="#products"
							class="btn btn-small {if count($model)} active{/if}">{lang('Products','admin')}
							<span style="top: -13px;"
							class="badge {if count($model)}badge-important{/if}">{count($model)}</span>
						</a> <a href="#users"
							class="btn btn-small {if !count($model) && count($users)} active{/if}">{lang('User','admin')}
							<span style="top: -13px;"
							class="badge {if count($users)} badge-important{/if}">{count($users)}</span>
						</a>
					</div>
				</div>
				{/if}
				...
				<div class="tab-content">
					<div class="tab-pane {if count($model)}active{/if}" id="products">
						...
						<table
							...
							<tbody>
								{foreach $model as $p} {$variants = $p->getProductVariants()}
								{if sizeof($variants) == 1}
								<tr data-id="{echo $p->getId()}">

									<td><p>{echo $p->getId()}</p></td>
									<td class="share_alt"><a
										href="/shop/product/{echo $p->getId()}" target="_blank"
										...
										href="/admin/components/run/shop/products/edit/{echo $p->getId()}"
										class="title">{truncate(ShopCore::encode($p->getName()),100)}</a>
									</td>
									<td class="share_alt"><a
										...
										class="pjax">{echo $p->getMainCategory()->getName()}</a></td>
									<td>
									...
									<td><span class="pull-right"><span
											class="neigh_form_field help-inline"></span>&nbsp;&nbsp;{$CS}</span>
										<div class="p_r o_h frame_price number">
											<input type="text"
												value="{echo $variants[0]->getPriceInMain()}"
												class="js_price"
												data-value="{echo $variants[0]->getPriceInMain()}">
										...
										<table>
											...
											<tbody>
												<tr>
													...
													<td><a
														href="/shop/category/{echo $p->getMainCategory()->getUrl()}">{echo
																...
																<tr data-title="{lang('Drag product variant','admin')}">
																	...
																	<td><span class="simple_tree">&#8627;</span>&nbsp;&nbsp;
																		{if $v->getName() != ''} <span>{echo
																			$v->getName()}</span> {else:} <span>{echo
																			$p->getName()}</span> {/if}</td>
																	<td></td>
																	...
																	<td><span class="pull-right"><span
																		...
																		<div class="p_r o_h frame_price number">
																			<input type="text"
																				value="{echo $v->getPriceInMain()}" class="js_price"
																				data-value="{echo $v->getPriceInMain()}">
																			<button class="btn btn-small refresh_price"
						...
						{else:}
						<div class="alert alert-info" style="margin: 18px;">{lang('No
							relevant data has been found','admin')}</div>
						{/if}

					</div>

					<div
						class="tab-pane  {if !count($model) && count($users)} active{/if}"
						id="users">
						{if count($users)}
						...
						<table
							...
							<tbody>
								...
								<tr>
									...
									<td><a
										href="/admin/components/run/shop/users/edit/{echo $user.id}"
										class="pjax">{echo $user.username}</a></td>
									<td>{$user.email}</td>
