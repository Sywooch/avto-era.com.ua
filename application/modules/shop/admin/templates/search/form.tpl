<div class="saPageHeader" style="float:left;width:100%;position:relative;">
    ...
    <div id="shopTopSearchForm"> <!-- begin form block -->
        <form method="get" action="{$ADMIN_URL}search/index"  style="width:100%">
            ...
            <div class="form_input">
                <select name="CategoryId" style="width:285px;" onChange="shopLoadProperiesByCategoryInSearch(this);">
                    ...
                    {foreach $categories as $category}
                        {if isset($_GET['CategoryId']) && $_GET['CategoryId'] == $category->getId()}
            ...
            <div class="form_input">
                <input type="text" name="text" value="{echo encode($_GET['text'])}" class="textbox_long" />
            ...
            <div class="form_input">
                <select name="Active" style="margin-left: 100px">
                    <option value="">-</option>
                    <option {if $_GET['Active'] == 'true'} selected="selected" {/if} value="true">{lang('Active','admin')}</option>
                    <option {if $_GET['Active'] == 'false'} selected="selected" {/if} value="false">{lang('inactive','admin')}</option>
                </select>

                <select name="Hit">
                    <option value="">-</option>
                    <option {if $_GET['Hit'] == 'true'} selected="selected" {/if} value="true">{lang('Hit','admin')}</option>
                    <option {if $_GET['Hit'] == 'false'} selected="selected" {/if} value="false">{lang('No','admin')} {lang('Hit','admin')}</option>
                </select>

                <select name="Hot">
                    <option value="">-</option>
                    <option {if $_GET['Hot'] == 'true'} selected="selected" {/if} value="true">{lang('New','admin')}</option>
                    <option {if $_GET['Hot'] == 'false'} selected="selected" {/if} value="false">{lang('No','admin')} {lang('New','admin')}</option>
                </select>

                <select name="Action">
                    <option value="">-</option>
                    <option {if $_GET['Action'] == 'true'} selected="selected" {/if} value="true">{lang('Special offers','admin')}</option>
                    <option {if $_GET['Action'] == 'false'} selected="selected" {/if} value="false">{lang('No','admin')} {lang('Special offers','admin')}</option>
                </select>
            </div>
            <div class="form_overflow"></div>
