<div class="row-category" data-title="{lang('Drug category','admin')}">
	...
	<div class="share_alt" style="width: 42%;">
		...
		<div class="title lev">
			...
			{else:} <span class="simple_tree">↳</span> {/if} <a
				href="{$ADMIN_URL}categories/edit/{echo $cat->id}" class="pjax"
				data-rel="tooltip" data-placement="top"
				data-original-title="{lang('Edit the category','admin')}">{echo
				$cat->name}</a>
		</div>
	</div>
	<div>
		<p>
			<a href="{shop_url('category/'.$cat->url)}" target="blank"> {echo
				$cat->url} </a>
		</p>
	</div>
	<div style="width: 8%;">
	...
	<div>
		<div class="frame_prod-on_off" data-rel="tooltip" data-placement="top"
			...
			<span
				class="prod-on_off cat_change_active{if !$cat->active} disable_tovar{/if}"
