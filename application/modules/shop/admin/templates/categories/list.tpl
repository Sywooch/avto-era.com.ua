<section class="mini-layout">
	<div class="frame_title clearfix">
		<div class="pull-left">
			<span class="help-inline"></span> <span class="title">{lang('Product
				categories','admin')}</span>
		</div>
		<div class="pull-right">
			<div class="d-i_b">
				<a class="btn btn-small btn-success pjax"
					href="/admin/components/run/shop/categories/create"><i
					class="icon-plus-sign icon-white"></i>{lang('Create a
					category','admin')}</a>
				<button type="button" onclick="shopCategories.deleteCategories()"
	...
	<div
		class="frame_level table table-striped table-bordered table-hover table-condensed products_table">
		{if sizeof($tree)>0}
		<div id="category">
			...
			<div class="body_category">{$htmlTree}</div>
		</div>
		{else:} </br>
		<div class="alert alert-info">{lang('There are no categories at
			the site','admin')}</div>
		{/if}
	</div>
	...
	<div class="modal hide fade">
		...
		<div class="modal-body">
			...
			<p>{lang('Attention! will also remove all the products from this
				category','admin')}</p>
		</div>
		<div class="modal-footer">
			<a href="#" class="btn btn-primary pjax"
				onclick="shopCategories.deleteCategoriesConfirm()">{lang('Delete','admin')}</a>
