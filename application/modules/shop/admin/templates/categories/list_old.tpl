...
<div id="notice" class="alert alert-info">
	{lang('Category list is empty','admin')}. <a href="#"
		onclick="ajaxShop('categories/create'); return false;">{lang('Create','admin')}.</a>
</div>
...
<div id="sortable">
	<table id="ShopCatsHtmlTable">
		<thead>
			...
			<th>{lang('Position','admin')} <img
				src="{$THEME}images/save.png" align="absmiddle"
		...
		<tbody>
			{foreach $tree as $c} {setDefaultLanguage($c)}
			<tr {if !$c->
				getActive()}class="unactiveCategory"{/if}>
				<td>{echo $c->getId()}</td>
				<td
					onclick="javascript:ajaxShop('categories/edit/{echo $c->getId()}/{echo $c->getLocale()}');">
					{str_repeat('-',$c->getLevel())} {if $c->getLevel()==0} <b>{echo
						ShopCore::encode($c->getName())}</b> {else:} {echo
					ShopCore::encode($c->getName())} {/if}
				</td>
				<td><a href="{shop_url('category/' . $c->getFullPath())}"
					target="_blank">{echo $c->getFullPath()}</a></td>
				<td>{if $c->getActive()} {lang('Yes','admin')} {else:}
					{lang('No','admin')} {/if}</td>
				<td><input type="text" value="{echo $c->getPosition()}"
					style="width: 26px;" class="SCategoryPos"
					id="SCat{echo $c->getId()}" /></td>
				<td style="text-align: right;">{if count($languages) > 1} <img
					onclick="translate_list_item({echo $c->getId()}); return false;"
					src="/application/modules/shop/admin/templates/assets/images/translateable.png"
					width="16" height="16" /> {/if} <img
					onclick="confirm_shop_category({echo $c->getId()});"
					src="{$THEME}images/delete.png" style="cursor: pointer" width="16"
					height="16" title="{lang('Delete','admin')}" />
				</td>
			</tr>
			{/foreach}
		</tbody>
