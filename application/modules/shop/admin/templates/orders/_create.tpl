			...
			<div class="tab-content content_big_td">
				<div class="tab-pane active" id="main_data">
									...
									<div class="inside_padd span12">
										...
										<div class="control-group">
											<label class="control-label">
												{lang('Status','admin')}: </label>
											<div class="controls">
												{form_dropdown('Status',SOrders::getStatuses())}</div>
										</div>

										<div class="control-group" style="display: none;">
											<label class="control-label" for="Comment"> {echo
												$model->getLabel('StatusComment')}: </label>
											<div class="controls">
											...
											<div class="controls">
												<span class="frame_label no_connection"> <span
													class="niceCheck b_n"> <input type="checkbox"
														id="Notify" value="1" />
												</span> {lang('Notify a buyer about the status change','admin')}
												</span>
											</div>
										</div>
										<div class="control-group">
											...
											<div class="controls">
												<span class="frame_label no_connection"> <span
													class="niceCheck b_n"> <input type="checkbox"
														name="Paid" value="1" id="Paid" {if $model->getPaid()==1}
														checked="checked" {/if}/>
										...
										<div class="control-group">
											<label class="control-label" for="UserFullName">
												{lang('Complete','admin')} {lang('User name','admin')}: </label>
											<div class="controls">
										...
										-1)->asAdminHtml()} {if !empty($addField)}
					...
					<table
						...
						<thead>
							<tr>
								<th colspan="6">{lang('Delivery and payment','admin')}</th>
							</tr>
						</thead>
						<tbody>
							<tr>
									...
									<div class="inside_padd span12">
										...
										<div class="control-group">
											<label class="control-label" for="DeliveryMethod-select">
												{lang('Delivery Method','admin')}: </label>
											<div class="controls">
												<select name="DeliveryMethod" id="DeliveryMethod-select">
													<option value="0">- none -</option> {if $deliveryMethods}
													{foreach $deliveryMethods as $dm} {if $dm->getId() ==
													$model->getDeliveryMethod()}
													{$selected='selected="selected"'} {else:} {$selected=''}
													{/if} {if $dm->getFreeFrom() == 0 && $dm->getPrice() > 0}
													{//$free = $dm->getPrice()} {elseif($total >=
													$dm->getFreeFrom()):} {$freeDeliveryMethods[] =
													$dm->getId()} {elseif($dm->getFreeFrom() > 0 &&
													$dm->getPrice() > 0):} {//$free = $dm->getPrice()} {/if}

													{if $dm->getId() == $model->getDeliveryMethod()}
													{$deliveryPrice = $model->getDeliveryPrice()} {/if}

													<option {$selected} value="{echo $dm->getId()}"
														class="{counter('row_dark','row_lite')} hover">{echo
														ShopCore::encode($dm->getName())}</option> {/foreach} {/if}
												</select>
											</div>
										</div>

										{/if} {if sizeof($paymentMethods) > 0}
										<div class="control-group">
											<label class="control-label" for="PaymentMethod">
												{lang('Payment method','admin')}: </label>
											<div class="controls">
												<select name="PaymentMethod" id="PaymentMethod">
													<option value="0">- none -</option> {if $paymentMethods}
													{foreach $paymentMethods as $pm} {if $pm->getId() ==
													...
													<option {$selected} value="{echo $pm->getId()}"
														class="{counter('row_dark','row_lite')} hover">{echo
														ShopCore::encode($pm->getName())}</option> {/foreach} {/if}
												</select>
											</div>
										</div>
										{/if}
