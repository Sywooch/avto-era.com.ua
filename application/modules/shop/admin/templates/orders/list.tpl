    ...
    <form method="get" action="/admin/components/run/shop/orders/index" id="ordersListFilter" class="listFilterForm">
            ...
            <div class="frame_title clearfix">
                ...
                <div class="pull-right">
                    <div class="d-i_b">
                        ...
                        <a href="{$ADMIN_URL}orders/index" title="{lang('Reset filter','admin')}" type="button" class="btn btn-small pjax"><i class="icon-refresh"></i>{lang('Cancel filter','admin')}</a>
                        ...
                        <button onclick="orders.deleteOrders()" type="button" class="btn btn-small btn-danger disabled action_on"><i class="icon-trash icon-white"></i>{lang('Delete','admin')}</button>
                    ...
                    <thead>
                        <tr>
                            ...
                            <th class="span2" id="orderDateCreated">
                                ...
                                {if isset($_GET['orderMethod']) AND $_GET['orderMethod'] == 'DateCreated'}
                                    {if $_GET['orderCriteria'] == 'ASC'}
                                        &nbsp;&nbsp;&nbsp;<span class="f-s_14">↑</span>
                                    {else:}
                                        &nbsp;&nbsp;&nbsp;<span class="f-s_14">↓</span>
                            ...
                            <th class="span2" id="orderTotalPrice">
                                ...
                                {if isset($_GET['orderMethod']) AND $_GET['orderMethod'] == 'TotalPrice'}
                                    {if $_GET['orderCriteria'] == 'ASC'}
                                        &nbsp;&nbsp;&nbsp;<span class="f-s_14">↑</span>
                                    {else:}
                                        &nbsp;&nbsp;&nbsp;<span class="f-s_14">↓</span>
                            ...
                            <th class="span2" id="orderPaid">
                                <a href="#" class="order_list_order" data-method="Paid" data-criteria="{if $orderField == 'Paid'}{$nextOrderCriteria}{else:}ASC{/if}">{lang('Paid','admin')}</a>
                                {if isset($_GET['orderMethod']) AND $_GET['orderMethod'] == 'Paid'}
                                    ...
                                    {/if}
                                {/if}
                                <input type="hidden" name="orderMethod" value="{if isset($_GET['orderMethod'])}{echo $_GET['orderMethod']}{else:}Id{/if}"/>
                                <!--<input type="hidden" name="orderCriteria" value="{if $_GET['orderCriteria'] != ''}{$_GET['orderCriteria']}{else:}DESC{/if}"/>-->
                                <input type="hidden" name="orderCriteria" value="{echo $_GET['orderCriteria']}"/>
                            </th>
                        </tr>
                        <tr class="head_body">
                            <td>
                            ...
                            <td>
                                <select name="status_id">
                                    <option {if !$_GET['status_id']} selected="selected" {/if} >-- none --</option>
                            ...
                            <td>
                                <label>
                                    <span class="pull-left"><span class="neigh_form_field help-inline"></span>{lang('from','admin')}&nbsp;&nbsp;</span>
                                    <span class="o_h d_b p_r">
                                        <input type="text" data-placement="top" data-original-title="{lang('choose a date','admin')}" data-rel="tooltip" class="datepicker "  name="created_from" value="{$_GET['created_from']}" >
                                        <i class="icon-calendar"></i>
                                    </span>
                                </label>
                                <label>
                                    <span class="pull-left"><span class="neigh_form_field help-inline"></span>{lang('to','admin')}&nbsp;&nbsp;</span>
                                    <span class="o_h d_b p_r">
                                        <input type="text" data-placement="top" data-original-title="{lang('choose a date','admin')}" data-rel="tooltip" class="datepicker "  name="created_to" value="{$_GET['created_to']}">
                                        <i class="icon-calendar"></i>
                                    </span>
                                </label>
                            </td>
                            ...
                            <td class="number">
                                <label>
                                    <span class="pull-left"><span class="neigh_form_field help-inline"></span>{lang('from','admin')}&nbsp;&nbsp;</span>
                                    <span class="o_h d_b"><input value="{echo $_GET['amount_from']}" type="text" data-placement="top" data-original-title="{lang('numbers only','admin')}" name="amount_from"></span>
                                </label>
                                <label>
                                    <span class="pull-left"><span class="neigh_form_field help-inline"></span>{lang('to','admin')}&nbsp;&nbsp;</span>
                                    <span class="o_h d_b"><input value="{echo $_GET['amount_to']}" type="text" data-placement="top" data-original-title="{lang('numbers only','admin')}" name="amount_to"></span>
                                </label>
                            </td>
                            <td>
                                <select name="paid">
                                    <option {if !$_GET['paid']} selected="selected" {/if} value="">-- none --</option>
                                    <option {if "0" === $_GET['paid']} selected="selected" {/if} value="0">{echo lang('No','admin')}</option>
                                    <option {if "1" == $_GET['paid']} selected="selected" {/if} value="1">{echo lang('Yes','admin')}</option>
                                </select>
                            </td>
                        </tr>
                    </thead>
                    <tbody >
                        ...
                        {foreach $model as $o}
                            <tr data-original-title="" class="simple_tr">
                                ...
                                <td><span class="badge {if $o->getStatus() == 2} label-success {/if}">
                                        {foreach $orderStatuses as $orderStatus}
                                ...
                                <td>
                                    {if $o->getUserId()}<a class="pjax" href="{$ADMIN_URL}users/edit/{echo $o->getUserId()}" data-title="{lang('Editing by','admin')}" data-rel="tooltip">{echo ShopCore::encode($o->getUserFullName())}</a>
                            ...
                            <td>
                                ...
                                {echo $o->getTotalPrice()}

                            </td>
                            <td> {$count_orders++}
                                {if $o->getPaid() == true}
                                    <span class="label label-success">{lang('Yes','admin')}</span>{$total_paid++}
                                {else:}
                                    <span class="label">{lang('No','admin')}</span>
                                {/if}
                            </td>
                        </tr>
                    {/foreach}
        ...
        <div class="clearfix">
            ...
            <div class="pagination pull-right" style="margin-right: 25px;">
                ...
                <select title="{lang('Orders on the page','admin')}" onchange="change_status('{$ADMIN_URL}orders/paginationVariant/' + this.value + '/1');">
