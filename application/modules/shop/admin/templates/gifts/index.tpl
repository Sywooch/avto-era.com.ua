...
<div class="modal hide fade modal_del">
	...
	<div class="modal-footer">
		<a href="#" class="btn btn-primary"
			onclick="delete_function.deleteFunctionConfirm('{$ADMIN_URL}gifts/delete')">{lang('Delete','admin')}</a>
...
<div id="delete_dialog"
	title="{lang('Removing Gift Certificate','admin')}"
	style="display: none">{lang('Remove a gift
	certificate?','admin')}</div>
<!-- ---------------------------------------------------Блок видалення---------------------------------------------------- -->

<section class="mini-layout">
	<div class="frame_title clearfix">
		<div class="pull-left">
			<span class="help-inline"></span> <span class="title">{lang('Gift
				certificate list','admin')}</span>
		</div>
		<div class="pull-right">
			<div class="d-i_b">
				<a class="btn btn-small btn-success pjax"
					href="/admin/components/run/shop/gifts/create"><i
					class="icon-plus-sign icon-white"></i>{lang('Create gift
					certificate','admin')}</a> <a class="btn btn-small pjax"
					href="/admin/components/run/shop/gifts/settings">{lang('Settings','admin')}</a>
				<button type="button"
					class="btn btn-small btn-danger disabled action_on"
					onclick="delete_function.deleteFunction()" id="del_sel_cert">
		...
		<table
			...
			<tbody class="sortable">
				...
				<tr data-id="{echo $c->getId()}">
					...
					<td>{echo $c->getId()}</td>
					<td><a class="pjax"
						href="{$ADMIN_URL}gifts/edit/{echo $c->getId()}">{echo
							$c->getKey()}</td>
					...
					<td>
						<div class="frame_prod-on_off" data-rel="tooltip"
							...
							<span
								class="prod-on_off ch_active {if date('U')>$c->getEspDate() || $c->getActive() == 0}disable_tovar{/if}"
								data-cid="{echo $c->getId()}"></span>
						</div>
					</td>
				</tr>
				{/foreach}
	...
	{else:}
	<div class="alert alert-info m-t_20">{lang('Gift certificate list
		is empty.','admin')}</div>
	{/if}
</section>