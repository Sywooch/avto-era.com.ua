<div class="container">
	...
	<div class="modal hide fade modal_del">
		...
		<div class="modal-footer">
			<a href="#" class="btn btn-primary"
				onclick="delete_function.deleteFunctionConfirm('{$ADMIN_URL}deliverymethods/deleteAll')">{lang('Delete','admin')}</a>
	...
	<div id="delete_dialog"
		title="{lang('Removing the shipping methods','admin')}"
	...
	<section class="mini-layout">
		...
		<div class="frame_title clearfix">
			...
			<div class="pull-right">
				<div class="d-i_b">
					<a href="/admin/components/run/shop/deliverymethods/create"
						class="btn btn-small btn-success pjax"><i
						class="icon-plus-sign icon-white"></i>{lang('Create a way to
						deliver','admin')}</a>
					<button type="button"
				...
				<table
					...
					<thead>
						<tr>
							...
							<th class="span4">{lang('Price','admin')}</th>
							<th class="span2">{lang('Free','admin')}
								{lang('from','admin')}</th>
							<th class="span2 t-a_c">{lang('Active','admin')}</th>
						</tr>
					</thead>
					<tbody class="sortable save_positions"
						data-url="/admin/components/run/shop/deliverymethods/save_positions">
						...
						<tr>
							...
							<td><a class="pjax"
								href="{$ADMIN_URL}deliverymethods/edit/{echo $c->getId()}"
								data-rel="tooltip" data-title="{lang('Editing','admin')}">{echo
									ShopCore::encode($c->getName())}</a></td>
							<td><p>{echo $c->getPrice()} {if
									$c->getIsPriceInPercent()}%{else:}{$CS}{/if}</p></td>
							<td><p>{echo $c->getFreeFrom()} {$CS}</p></td>
							<td class="t-a_c">
								<div class="frame_prod-on_off" data-rel="tooltip"
									...
									<span
										class="prod-on_off {if $c->getEnabled() == NULL}disable_tovar{/if}"
										style=""> </span>
								</div>
								</div>
							</td>
		...
		{else:} </br>
		<div class="alert alert-info">{lang('Delivery methods list is
			empty','admin')}</div>
		{/if}
	</section>
</div>
