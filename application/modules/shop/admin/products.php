...
class ShopAdminProducts extends ShopAdminController {
        ...
        if (!empty($_POST)) {
            ...
            if ($validation->run($this) == FALSE) {
                ...
                if ($_POST['Url'] == null && $_POST['Name'] != null) {
                    $urlName = SProductsQuery::create()
                            ->where('SProducts.Url = ?', (string) $_POST['Name'])
                            ->findOne();
                ...
                \CMSFactory\Events::create()->registerEvent(array('productId' => $model->getId(), 'userId' => $this->dx_auth->get_user_id()));
                \CMSFactory\Events::runFactory();
    ...
    public function edit($productId, $locale = null) {
        ...
        \CMSFactory\Events::create()->registerEvent(array('model' => $model, 'userId' => $this->dx_auth->get_user_id()), 'ShopAdminProducts:preEdit');
        \CMSFactory\Events::runFactory();
            ...
            if ($validation->run($this) == FALSE) {
                ...
                foreach ($_POST['variants']['MainImageForDel'] as $key => $value) {
                    if ($value) {
                        ...
                        $this->db->where('id', $key)->update('shop_product_variants', array('mainImage' => ''));
                        //Delete files
...
//++++++++++++++++++++++++++++++++
                if (isset($variant['CurrentId']) && $variant['CurrentId'] > 0)
                    $productVariant_loc = SProductVariantsQuery::create()->joinWithI18n($locale)
...
//+++++++++++++++++++++++
    ...
    public function ajaxUpdatePrice($productId = null) {
        if ($productId !== null) {
            ...
            \CMSFactory\Events::create()->registerEvent(array(
                'productId' => $productId,
                'userId' => $this->dx_auth->get_user_id()), 'ShopAdminProducts:edit');
            \CMSFactory\Events::runFactory();
    ...
    public function ajaxDeleteProducts() {
        if (sizeof($_POST['ids'] > 0)) {
            //Start. Kits delete
            $kits = $this->db
                    ->join('shop_kit_product', 'shop_kit_product.kit_id=shop_kit.id')
                    ->where_in('shop_kit_product.product_id', $_POST['ids'])
                    ->or_where_in('shop_kit.product_id', $_POST['ids'])
                    ->group_by('id')
                    ->get('shop_kit')
                    ->result();
            ...
            /** Init Event. Create Shop Product */
            \CMSFactory\Events::create()->registerEvent(array('model' => $modelProduct, 'userId' => $this->dx_auth->get_user_id()), 'ShopAdminProducts:delete');
            \CMSFactory\Events::runFactory();
        }
    }
