...
class ShopAdminComulativ extends ShopAdminController {
    ...
    public function create() {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run($this) == FALSE) {
                ...
                $info = $this->db->get_where('shop_comulativ_discount', array('description' => $data['description']));
                $row = $info->row();
    ...
    public function edit($id = null) {
        $model = $this->db->get_where($this->dbName, array('id' => $id))->result_array();
    ...
    public function deleteAll() {
        ...
        if (sizeof($_POST['ids'] > 0)) {
            $model = ShopComulativQuery::create()
                    ->findPks($_POST['ids']);
