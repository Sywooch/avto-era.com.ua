...
//        $this->pagination->initialize($config);
    ...
    public function getAvailableNotification() {
        $totalNewNotifications = $this->_count(SNotificationsQuery::create()
                        ->joinSProductVariants(null, 'left join')
                        ->filterByStatus(1)
                        ->where('SNotifications.ActiveTo > ?', strtotime('today'))
                        ->filterByNotifiedByEmail(FALSE)
                        ->where('SProductVariants.Stock > ?', 0)
        );

        $totalNewOrders = SOrdersQuery::create()
                ->filterByStatus(1)
                ->count();
        ...
        $toralProductsWithoutImage = count($this->db
...
//                        ->or_where('not mainImage')
        ...
        if (!$this->avalaibleComments)
            $totalNewComments = count($this->db->where('status', 1)
                            ->get('comments')
                            ->result_array()
            );
