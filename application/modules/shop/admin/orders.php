...
class ShopAdminOrders extends ShopAdminController {
    ...
    public function index() {
        ...
        if ($_GET['product_id'])
            $model = $model->leftJoinSOrderProducts()
                    ->where('SOrderProducts.ProductId = ?', $_GET['product_id']);
...
//            $cvcv = array('SOrders.UserFullName LIKE ?' => '%' . $_GET['customer'] . '%', 'SOrders.UserEmail LIKE ?' => '%' . $_GET['customer'] . '%', 'SOrders.UserPhone LIKE ?'=> '%'.$_GET['customer'].'%');
        ...
        if ($_GET['paid'] != '-- none --')
            if ($_GET['paid'] === "0")
                $model = $model->where('SOrders.Paid IS NULL')
                        ->orWhere('SOrders.Paid = 0');
            else if ($_GET['paid'] === "1")
        ...
        if ($_POST) {
            ...
            if ($validation->run()) {
                ...
                $deliveryMethod = SDeliveryMethodsQuery::create()
                        ->findPk((int) $_POST[shop_orders]['delivery_method']);
                ...
                $paymentMethod = SPaymentMethodsQuery::create()
                        ->findPk((int) $_POST[shop_orders]['payment_method']);
...
//                $products = SProductVariantsQuery::create()->filterById($number->getProductId())->find();
    ...
    public function ajaxGetProductList($type = NULL) {
        ...
        if (!empty($_GET['term'])) {
            ...
            if ($type != 'number') {
                $products = $products
                        ->filterById($_GET['term'])
                        ->_or()
                        ->useI18nQuery(MY_Controller::getCurrentLocale())
                        ->filterByName('%' . $_GET['term'] . '%')
                        ->endUse()
                        ->_or()
                        ->useProductVariantQuery()
                        ->filterByNumber('%' . $_GET['term'] . '%')
                        ->endUse();
            } else {
                $products = $products
                        ->useProductVariantQuery()
...
//       var_dumps($new_quan);
    ...
    public function ajaxEditOrderCart($Id) {
        ...
        $product = SProductsQuery::create()
                        ->filterById((int) $_POST['newProductId'])->findOne();
        $variant = SProductVariantsQuery::create()
                ->filterByProductId((int) $_POST['newProductId'])
                ->filterById((int) $_POST['newVariantId'])
                ->findOne();
        if ($order === null) {
            return;
        ...
        if ($_POST) {
            ...
            if ($shopOrder['user_id'] != null && $shopOrder['delivery_method'] != 0 && $shopOrder['payment_method'] != 0) {
                ...
                \CMSFactory\Events::create()->registerEvent(array(
                    'order_products' => $orderProducts), 'ShopAdminOrder:create');
    ...
    private function makeNotActiveGiftCert($key) {
        ...
        $this->db->where('key', $key)->update('shop_gifts', array('active' => 0));
    }
    ...
    public function ajaxGetProductVariants() {
        ...
        $productVariants = $this->db->select('shop_product_variants.id, shop_product_variants_i18n.name, shop_product_variants.price,shop_currencies.symbol, shop_product_variants.stock')
                ->from('shop_products')
    ...
    public function getImageName() {
        ...
        $query = $this->db->select('mainImage')
                ->where('id', $variantId)
