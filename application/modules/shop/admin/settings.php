...
//          $data['adminMessages'] = serialize($_POST['messages']);
    ...
    public function getAllProductsVariantsIds() {
        $ids = null;
        $array = $this->db->select('id,mainImage')
    ...
    public function setSorting() {
        ...
        if ($this->db->where('locale', $locale)->where('id', $id)->get('shop_sorting_i18n')->num_rows())
            $this->db->where('locale', $locale)->where('id', $id)
                    ->update('shop_sorting_i18n', array('name' => $name, 'name_front' => $name_front, 'tooltip' => $tooltip));
        else
            $this->db
                    ->insert('shop_sorting_i18n', array('locale' => $locale, 'id' => $id, 'name' => $name, 'name_front' => $name_front, 'tooltip' => $tooltip));
    }
