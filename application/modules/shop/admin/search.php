...
//            $model = $model->where('');
        ...
        if (isset($_GET['CategoryId']) && $_GET['CategoryId'] > 0) {
            $category = SCategoryQuery::create()
                    ->filterById((int) $_GET['CategoryId'])
                    ->findOne();
            if ($category)
        ...
        if (isset($_GET['orderMethod']) && $_GET['orderMethod'] != '') {
            $order_methods = array('Id', 'Name', 'CategoryId', 'Price', 'Active', 'Reference');
    ...
    private function _advanced_search_by_term($term, $fullData = false) {
        ...
        $model = $model->useI18nQuery($this->defaultLanguage['identif'])
                ->where('SProductsI18n.Name LIKE ?', '%' . $term . '%')
                ->endUse()
                ->orWhere('ProductVariant.Number = ?', '%' . $term . '%')
                ->distinct()
                ->find();
        ...
        if (is_array($term)) {
            $users = $this->db->select($select)
                    ->where("username like '%$term[0]%'")
                    ->or_where("phone LIKE '%$term[1]%'")
                    ->or_where("email LIKE '%$term[2]%'")
                    ->get('users')
                    ->result_array();
        } else {
            $users = $this->db->select($select)
