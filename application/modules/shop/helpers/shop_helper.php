...
if (! function_exists ( 'promoLabel' )) {
	function promoLabel($action, $hot, $hit, $disc) {
		...
		if ($disc && ( float ) $disc > 0)
			$out .= '<span class="product-status discount"><span class="text-el"><span>-&nbsp;</span>' . round ( $disc, 0 ) . '%</span></span>';
		return $out;
	}
}
...
function getBrand($cid, $nin = false) {
	if (! $nin)
...
if (! function_exists ( 'getPromoBlock' )) {
	function getPromoBlock($type = 'action', $limit = 10, $idCategory = NULL, $idBrand = NULL) {
		$model = SProductsQuery::create ()->joinWithI18n ( ShopController::getCurrentLocale () )->orderByCreated ( 'DESC' );
		if ($idCategory) {
	...
	function productInCart(&$haystack, $prodId, $varProdId, $stock = 1) {
		$response = array (
				array (
						...
						'class' => 'button_gs',
						'identif' => 'goBuy',
						'link' => shop_url ( 'product/' . $prodId ) 
				),
				array (
						'message' => lang ( 'Formulation <br/> order' ),
						'class' => 'button_middle_blue',
						'identif' => 'goToCart',
						'link' => '/shop/cart' 
				),
				array (
						'message' => 'Сообщить <br />о появлении',
						'class' => 'button_greys',
						'identif' => 'goNotifMe',
						'link' => shop_url ( 'product/' . $prodId ) 
				) 
...
if (! function_exists ( 'productInCartI' )) {
	function productInCartI(&$haystack, $prodId, $varProdId, $stock = 1) {
		$response = array (
				array (
						'message' => lang ( 'Buy' ),
						'class' => 'buttons button_big_green',
						'identif' => 'goBuy',
						'link' => '#' 
				),
				array (
						'message' => lang ( 'Formulation order' ),
						'class' => 'buttons button_big_blue',
						'identif' => 'goToCart',
						'link' => '/shop/cart' 
				),
				array (
						'message' => lang ( 'Announce a' ),
						'class' => 'buttons button_big_greys',
						'identif' => 'goNotifMe',
						'link' => shop_url ( 'product/' . $prodId ) 
				) 
		);
...
if (! function_exists ( 'getLimiPrice' )) {
	function getLimiPrice($cat_id, $way = 'min') {
		...
		$child_cats = SCategoryQuery::create ()->where ( 'SCategory.ParentId = ?', $cat_id )->select ( array (
				'Id' 
		) )->find ()->toArray ();
		...
		if ($way == 'min') {
			$min_price = SProductsQuery::create ()->leftJoin ( 'ProductVariant' )->filterByCategoryId ( $child_cats )->filterByActive ( true )->select ( array (
					'ProductVariant.PriceInMain' 
			) )->limit ( 1 )->orderBy ( 'ProductVariant.PriceInMain', Criteria::ASC )->find ()->toArray ();
			...
			) )->limit ( 1 )->orderBy ( 'ProductVariant.Price', Criteria::DESC )->find ()->toArray ();
			if (count ( $add_filter ) > 0)
...
if (! function_exists ( 'getLimitAllPrice' )) {
	function getLimitAllPrice($way = 'min') {
		if ($way == 'min') {
			$min_price = SProductsQuery::create ()->leftJoin ( 'ProductVariant' )->filterByActive ( true )->select ( array (
					'ProductVariant.Price' 
			) )->limit ( 1 )->orderBy ( 'ProductVariant.Price', Criteria::ASC )->find ()->toArray ();
		} else {
			$min_price = SProductsQuery::create ()->leftJoin ( 'ProductVariant' )->filterByActive ( true )->select ( array (
					'ProductVariant.Price' 
			) )->limit ( 1 )->orderBy ( 'ProductVariant.Price', Criteria::DESC )->find ()->toArray ();
...
function getProduct($id) {
	return SProductsQuery::create ()->joinWithI18n ( MY_Controller::getCurrentLocale () )->findPk ( $id );
}
...
if (! function_exists ( 'getSimilarProduct' )) {
	function getSimilarProduct($model, $limit = 8) {
		...
		$product = SProductsQuery::create ()->filterById ( $model->getId (), Criteria::ALT_NOT_EQUAL )->joinWithI18n ( $currentLocale )->filterByCategoryId ( $model->getCategoryId () )->filterByActive ( true )->useProductVariantQuery ()->filterByPrice ( array (
				'min' => $min,
				'max' => $max 
		) )->endUse ()->distinct ()->limit ( $limit )->find ();
