...
{
  if(!$nin) $bid = SProductsQuery::create()->select(array('BrandId'))->filterByCategoryId($cid)->find()->toArray();
  else $bid = SProductsQuery::create()->select(array('BrandId'))->where('SProducts.CategoryId NOT IN ('. $cid .')')->find()->toArray();

   if(count($bid) > 0) $brands = SBrandsQuery::create()->filterById($bid)->orderByUrl()->find();
   else return false;
   return $brands;
}
