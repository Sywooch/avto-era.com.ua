...
class Yandex extends ShopController {
	...
	public function allCatId($arg) {
		$query = $this->db->get_where ( 'shop_product_categories', array (
				'product_id' => $arg 
		) );
		$row = $query->row ();
	...
	public function renderCategories() {
		$categories = SCategoryQuery::create ()->filterById ( ShopCore::app ()->SSettings->getSelectedCats () )->find ();
	...
	public function getProducts() {
		$productsIds = ShopProductCategoriesQuery::create ()->select ( 'ProductId' )->distinct ()->filterByCategoryId ( ShopCore::app ()->SSettings->getSelectedCats () )->find ()->toArray ();
		$products = SProductsQuery::create ()->distinct ()->filterById ( $productsIds )->leftJoin ( 'ProductVariant' )->useProductVariantQuery ()->filterByStock ( array (
				'min' => 1 
		) )->endUse ()->filterByActive ( true )->find ();
