...
class Front_api extends ShopController {
	...
	public function getVysota() {
		...
		$shirina = SProductPropertiesDataQuery::create ()->filterByProductId ( $pids )->filterByPropertyId ( 42 )->filterByValue ( $shirina )->distinct ()->select ( array (
				'ProductId' 
		) )->find ()->toArray ();
		
		$vysota = SProductPropertiesDataQuery::create ()->filterByProductId ( $shirina )->filterByPropertyId ( 43 )->distinct ()->select ( array (
				'Value' 
		) )->find ()->toArray ();
	...
	public function getDiametrShyn() {
		...
		$vysota = SProductPropertiesDataQuery::create ()->filterByProductId ( $shirina )->filterByPropertyId ( 43 )->filterByValue ( $vysota )->distinct ()->select ( array (
				'ProductId' 
		) )->find ()->toArray ();
		
		$diametr = SProductPropertiesDataQuery::create ()->filterByProductId ( $vysota )->filterByPropertyId ( 44 )->distinct ()->select ( array (
				'Value' 
		) )->find ()->toArray ();
	...
	public function getFiltrShyn() {
		...
		if (! empty ( $bid ))
			$where .= " AND shop_products.brand_id =   '$bid' ";
		if (! empty ( $cid ))
			$where .= " AND (shop_products.category_id =  '$cid' OR shop_products.category_id IN (SELECT id FROM shop_category WHERE parent_id = '$cid')) ";
		if (! empty ( $shirina ))
			$where .= " AND shop_products.id IN (SELECT product_id FROM shop_product_properties_data WHERE property_id = 42 AND value = '$shirina') ";
		if (! empty ( $vysota ))
			$where .= " AND shop_products.id IN (SELECT product_id FROM shop_product_properties_data WHERE property_id = 43 AND value = '$vysota') ";
		if (! empty ( $diametr ))
			$where .= " AND shop_products.id IN (SELECT product_id FROM shop_product_properties_data WHERE property_id = 44 AND value = '$diametr') ";
		
		$ids = $ci->db->query ( 'SELECT id FROM shop_products ' . $where )->result_array ();
		var_dump ( $ids, 'SELECT id FROM shop_products ' . $where );
		exit ();
	...
	public function getDiametr() {
		...
		$diamers = SProductPropertiesDataQuery::create ()->filterByProductId ( $pids )->filterByPropertyId ( 59 )->distinct ()->select ( array (
				'Value' 
		) )->find ()->toArray ();
	...
	public function getDiscType() {
		...
		$diamers = SProductPropertiesDataQuery::create ()->filterByProductId ( $pids )->filterByPropertyId ( 59 )->filterByValue ( $diam )->distinct ()->select ( array (
				'ProductId' 
		) )->find ()->toArray ();
		
		$type = SProductPropertiesDataQuery::create ()->filterByProductId ( $diamers )->filterByPropertyId ( 65 )->distinct ()->select ( array (
				'Value' 
		) )->find ()->toArray ();
	...
	public function getPCD() {
		...
		$type = SProductPropertiesDataQuery::create ()->filterByProductId ( $diamers )->filterByPropertyId ( 65 )->filterByValue ( $type )->distinct ()->select ( array (
				'ProductId' 
		) )->find ()->toArray ();
		
		$pcd = SProductPropertiesDataQuery::create ()->filterByProductId ( $diamers )->filterByPropertyId ( 60 )->distinct ()->select ( array (
				'Value' 
		) )->find ()->toArray ();
	...
	public function getET() {
		...
		$pcd = SProductPropertiesDataQuery::create ()->filterByProductId ( $type )->filterByPropertyId ( 60 )->filterByValue ( $pcd )->distinct ()->select ( array (
				'ProductId' 
		) )->find ()->toArray ();
		
		$et = SProductPropertiesDataQuery::create ()->filterByProductId ( $diamers )->filterByPropertyId ( 62 )->distinct ()->select ( array (
				'Value' 
		) )->find ()->toArray ();
	...
	public function getStupica() {
		...
		$et = SProductPropertiesDataQuery::create ()->filterByProductId ( $pcd )->filterByPropertyId ( 62 )->filterByValue ( $et )->distinct ()->select ( array (
				'ProductId' 
		) )->find ()->toArray ();
		
		$stupica = SProductPropertiesDataQuery::create ()->filterByProductId ( $et )->filterByPropertyId ( 63 )->distinct ()->select ( array (
				'Value' 
		) )->find ()->toArray ();
