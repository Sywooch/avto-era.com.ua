...
class SProducts extends BaseSProducts {
	...
	public function preDelete() {
		...
		$productVariants = SProductVariantsQuery::create ()->filterByProductId ( $this->getId () )->find ();
	...
	public function getKitProductVariants(ShopKitProduct $shopKitProduct, $criteria = null, PropelPDO $con = null) {
		$criteria = SProductVariantsQuery::create ( null, $criteria )->joinWithI18n ( MY_Controller::getCurrentLocale (), Criteria::LEFT_JOIN );
	...
	public function getRelatedProductsModels($limit = 5) {
		...
		if (is_array ( $ids ) && count ( $ids ) > 0) {
			$models = SProductsQuery::create ()->joinWithI18n ( MY_Controller::getCurrentLocale () )->orderByCreated ( Criteria::DESC )->findPks ( $ids );
	...
	public function getSampleHitsModels($limit = 5) {
		$models = SProductsQuery::create ()->joinWithI18n ( MY_Controller::getCurrentLocale () )->orderByCreated ( Criteria::DESC )->where ( 'SProducts.Id NOT IN ?', $this->getId () )->filterByHit ( 1 )->filterByCategoryId ( $this->getCategoryId () )->limit ( $limit )->find ();
	...
	public function getSampleNewestModels($limit = 6) {
		$models = SProductsQuery::create ()->joinWithI18n ( MY_Controller::getCurrentLocale () )->orderByCreated ( Criteria::DESC )->where ( 'SProducts.Id NOT IN ?', $this->getId () )->filterByCategoryId ( $this->getCategoryId () )->filterByHot ( 1 )->limit ( $limit )->find ();
	...
	public function getSameBrandCategoryProductsModels($limit = 6) {
		$models = SProductsQuery::create ()->joinWithI18n ( MY_Controller::getCurrentLocale () )->orderByCreated ( Criteria::DESC )->where ( 'SProducts.Id NOT IN ?', $this->getId () )->filterByCategoryId ( $this->getCategoryId () )->filterByBrandId ( $this->getBrandId () )->limit ( $limit )->find ();
	...
	public function getSimilarPriceProductsModels($limit = 6, $price_percent = 20) {
		...
		$models = SProductsQuery::create ()->joinWithI18n ( MY_Controller::getCurrentLocale () )->leftJoin ( 'ProductVariant' )->distinct ( 'ProductId' )->filterByCategoryId ( $this->getCategoryId () )->where ( 'SProducts.Id NOT IN ?', $this->getId () )->where ( 'ProductVariant.Price >= ?', $low_similar )->where ( 'ProductVariant.Price <= ?', $high_similar )->orderByCreated ( Criteria::DESC )->limit ( $limit )->find ();
	...
	public function getShopKits($criteria = null, PropelPDO $con = null) {
		if (null === $this->collShopKits || null !== $criteria) {
			if ($this->isNew () && null === $this->collShopKits) {
				...
				$collShopKits = ShopKitQuery::create ( null, $criteria )->filterBySProducts ( $this )->filterByActive ( TRUE )->orderByPosition ( Criteria::ASC )->find ( $con );
				if (null !== $criteria) {
					return $collShopKits;
				}
	 ...
	 * 
	 * @param string $type
	 *        	Type of property (main, showInCompare, showInFilter, showOnProductPage)
	 * @return array
	 */
	public function getProductProperties($type = null) {
		 ...
		 */
		$properties = SPropertiesQuery::create ()->filterByActive ( 1 );
		...
		$properties = $properties->joinSProductPropertiesData ()->joinWithI18n ( MY_Controller::getCurrentLocale () )->where ( 'SProductPropertiesData.ProductId = ?', $this->getId () )->select ( array (
				'Id',
				'ShowInCompare',
				'ShowInFilter',
				'ShowOnSite',
		...
		) )->withColumn ( 'SProductPropertiesData.Value', 'Value' )->distinct ()->find ()->toArray ();
		...
		foreach ( $properties as $key => $prop ) {
			if (array_key_exists ( $prop ['SPropertiesI18n.Name'], $propertiesRes )) {
				...
				$propertiesRes [$prop ['SPropertiesI18n.Name']] = array (
						$prop ['Value'] 
				);
			}
		}
