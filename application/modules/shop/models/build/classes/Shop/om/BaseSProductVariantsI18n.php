 ...
 */
abstract class BaseSProductVariantsI18n extends ShopBaseObject implements Persistent {
	
	/**
	 * Peer class name
	 */
	const PEER = 'SProductVariantsI18nPeer';
	 ...
	 * @param int $v
	 *        	new value
	 * @return SProductVariantsI18n The current object (for fluent API support)
	 */
	public function setId($v) {
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns [] = SProductVariantsI18nPeer::ID;
		}
	...
	public function setLocale($v) {
		...
		if ($this->locale !== $v) {
			$this->locale = $v;
			$this->modifiedColumns [] = SProductVariantsI18nPeer::LOCALE;
		}
	...
	public function setName($v) {
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns [] = SProductVariantsI18nPeer::NAME;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false) {
		try {
			...
			return $startcol + 3; // 3 = SProductVariantsI18nPeer::NUM_HYDRATE_COLUMNS.
		} catch ( Exception $e ) {
			throw new PropelException ( "Error populating SProductVariantsI18n object", $e );
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null) {
		...
		$stmt = SProductVariantsI18nPeer::doSelectStmt ( $this->buildPkeyCriteria (), $con );
	...
	public function delete(PropelPDO $con = null) {
		...
		try {
			$deleteQuery = SProductVariantsI18nQuery::create ()->filterByPrimaryKey ( $this->getPrimaryKey () );
	...
	public function save(PropelPDO $con = null) {
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave ( $con );
				SProductVariantsI18nPeer::addInstanceToPool ( $this );
			} else {
				$affectedRows = 0;
			}
			$con->commit ();
	...
	protected function doInsert(PropelPDO $con) {
		...
		if ($this->isColumnModified ( SProductVariantsI18nPeer::ID )) {
			$modifiedColumns [':p' . $index ++] = '`ID`';
		}
		if ($this->isColumnModified ( SProductVariantsI18nPeer::LOCALE )) {
			$modifiedColumns [':p' . $index ++] = '`LOCALE`';
		}
		if ($this->isColumnModified ( SProductVariantsI18nPeer::NAME )) {
			$modifiedColumns [':p' . $index ++] = '`NAME`';
		}
		
		$sql = sprintf ( 'INSERT INTO `shop_product_variants_i18n` (%s) VALUES (%s)', implode ( ', ', $modifiedColumns ), implode ( ', ', array_keys ( $modifiedColumns ) ) );
	...
	protected function doValidate($columns = null) {
		if (! $this->alreadyInValidation) {
			...
			if (($retval = SProductVariantsI18nPeer::doValidate ( $this, $columns )) !== true) {
	...
	public function getByName($name, $type = BasePeer::TYPE_PHPNAME) {
		$pos = SProductVariantsI18nPeer::translateFieldName ( $name, $type, BasePeer::TYPE_NUM );
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false) {
		if (isset ( $alreadyDumpedObjects ['SProductVariantsI18n'] [serialize ( $this->getPrimaryKey () )] )) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects ['SProductVariantsI18n'] [serialize ( $this->getPrimaryKey () )] = true;
		$keys = SProductVariantsI18nPeer::getFieldNames ( $keyType );
		$result = array (
				$keys [0] => $this->getId (),
	...
	public function buildCriteria() {
		...
		if ($this->isColumnModified ( SProductVariantsI18nPeer::ID ))
			$criteria->add ( SProductVariantsI18nPeer::ID, $this->id );
		if ($this->isColumnModified ( SProductVariantsI18nPeer::LOCALE ))
			$criteria->add ( SProductVariantsI18nPeer::LOCALE, $this->locale );
		if ($this->isColumnModified ( SProductVariantsI18nPeer::NAME ))
			$criteria->add ( SProductVariantsI18nPeer::NAME, $this->name );
	 ...
	 *        	An object of SProductVariantsI18n (or compatible) type.
	 ...
	 *
	 * @return SProductVariantsI18nPeer
	 */
	public function getPeer() {
		if (self::$peer === null) {
			self::$peer = new SProductVariantsI18nPeer ();
		}
		return self::$peer;
	}
	...
	public function getSProductVariants(PropelPDO $con = null) {
		if ($this->aSProductVariants === null && ($this->id !== null)) {
			$this->aSProductVariants = SProductVariantsQuery::create ()->findPk ( $this->id, $con );
			/*
	...
	public function __toString() {
		return ( string ) $this->exportTo ( SProductVariantsI18nPeer::DEFAULT_STRING_FORMAT );
	}
} // BaseSProductVariantsI18n
