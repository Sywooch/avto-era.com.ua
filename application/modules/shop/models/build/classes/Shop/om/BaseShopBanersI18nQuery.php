...
abstract class BaseShopBanersI18nQuery extends ModelCriteria
{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = ShopBanersI18nPeer::getInstanceFromPool(serialize(array((string) $key[0], (string) $key[1]))))) && !$this->formatter) {
	...
	protected function findPkSimple($key, $con)
	{
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			$obj->hydrate($row);
			ShopBanersI18nPeer::addInstanceToPool($obj, serialize(array((string) $key[0], (string) $key[1])));
		}
		$stmt->closeCursor();
	 ...
	 *
	 * @param     ShopBaners|PropelCollection $shopBaners The related object(s) to use as filter
	 * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
	 *
	 * @return    ShopBanersI18nQuery The current query, for fluid interface
	 */
	public function filterByShopBaners($shopBaners, $comparison = null)
	{
		if ($shopBaners instanceof ShopBaners) {
			return $this
				->addUsingAlias(ShopBanersI18nPeer::ID, $shopBaners->getId(), $comparison);
		} elseif ($shopBaners instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this
				->addUsingAlias(ShopBanersI18nPeer::ID, $shopBaners->toKeyValue('PrimaryKey', 'Id'), $comparison);
		} else {
	 ...
	 *
	 * @param     ShopBanersI18n $shopBanersI18n Object to remove from the list of results
	 *
	 * @return    ShopBanersI18nQuery The current query, for fluid interface
	 */
