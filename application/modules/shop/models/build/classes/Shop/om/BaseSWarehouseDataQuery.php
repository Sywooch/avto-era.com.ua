...
abstract class BaseSWarehouseDataQuery extends ModelCriteria {
	...
	public function filterBySProducts($sProducts, $comparison = null) {
		if ($sProducts instanceof SProducts) {
			return $this->addUsingAlias ( SWarehouseDataPeer::PRODUCT_ID, $sProducts->getId (), $comparison );
		} elseif ($sProducts instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this->addUsingAlias ( SWarehouseDataPeer::PRODUCT_ID, $sProducts->toKeyValue ( 'PrimaryKey', 'Id' ), $comparison );
		} else {
	 ...
	 *
	 * @param SWarehouses|PropelCollection $sWarehouses
	 *        	The related object(s) to use as filter
	 * @param string $comparison
	 *        	Operator to use for the column comparison, defaults to Criteria::EQUAL
	 *        	
	...
	public function filterBySWarehouses($sWarehouses, $comparison = null) {
		if ($sWarehouses instanceof SWarehouses) {
			return $this->addUsingAlias ( SWarehouseDataPeer::WAREHOUSE_ID, $sWarehouses->getId (), $comparison );
		} elseif ($sWarehouses instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this->addUsingAlias ( SWarehouseDataPeer::WAREHOUSE_ID, $sWarehouses->toKeyValue ( 'PrimaryKey', 'Id' ), $comparison );
		} else {
	...
	public function useSWarehousesQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN) {
		return $this->joinSWarehouses ( $relationAlias, $joinType )->useQuery ( $relationAlias ? $relationAlias : 'SWarehouses', 'SWarehousesQuery' );
	}
