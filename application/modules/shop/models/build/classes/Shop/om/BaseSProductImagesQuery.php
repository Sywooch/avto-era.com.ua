...
abstract class BaseSProductImagesQuery extends ModelCriteria {
	 ...
	 * </code>
	 *
	 * @param array[$product_id, $image_name]
	...
	public function findPk($key, $con = null) {
		...
		if ((null !== ($obj = SProductImagesPeer::getInstanceFromPool ( serialize ( array (
				( string ) $key [0],
				( string ) $key [1] 
		) ) ))) && ! $this->formatter) {
	...
	protected function findPkSimple($key, $con) {
		...
		if ($row = $stmt->fetch ( PDO::FETCH_NUM )) {
			...
			$obj->hydrate ( $row );
			SProductImagesPeer::addInstanceToPool ( $obj, serialize ( array (
					( string ) $key [0],
					( string ) $key [1] 
			) ) );
		}
	...
	public function filterBySProducts($sProducts, $comparison = null) {
		if ($sProducts instanceof SProducts) {
			return $this->addUsingAlias ( SProductImagesPeer::PRODUCT_ID, $sProducts->getId (), $comparison );
		} elseif ($sProducts instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this->addUsingAlias ( SProductImagesPeer::PRODUCT_ID, $sProducts->toKeyValue ( 'PrimaryKey', 'Id' ), $comparison );
		} else {
