...
abstract class BaseSPaymentMethodsI18nQuery extends ModelCriteria
{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = SPaymentMethodsI18nPeer::getInstanceFromPool(serialize(array((string) $key[0], (string) $key[1]))))) && !$this->formatter) {
	...
	protected function findPkSimple($key, $con)
	{
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			$obj->hydrate($row);
			SPaymentMethodsI18nPeer::addInstanceToPool($obj, serialize(array((string) $key[0], (string) $key[1])));
		}
		$stmt->closeCursor();
	...
	public function filterBySPaymentMethods($sPaymentMethods, $comparison = null)
	{
		if ($sPaymentMethods instanceof SPaymentMethods) {
			return $this
				->addUsingAlias(SPaymentMethodsI18nPeer::ID, $sPaymentMethods->getId(), $comparison);
		} elseif ($sPaymentMethods instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this
				->addUsingAlias(SPaymentMethodsI18nPeer::ID, $sPaymentMethods->toKeyValue('PrimaryKey', 'Id'), $comparison);
		} else {
	 ...
	 *
	 * @param     SPaymentMethodsI18n $sPaymentMethodsI18n Object to remove from the list of results
	 *
	 * @return    SPaymentMethodsI18nQuery The current query, for fluid interface
	 */
