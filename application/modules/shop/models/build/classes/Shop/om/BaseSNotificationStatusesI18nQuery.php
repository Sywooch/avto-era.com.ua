...
abstract class BaseSNotificationStatusesI18nQuery extends ModelCriteria {
	...
	public function findPk($key, $con = null) {
		...
		if ((null !== ($obj = SNotificationStatusesI18nPeer::getInstanceFromPool ( serialize ( array (
				( string ) $key [0],
				( string ) $key [1] 
		) ) ))) && ! $this->formatter) {
	...
	protected function findPkSimple($key, $con) {
		...
		if ($row = $stmt->fetch ( PDO::FETCH_NUM )) {
			...
			$obj->hydrate ( $row );
			SNotificationStatusesI18nPeer::addInstanceToPool ( $obj, serialize ( array (
					( string ) $key [0],
					( string ) $key [1] 
			) ) );
		}
	...
	public function filterBySNotificationStatuses($sNotificationStatuses, $comparison = null) {
		if ($sNotificationStatuses instanceof SNotificationStatuses) {
			return $this->addUsingAlias ( SNotificationStatusesI18nPeer::ID, $sNotificationStatuses->getId (), $comparison );
		} elseif ($sNotificationStatuses instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this->addUsingAlias ( SNotificationStatusesI18nPeer::ID, $sNotificationStatuses->toKeyValue ( 'PrimaryKey', 'Id' ), $comparison );
		} else {
