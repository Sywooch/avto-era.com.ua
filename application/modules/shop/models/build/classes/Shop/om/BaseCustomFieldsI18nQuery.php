...
abstract class BaseCustomFieldsI18nQuery extends ModelCriteria
{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = CustomFieldsI18nPeer::getInstanceFromPool(serialize(array((string) $key[0], (string) $key[1]))))) && !$this->formatter) {
	...
	protected function findPkSimple($key, $con)
	{
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			$obj->hydrate($row);
			CustomFieldsI18nPeer::addInstanceToPool($obj, serialize(array((string) $key[0], (string) $key[1])));
		}
		$stmt->closeCursor();
	 ...
	 *
	 * @param     CustomFields|PropelCollection $customFields The related object(s) to use as filter
	 * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
	 *
	 * @return    CustomFieldsI18nQuery The current query, for fluid interface
	 */
	public function filterByCustomFields($customFields, $comparison = null)
	{
		if ($customFields instanceof CustomFields) {
			return $this
				->addUsingAlias(CustomFieldsI18nPeer::ID, $customFields->getId(), $comparison);
		} elseif ($customFields instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this
				->addUsingAlias(CustomFieldsI18nPeer::ID, $customFields->toKeyValue('PrimaryKey', 'Id'), $comparison);
		} else {
	 ...
	 *
	 * @param     CustomFieldsI18n $customFieldsI18n Object to remove from the list of results
	 *
	 * @return    CustomFieldsI18nQuery The current query, for fluid interface
	 */
