...
abstract class BaseSPaymentMethods extends ShopBaseObject  implements Persistent
{
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = SPaymentMethodsQuery::create()
			...
			if ($this->sDeliveryMethodssScheduledForDeletion !== null) {
				if (!$this->sDeliveryMethodssScheduledForDeletion->isEmpty()) {
					ShopDeliveryMethodsSystemsQuery::create()
						->filterByPrimaryKeys($this->sDeliveryMethodssScheduledForDeletion->getPrimaryKeys(false))
						->delete($con);
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setPaymentSystemName($arr[$keys[3]]);
	...
	public function getSPaymentMethodsI18ns($criteria = null, PropelPDO $con = null)
	{
		if(null === $this->collSPaymentMethodsI18ns || null !== $criteria) {
			if ($this->isNew() && null === $this->collSPaymentMethodsI18ns) {
				...
				$collSPaymentMethodsI18ns = SPaymentMethodsI18nQuery::create(null, $criteria)
					->filterBySPaymentMethods($this)
					->find($con);
				if (null !== $criteria) {
	 ...
	 * and new objects from the given Propel collection.
	 *
	 * @param      PropelCollection $sPaymentMethodsI18ns A Propel collection.
	 * @param      PropelPDO $con Optional connection object
	 */
	 ...
	 *
	 * @param      SPaymentMethodsI18n $l SPaymentMethodsI18n
	 * @return     SPaymentMethods The current object (for fluent API support)
	 */
	...
	/**
	 * @param	SPaymentMethodsI18n $sPaymentMethodsI18n The sPaymentMethodsI18n object to add.
	 */
	 ...
	 * and new objects from the given Propel collection.
	 *
	 * @param      PropelCollection $sDeliveryMethodss A Propel collection.
	 * @param      PropelPDO $con Optional connection object
	 */
	public function setSDeliveryMethodss(PropelCollection $sDeliveryMethodss, PropelPDO $con = null)
	{
		$shopDeliveryMethodsSystemss = ShopDeliveryMethodsSystemsQuery::create()
			->filterBySDeliveryMethods($sDeliveryMethodss)
			->filterByPaymentMethods($this)
			->find($con);
	...
	/**
	 * @param	SDeliveryMethods $sDeliveryMethods The sDeliveryMethods object to add.
	 */
	...
	public function getTranslation($locale = 'ru', PropelPDO $con = null)
	{
		if (!isset($this->currentTranslations[$locale])) {
			...
			if ($this->isNew()) {
				...
				$translation = SPaymentMethodsI18nQuery::create()
