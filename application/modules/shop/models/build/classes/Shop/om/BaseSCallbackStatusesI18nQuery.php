...
abstract class BaseSCallbackStatusesI18nQuery extends ModelCriteria
{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = SCallbackStatusesI18nPeer::getInstanceFromPool(serialize(array((string) $key[0], (string) $key[1]))))) && !$this->formatter) {
	...
	protected function findPkSimple($key, $con)
	{
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			$obj->hydrate($row);
			SCallbackStatusesI18nPeer::addInstanceToPool($obj, serialize(array((string) $key[0], (string) $key[1])));
		}
		$stmt->closeCursor();
	...
	public function filterBySCallbackStatuses($sCallbackStatuses, $comparison = null)
	{
		if ($sCallbackStatuses instanceof SCallbackStatuses) {
			return $this
				->addUsingAlias(SCallbackStatusesI18nPeer::ID, $sCallbackStatuses->getId(), $comparison);
		} elseif ($sCallbackStatuses instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this
				->addUsingAlias(SCallbackStatusesI18nPeer::ID, $sCallbackStatuses->toKeyValue('PrimaryKey', 'Id'), $comparison);
		} else {
	 ...
	 *
	 * @param     SCallbackStatusesI18n $sCallbackStatusesI18n Object to remove from the list of results
	 *
	 * @return    SCallbackStatusesI18nQuery The current query, for fluid interface
	 */
