...
abstract class BaseShopKitProductQuery extends ModelCriteria
{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = ShopKitProductPeer::getInstanceFromPool(serialize(array((string) $key[0], (string) $key[1]))))) && !$this->formatter) {
	...
	protected function findPkSimple($key, $con)
	{
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			$obj->hydrate($row);
			ShopKitProductPeer::addInstanceToPool($obj, serialize(array((string) $key[0], (string) $key[1])));
		}
		$stmt->closeCursor();
	...
	public function filterBySProducts($sProducts, $comparison = null)
	{
		if ($sProducts instanceof SProducts) {
			return $this
				->addUsingAlias(ShopKitProductPeer::PRODUCT_ID, $sProducts->getId(), $comparison);
		} elseif ($sProducts instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this
				->addUsingAlias(ShopKitProductPeer::PRODUCT_ID, $sProducts->toKeyValue('PrimaryKey', 'Id'), $comparison);
		} else {
	...
	public function filterBySProductVariants($sProductVariants, $comparison = null)
	{
		if ($sProductVariants instanceof SProductVariants) {
			return $this
				->addUsingAlias(ShopKitProductPeer::PRODUCT_ID, $sProductVariants->getProductId(), $comparison);
		} elseif ($sProductVariants instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this
				->addUsingAlias(ShopKitProductPeer::PRODUCT_ID, $sProductVariants->toKeyValue('PrimaryKey', 'ProductId'), $comparison);
		} else {
	 ...
	 *
	 * @param     ShopKit|PropelCollection $shopKit The related object(s) to use as filter
	 * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
	 *
	 * @return    ShopKitProductQuery The current query, for fluid interface
	 */
	public function filterByShopKit($shopKit, $comparison = null)
	{
		if ($shopKit instanceof ShopKit) {
			return $this
				->addUsingAlias(ShopKitProductPeer::KIT_ID, $shopKit->getId(), $comparison);
		} elseif ($shopKit instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this
				->addUsingAlias(ShopKitProductPeer::KIT_ID, $shopKit->toKeyValue('PrimaryKey', 'Id'), $comparison);
		} else {
	 ...
	 *
	 * @param     ShopKitProduct $shopKitProduct Object to remove from the list of results
	 *
	 * @return    ShopKitProductQuery The current query, for fluid interface
	 */
