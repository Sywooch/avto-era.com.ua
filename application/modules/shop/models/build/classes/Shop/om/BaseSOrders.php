...
abstract class BaseSOrders extends ShopBaseObject implements Persistent {
	...
	public function delete(PropelPDO $con = null) {
		...
		try {
			$deleteQuery = SOrdersQuery::create ()->filterByPrimaryKey ( $this->getPrimaryKey () );
	...
	protected function doInsert(PropelPDO $con) {
		...
		$sql = sprintf ( 'INSERT INTO `shop_orders` (%s) VALUES (%s)', implode ( ', ', $modifiedColumns ), implode ( ', ', array_keys ( $modifiedColumns ) ) );
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME) {
		...
		if (array_key_exists ( $keys [9], $arr ))
			$this->setUserEmail ( $arr [$keys [9]] );
		if (array_key_exists ( $keys [10], $arr ))
			$this->setUserPhone ( $arr [$keys [10]] );
		if (array_key_exists ( $keys [11], $arr ))
			$this->setUserDeliverTo ( $arr [$keys [11]] );
		if (array_key_exists ( $keys [12], $arr ))
			$this->setUserComment ( $arr [$keys [12]] );
		if (array_key_exists ( $keys [13], $arr ))
			$this->setDateCreated ( $arr [$keys [13]] );
		if (array_key_exists ( $keys [14], $arr ))
			$this->setDateUpdated ( $arr [$keys [14]] );
		if (array_key_exists ( $keys [15], $arr ))
			$this->setUserIp ( $arr [$keys [15]] );
		if (array_key_exists ( $keys [16], $arr ))
			$this->setTotalPrice ( $arr [$keys [16]] );
		if (array_key_exists ( $keys [17], $arr ))
			$this->setExternalId ( $arr [$keys [17]] );
		if (array_key_exists ( $keys [18], $arr ))
			$this->setGiftCertKey ( $arr [$keys [18]] );
		if (array_key_exists ( $keys [19], $arr ))
			$this->setDiscount ( $arr [$keys [19]] );
		if (array_key_exists ( $keys [20], $arr ))
			$this->setGiftCertPrice ( $arr [$keys [20]] );
		if (array_key_exists ( $keys [21], $arr ))
			$this->setDiscountInfo ( $arr [$keys [21]] );
		if (array_key_exists ( $keys [22], $arr ))
			$this->setOriginPrice ( $arr [$keys [22]] );
		if (array_key_exists ( $keys [23], $arr ))
			$this->setComulativ ( $arr [$keys [23]] );
	}
	...
	public function buildCriteria() {
		...
		if ($this->isColumnModified ( SOrdersPeer::USER_ID ))
			$criteria->add ( SOrdersPeer::USER_ID, $this->user_id );
		if ($this->isColumnModified ( SOrdersPeer::KEY ))
			$criteria->add ( SOrdersPeer::KEY, $this->key );
		if ($this->isColumnModified ( SOrdersPeer::DELIVERY_METHOD ))
			$criteria->add ( SOrdersPeer::DELIVERY_METHOD, $this->delivery_method );
		if ($this->isColumnModified ( SOrdersPeer::DELIVERY_PRICE ))
			$criteria->add ( SOrdersPeer::DELIVERY_PRICE, $this->delivery_price );
		if ($this->isColumnModified ( SOrdersPeer::PAYMENT_METHOD ))
			$criteria->add ( SOrdersPeer::PAYMENT_METHOD, $this->payment_method );
		if ($this->isColumnModified ( SOrdersPeer::STATUS ))
			$criteria->add ( SOrdersPeer::STATUS, $this->status );
		if ($this->isColumnModified ( SOrdersPeer::PAID ))
			$criteria->add ( SOrdersPeer::PAID, $this->paid );
		if ($this->isColumnModified ( SOrdersPeer::USER_FULL_NAME ))
			$criteria->add ( SOrdersPeer::USER_FULL_NAME, $this->user_full_name );
		if ($this->isColumnModified ( SOrdersPeer::USER_EMAIL ))
			$criteria->add ( SOrdersPeer::USER_EMAIL, $this->user_email );
		if ($this->isColumnModified ( SOrdersPeer::USER_PHONE ))
			$criteria->add ( SOrdersPeer::USER_PHONE, $this->user_phone );
		if ($this->isColumnModified ( SOrdersPeer::USER_DELIVER_TO ))
			$criteria->add ( SOrdersPeer::USER_DELIVER_TO, $this->user_deliver_to );
		if ($this->isColumnModified ( SOrdersPeer::USER_COMMENT ))
			$criteria->add ( SOrdersPeer::USER_COMMENT, $this->user_comment );
		if ($this->isColumnModified ( SOrdersPeer::DATE_CREATED ))
			$criteria->add ( SOrdersPeer::DATE_CREATED, $this->date_created );
		if ($this->isColumnModified ( SOrdersPeer::DATE_UPDATED ))
			$criteria->add ( SOrdersPeer::DATE_UPDATED, $this->date_updated );
		if ($this->isColumnModified ( SOrdersPeer::USER_IP ))
			$criteria->add ( SOrdersPeer::USER_IP, $this->user_ip );
		if ($this->isColumnModified ( SOrdersPeer::TOTAL_PRICE ))
			$criteria->add ( SOrdersPeer::TOTAL_PRICE, $this->total_price );
		if ($this->isColumnModified ( SOrdersPeer::EXTERNAL_ID ))
			$criteria->add ( SOrdersPeer::EXTERNAL_ID, $this->external_id );
		if ($this->isColumnModified ( SOrdersPeer::GIFT_CERT_KEY ))
			$criteria->add ( SOrdersPeer::GIFT_CERT_KEY, $this->gift_cert_key );
		if ($this->isColumnModified ( SOrdersPeer::DISCOUNT ))
			$criteria->add ( SOrdersPeer::DISCOUNT, $this->discount );
		if ($this->isColumnModified ( SOrdersPeer::GIFT_CERT_PRICE ))
			$criteria->add ( SOrdersPeer::GIFT_CERT_PRICE, $this->gift_cert_price );
		if ($this->isColumnModified ( SOrdersPeer::DISCOUNT_INFO ))
			$criteria->add ( SOrdersPeer::DISCOUNT_INFO, $this->discount_info );
		if ($this->isColumnModified ( SOrdersPeer::ORIGIN_PRICE ))
			$criteria->add ( SOrdersPeer::ORIGIN_PRICE, $this->origin_price );
		if ($this->isColumnModified ( SOrdersPeer::COMULATIV ))
			$criteria->add ( SOrdersPeer::COMULATIV, $this->comulativ );
	 ...
	 *        	An object of SOrders (or compatible) type.
	...
	public function getSOrderProductss($criteria = null, PropelPDO $con = null) {
		if (null === $this->collSOrderProductss || null !== $criteria) {
			if ($this->isNew () && null === $this->collSOrderProductss) {
				...
				$collSOrderProductss = SOrderProductsQuery::create ( null, $criteria )->filterBySOrders ( $this )->find ( $con );
				if (null !== $criteria) {
	...
	public function countSOrderProductss(Criteria $criteria = null, $distinct = false, PropelPDO $con = null) {
		if (null === $this->collSOrderProductss || null !== $criteria) {
			if ($this->isNew () && null === $this->collSOrderProductss) {
				...
				return $query->filterBySOrders ( $this )->count ( $con );
			}
		} else {
	...
	public function getSOrderStatusHistorys($criteria = null, PropelPDO $con = null) {
		if (null === $this->collSOrderStatusHistorys || null !== $criteria) {
			if ($this->isNew () && null === $this->collSOrderStatusHistorys) {
				...
				$collSOrderStatusHistorys = SOrderStatusHistoryQuery::create ( null, $criteria )->filterBySOrders ( $this )->find ( $con );
				if (null !== $criteria) {
