...
abstract class BaseShopSettingsQuery extends ModelCriteria
{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = ShopSettingsPeer::getInstanceFromPool(serialize(array((string) $key[0], (string) $key[1]))))) && !$this->formatter) {
	...
	protected function findPkSimple($key, $con)
	{
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			$obj->hydrate($row);
			ShopSettingsPeer::addInstanceToPool($obj, serialize(array((string) $key[0], (string) $key[1])));
		}
		$stmt->closeCursor();
