...
abstract class BaseSBrands extends ShopBaseObject implements Persistent {
	...
	/**
	 *
	 * @var array SBrandsI18n[] Collection to store aggregation of SBrandsI18n objects.
	 */
	...
	public function setImage($v) {
		...
		if ($this->image !== $v) {
			$this->image = $v;
			$this->modifiedColumns [] = SBrandsPeer::IMAGE;
		}
	...
	public function setPosition($v) {
		...
		if ($this->position !== $v) {
			$this->position = $v;
			$this->modifiedColumns [] = SBrandsPeer::POSITION;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false) {
		try {
			...
			return $startcol + 4; // 4 = SBrandsPeer::NUM_HYDRATE_COLUMNS.
		} catch ( Exception $e ) {
	...
	public function delete(PropelPDO $con = null) {
		...
		try {
			$deleteQuery = SBrandsQuery::create ()->filterByPrimaryKey ( $this->getPrimaryKey () );
			...
			if ($ret) {
				...
				SBrandsI18nQuery::create ()->filterBySBrands ( $this )->delete ( $con );
				$con->commit ();
		...
		if (! $this->alreadyInSave) {
			...
			if ($this->sBrandsI18nsScheduledForDeletion !== null) {
				if (! $this->sBrandsI18nsScheduledForDeletion->isEmpty ()) {
					SBrandsI18nQuery::create ()->filterByPrimaryKeys ( $this->sBrandsI18nsScheduledForDeletion->getPrimaryKeys ( false ) )->delete ( $con );
	...
	protected function doInsert(PropelPDO $con) {
		...
		if ($this->isColumnModified ( SBrandsPeer::ID )) {
			$modifiedColumns [':p' . $index ++] = '`ID`';
		}
		if ($this->isColumnModified ( SBrandsPeer::URL )) {
			$modifiedColumns [':p' . $index ++] = '`URL`';
		}
		if ($this->isColumnModified ( SBrandsPeer::IMAGE )) {
			$modifiedColumns [':p' . $index ++] = '`IMAGE`';
		}
		if ($this->isColumnModified ( SBrandsPeer::POSITION )) {
			$modifiedColumns [':p' . $index ++] = '`POSITION`';
		}
		
		$sql = sprintf ( 'INSERT INTO `shop_brands` (%s) VALUES (%s)', implode ( ', ', $modifiedColumns ), implode ( ', ', array_keys ( $modifiedColumns ) ) );
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false) {
		if (isset ( $alreadyDumpedObjects ['SBrands'] [$this->getPrimaryKey ()] )) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects ['SBrands'] [$this->getPrimaryKey ()] = true;
		...
		if ($includeForeignObjects) {
			...
			if (null !== $this->collSBrandsI18ns) {
				$result ['SBrandsI18ns'] = $this->collSBrandsI18ns->toArray ( null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects );
			}
		}
		return $result;
	}
	...
	public function buildCriteria() {
		...
		if ($this->isColumnModified ( SBrandsPeer::URL ))
			$criteria->add ( SBrandsPeer::URL, $this->url );
		if ($this->isColumnModified ( SBrandsPeer::IMAGE ))
			$criteria->add ( SBrandsPeer::IMAGE, $this->image );
		if ($this->isColumnModified ( SBrandsPeer::POSITION ))
			$criteria->add ( SBrandsPeer::POSITION, $this->position );
	 ...
	 *        	An object of SBrands (or compatible) type.
	...
	public function getSProductss($criteria = null, PropelPDO $con = null) {
		if (null === $this->collSProductss || null !== $criteria) {
			if ($this->isNew () && null === $this->collSProductss) {
				...
				$collSProductss = SProductsQuery::create ( null, $criteria )->filterByBrand ( $this )->find ( $con );
				if (null !== $criteria) {
	...
	public function countSProductss(Criteria $criteria = null, $distinct = false, PropelPDO $con = null) {
		if (null === $this->collSProductss || null !== $criteria) {
			if ($this->isNew () && null === $this->collSProductss) {
				...
				return $query->filterByBrand ( $this )->count ( $con );
			}
		} else {
	...
	public function initSBrandsI18ns($overrideExisting = true) {
		...
		$this->collSBrandsI18ns = new PropelObjectCollection ();
		$this->collSBrandsI18ns->setModel ( 'SBrandsI18n' );
	}
	
	/**
	 * Gets an array of SBrandsI18n objects which contain a foreign key that references this object.
	 *
	 * If the $criteria is not null, it is used to always fetch the results from the database.
	 * Otherwise the results are fetched from the database the first time, then cached.
	 * Next time the same method is called without $criteria, the cached collection is returned.
	 ...
	 * @param PropelPDO $con
	 *        	optional connection object
	 * @return PropelCollection|array SBrandsI18n[] List of SBrandsI18n objects
	 * @throws PropelException
	 */
	public function getSBrandsI18ns($criteria = null, PropelPDO $con = null) {
		if (null === $this->collSBrandsI18ns || null !== $criteria) {
			if ($this->isNew () && null === $this->collSBrandsI18ns) {
				// return empty collection
				$this->initSBrandsI18ns ();
			} else {
				$collSBrandsI18ns = SBrandsI18nQuery::create ( null, $criteria )->filterBySBrands ( $this )->find ( $con );
				if (null !== $criteria) {
	...
	/**
	 * Sets a collection of SBrandsI18n objects related by a one-to-many relationship
	 * to the current object.
	 * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
	 * and new objects from the given Propel collection.
	 *
	 * @param PropelCollection $sBrandsI18ns
	 *        	A Propel collection.
	 * @param PropelPDO $con
	 *        	Optional connection object
	 */
	public function setSBrandsI18ns(PropelCollection $sBrandsI18ns, PropelPDO $con = null) {
		$this->sBrandsI18nsScheduledForDeletion = $this->getSBrandsI18ns ( new Criteria (), $con )->diff ( $sBrandsI18ns );
		
		foreach ( $sBrandsI18ns as $sBrandsI18n ) {
			// Fix issue with collection modified by reference
			if ($sBrandsI18n->isNew ()) {
	 ...
	 * @throws PropelException
	 */
	public function countSBrandsI18ns(Criteria $criteria = null, $distinct = false, PropelPDO $con = null) {
		if (null === $this->collSBrandsI18ns || null !== $criteria) {
			if ($this->isNew () && null === $this->collSBrandsI18ns) {
				return 0;
			} else {
				$query = SBrandsI18nQuery::create ( null, $criteria );
				if ($distinct) {
					$query->distinct ();
				}
				return $query->filterBySBrands ( $this )->count ( $con );
			}
		} else {
	...
	/**
	 * Method called to associate a SBrandsI18n object to this object
	 * through the SBrandsI18n foreign key attribute.
	 *
	 * @param SBrandsI18n $l
	 *        	SBrandsI18n
	 * @return SBrands The current object (for fluent API support)
	 */
	public function addSBrandsI18n(SBrandsI18n $l) {
		...
		if (! $this->collSBrandsI18ns->contains ( $l )) { // only add it if the **same** object is not already associated
	...
	/**
	 *
	 * @param SBrandsI18n $sBrandsI18n
	 *        	The sBrandsI18n object to add.
	 */
	protected function doAddSBrandsI18n($sBrandsI18n) {
	...
	public function clearAllReferences($deep = false) {
		...
		$this->collSProductss = null;
		if ($this->collSBrandsI18ns instanceof PropelCollection) {
			$this->collSBrandsI18ns->clearIterator ();
		}
		$this->collSBrandsI18ns = null;
	}
	...
	public function getTranslation($locale = 'ru', PropelPDO $con = null) {
		if (! isset ( $this->currentTranslations [$locale] )) {
			...
			if ($this->isNew ()) {
				$translation = new SBrandsI18n ();
				$translation->setLocale ( $locale );
			} else {
				$translation = SBrandsI18nQuery::create ()->filterByPrimaryKey ( array (
						$this->getPrimaryKey (),
						$locale 
				) )->findOneOrCreate ( $con );
				$this->currentTranslations [$locale] = $translation;
	...
	public function removeTranslation($locale = 'ru', PropelPDO $con = null) {
		if (! $this->isNew ()) {
			SBrandsI18nQuery::create ()->filterByPrimaryKey ( array (
					$this->getPrimaryKey (),
					$locale 
			) )->delete ( $con );
		}
