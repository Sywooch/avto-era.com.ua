 ...
 */
abstract class BaseSOrderStatusesI18n extends ShopBaseObject implements Persistent {
	
	/**
	 * Peer class name
	 */
	const PEER = 'SOrderStatusesI18nPeer';
	 ...
	 * @param int $v
	 *        	new value
	 * @return SOrderStatusesI18n The current object (for fluent API support)
	 */
	public function setId($v) {
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns [] = SOrderStatusesI18nPeer::ID;
		}
	...
	public function setLocale($v) {
		...
		if ($this->locale !== $v) {
			$this->locale = $v;
			$this->modifiedColumns [] = SOrderStatusesI18nPeer::LOCALE;
		}
	...
	public function setName($v) {
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns [] = SOrderStatusesI18nPeer::NAME;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false) {
		try {
			...
			return $startcol + 3; // 3 = SOrderStatusesI18nPeer::NUM_HYDRATE_COLUMNS.
		} catch ( Exception $e ) {
			throw new PropelException ( "Error populating SOrderStatusesI18n object", $e );
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null) {
		...
		$stmt = SOrderStatusesI18nPeer::doSelectStmt ( $this->buildPkeyCriteria (), $con );
	...
	public function delete(PropelPDO $con = null) {
		...
		try {
			$deleteQuery = SOrderStatusesI18nQuery::create ()->filterByPrimaryKey ( $this->getPrimaryKey () );
	...
	public function save(PropelPDO $con = null) {
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave ( $con );
				SOrderStatusesI18nPeer::addInstanceToPool ( $this );
			} else {
				$affectedRows = 0;
			}
			$con->commit ();
	...
	protected function doInsert(PropelPDO $con) {
		...
		if ($this->isColumnModified ( SOrderStatusesI18nPeer::ID )) {
			$modifiedColumns [':p' . $index ++] = '`ID`';
		}
		if ($this->isColumnModified ( SOrderStatusesI18nPeer::LOCALE )) {
			$modifiedColumns [':p' . $index ++] = '`LOCALE`';
		}
		if ($this->isColumnModified ( SOrderStatusesI18nPeer::NAME )) {
			$modifiedColumns [':p' . $index ++] = '`NAME`';
		}
		
		$sql = sprintf ( 'INSERT INTO `shop_order_statuses_i18n` (%s) VALUES (%s)', implode ( ', ', $modifiedColumns ), implode ( ', ', array_keys ( $modifiedColumns ) ) );
	...
	protected function doValidate($columns = null) {
		if (! $this->alreadyInValidation) {
			...
			if (($retval = SOrderStatusesI18nPeer::doValidate ( $this, $columns )) !== true) {
	...
	public function getByName($name, $type = BasePeer::TYPE_PHPNAME) {
		$pos = SOrderStatusesI18nPeer::translateFieldName ( $name, $type, BasePeer::TYPE_NUM );
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false) {
		if (isset ( $alreadyDumpedObjects ['SOrderStatusesI18n'] [serialize ( $this->getPrimaryKey () )] )) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects ['SOrderStatusesI18n'] [serialize ( $this->getPrimaryKey () )] = true;
		$keys = SOrderStatusesI18nPeer::getFieldNames ( $keyType );
		$result = array (
				$keys [0] => $this->getId (),
	...
	public function buildCriteria() {
		...
		if ($this->isColumnModified ( SOrderStatusesI18nPeer::ID ))
			$criteria->add ( SOrderStatusesI18nPeer::ID, $this->id );
		if ($this->isColumnModified ( SOrderStatusesI18nPeer::LOCALE ))
			$criteria->add ( SOrderStatusesI18nPeer::LOCALE, $this->locale );
		if ($this->isColumnModified ( SOrderStatusesI18nPeer::NAME ))
			$criteria->add ( SOrderStatusesI18nPeer::NAME, $this->name );
	 ...
	 *        	An object of SOrderStatusesI18n (or compatible) type.
	 ...
	 *
	 * @return SOrderStatusesI18nPeer
	 */
	public function getPeer() {
		if (self::$peer === null) {
			self::$peer = new SOrderStatusesI18nPeer ();
		}
		return self::$peer;
	}
	...
	public function getSOrderStatuses(PropelPDO $con = null) {
		if ($this->aSOrderStatuses === null && ($this->id !== null)) {
			$this->aSOrderStatuses = SOrderStatusesQuery::create ()->findPk ( $this->id, $con );
			/*
	...
	public function __toString() {
		return ( string ) $this->exportTo ( SOrderStatusesI18nPeer::DEFAULT_STRING_FORMAT );
	}
} // BaseSOrderStatusesI18n
