 ...
 */
abstract class BaseSNotificationStatuses extends ShopBaseObject implements Persistent {
	
	/**
	 * Peer class name
	 */
	const PEER = 'SNotificationStatusesPeer';
	...
	/**
	 *
	 * @var array SNotificationStatusesI18n[] Collection to store aggregation of SNotificationStatusesI18n objects.
	 */
	 ...
	 * @param int $v
	 *        	new value
	 * @return SNotificationStatuses The current object (for fluent API support)
	 */
	public function setId($v) {
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns [] = SNotificationStatusesPeer::ID;
		}
	...
	public function setPosition($v) {
		...
		if ($this->position !== $v) {
			$this->position = $v;
			$this->modifiedColumns [] = SNotificationStatusesPeer::POSITION;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false) {
		try {
			...
			return $startcol + 2; // 2 = SNotificationStatusesPeer::NUM_HYDRATE_COLUMNS.
		} catch ( Exception $e ) {
			throw new PropelException ( "Error populating SNotificationStatuses object", $e );
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null) {
		...
		$stmt = SNotificationStatusesPeer::doSelectStmt ( $this->buildPkeyCriteria (), $con );
	...
	public function delete(PropelPDO $con = null) {
		...
		try {
			$deleteQuery = SNotificationStatusesQuery::create ()->filterByPrimaryKey ( $this->getPrimaryKey () );
			...
			if ($ret) {
				...
				SNotificationStatusesI18nQuery::create ()->filterBySNotificationStatuses ( $this )->delete ( $con );
				$con->commit ();
	...
	public function save(PropelPDO $con = null) {
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave ( $con );
				SNotificationStatusesPeer::addInstanceToPool ( $this );
			} else {
				$affectedRows = 0;
			}
			$con->commit ();
		...
		if (! $this->alreadyInSave) {
			...
			if ($this->sNotificationStatusesI18nsScheduledForDeletion !== null) {
				if (! $this->sNotificationStatusesI18nsScheduledForDeletion->isEmpty ()) {
					SNotificationStatusesI18nQuery::create ()->filterByPrimaryKeys ( $this->sNotificationStatusesI18nsScheduledForDeletion->getPrimaryKeys ( false ) )->delete ( $con );
	...
	protected function doInsert(PropelPDO $con) {
		...
		if ($this->isColumnModified ( SNotificationStatusesPeer::ID )) {
			$modifiedColumns [':p' . $index ++] = '`ID`';
		}
		if ($this->isColumnModified ( SNotificationStatusesPeer::POSITION )) {
			$modifiedColumns [':p' . $index ++] = '`POSITION`';
		}
		
		$sql = sprintf ( 'INSERT INTO `shop_notification_statuses` (%s) VALUES (%s)', implode ( ', ', $modifiedColumns ), implode ( ', ', array_keys ( $modifiedColumns ) ) );
	...
	protected function doValidate($columns = null) {
		if (! $this->alreadyInValidation) {
			...
			if (($retval = SNotificationStatusesPeer::doValidate ( $this, $columns )) !== true) {
	...
	public function getByName($name, $type = BasePeer::TYPE_PHPNAME) {
		$pos = SNotificationStatusesPeer::translateFieldName ( $name, $type, BasePeer::TYPE_NUM );
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false) {
		if (isset ( $alreadyDumpedObjects ['SNotificationStatuses'] [$this->getPrimaryKey ()] )) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects ['SNotificationStatuses'] [$this->getPrimaryKey ()] = true;
		$keys = SNotificationStatusesPeer::getFieldNames ( $keyType );
		$result = array (
				$keys [0] => $this->getId (),
		...
		if ($includeForeignObjects) {
			...
			if (null !== $this->collSNotificationStatusesI18ns) {
				$result ['SNotificationStatusesI18ns'] = $this->collSNotificationStatusesI18ns->toArray ( null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects );
			}
		}
		return $result;
	}
	...
	public function buildCriteria() {
		...
		if ($this->isColumnModified ( SNotificationStatusesPeer::ID ))
			$criteria->add ( SNotificationStatusesPeer::ID, $this->id );
		if ($this->isColumnModified ( SNotificationStatusesPeer::POSITION ))
			$criteria->add ( SNotificationStatusesPeer::POSITION, $this->position );
	 ...
	 *        	An object of SNotificationStatuses (or compatible) type.
	 ...
	 *
	 * @return SNotificationStatusesPeer
	 */
	public function getPeer() {
		if (self::$peer === null) {
			self::$peer = new SNotificationStatusesPeer ();
		}
		return self::$peer;
	}
	 ...
	 * If this SNotificationStatuses is new, it will return
	...
	public function getSNotificationss($criteria = null, PropelPDO $con = null) {
		if (null === $this->collSNotificationss || null !== $criteria) {
			if ($this->isNew () && null === $this->collSNotificationss) {
				...
				$collSNotificationss = SNotificationsQuery::create ( null, $criteria )->filterBySNotificationStatuses ( $this )->find ( $con );
				if (null !== $criteria) {
	...
	public function countSNotificationss(Criteria $criteria = null, $distinct = false, PropelPDO $con = null) {
		if (null === $this->collSNotificationss || null !== $criteria) {
			if ($this->isNew () && null === $this->collSNotificationss) {
				...
				return $query->filterBySNotificationStatuses ( $this )->count ( $con );
			}
		} else {
	...
	public function initSNotificationStatusesI18ns($overrideExisting = true) {
		...
		$this->collSNotificationStatusesI18ns = new PropelObjectCollection ();
		$this->collSNotificationStatusesI18ns->setModel ( 'SNotificationStatusesI18n' );
	}
	
	/**
	 * Gets an array of SNotificationStatusesI18n objects which contain a foreign key that references this object.
	 *
	 * If the $criteria is not null, it is used to always fetch the results from the database.
	 * Otherwise the results are fetched from the database the first time, then cached.
	 * Next time the same method is called without $criteria, the cached collection is returned.
	 ...
	 * @param PropelPDO $con
	 *        	optional connection object
	 * @return PropelCollection|array SNotificationStatusesI18n[] List of SNotificationStatusesI18n objects
	 * @throws PropelException
	 */
	public function getSNotificationStatusesI18ns($criteria = null, PropelPDO $con = null) {
		if (null === $this->collSNotificationStatusesI18ns || null !== $criteria) {
			if ($this->isNew () && null === $this->collSNotificationStatusesI18ns) {
				// return empty collection
				$this->initSNotificationStatusesI18ns ();
			} else {
				$collSNotificationStatusesI18ns = SNotificationStatusesI18nQuery::create ( null, $criteria )->filterBySNotificationStatuses ( $this )->find ( $con );
				if (null !== $criteria) {
	...
	/**
	 * Sets a collection of SNotificationStatusesI18n objects related by a one-to-many relationship
	 * to the current object.
	 * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
	 * and new objects from the given Propel collection.
	 *
	 * @param PropelCollection $sNotificationStatusesI18ns
	 *        	A Propel collection.
	 * @param PropelPDO $con
	 *        	Optional connection object
	 */
	public function setSNotificationStatusesI18ns(PropelCollection $sNotificationStatusesI18ns, PropelPDO $con = null) {
		$this->sNotificationStatusesI18nsScheduledForDeletion = $this->getSNotificationStatusesI18ns ( new Criteria (), $con )->diff ( $sNotificationStatusesI18ns );
		
		foreach ( $sNotificationStatusesI18ns as $sNotificationStatusesI18n ) {
			// Fix issue with collection modified by reference
			if ($sNotificationStatusesI18n->isNew ()) {
	 ...
	 * @throws PropelException
	 */
	public function countSNotificationStatusesI18ns(Criteria $criteria = null, $distinct = false, PropelPDO $con = null) {
		if (null === $this->collSNotificationStatusesI18ns || null !== $criteria) {
			if ($this->isNew () && null === $this->collSNotificationStatusesI18ns) {
				return 0;
			} else {
				$query = SNotificationStatusesI18nQuery::create ( null, $criteria );
				if ($distinct) {
					$query->distinct ();
				}
				return $query->filterBySNotificationStatuses ( $this )->count ( $con );
	...
	/**
	 * Method called to associate a SNotificationStatusesI18n object to this object
	 * through the SNotificationStatusesI18n foreign key attribute.
	 *
	 * @param SNotificationStatusesI18n $l
	 *        	SNotificationStatusesI18n
	 * @return SNotificationStatuses The current object (for fluent API support)
	 */
	public function addSNotificationStatusesI18n(SNotificationStatusesI18n $l) {
		...
		if (! $this->collSNotificationStatusesI18ns->contains ( $l )) { // only add it if the **same** object is not already associated
	 ...
	 *        	The sNotificationStatusesI18n object to add.
	 */
	protected function doAddSNotificationStatusesI18n($sNotificationStatusesI18n) {
	...
	public function clearAllReferences($deep = false) {
		...
		$this->collSNotificationss = null;
		if ($this->collSNotificationStatusesI18ns instanceof PropelCollection) {
			$this->collSNotificationStatusesI18ns->clearIterator ();
		}
		$this->collSNotificationStatusesI18ns = null;
	}
	...
	public function __toString() {
		return ( string ) $this->exportTo ( SNotificationStatusesPeer::DEFAULT_STRING_FORMAT );
	}
	...
	public function getTranslation($locale = 'ru', PropelPDO $con = null) {
		if (! isset ( $this->currentTranslations [$locale] )) {
			...
			if ($this->isNew ()) {
				$translation = new SNotificationStatusesI18n ();
				$translation->setLocale ( $locale );
			} else {
				$translation = SNotificationStatusesI18nQuery::create ()->filterByPrimaryKey ( array (
						$this->getPrimaryKey (),
						$locale 
				) )->findOneOrCreate ( $con );
				$this->currentTranslations [$locale] = $translation;
	...
	public function removeTranslation($locale = 'ru', PropelPDO $con = null) {
		if (! $this->isNew ()) {
			SNotificationStatusesI18nQuery::create ()->filterByPrimaryKey ( array (
					$this->getPrimaryKey (),
					$locale 
			) )->delete ( $con );
		}
