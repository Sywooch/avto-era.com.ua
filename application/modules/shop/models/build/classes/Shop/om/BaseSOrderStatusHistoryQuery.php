...
abstract class BaseSOrderStatusHistoryQuery extends ModelCriteria {
	...
	public function filterBySOrders($sOrders, $comparison = null) {
		if ($sOrders instanceof SOrders) {
			return $this->addUsingAlias ( SOrderStatusHistoryPeer::ORDER_ID, $sOrders->getId (), $comparison );
		} elseif ($sOrders instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this->addUsingAlias ( SOrderStatusHistoryPeer::ORDER_ID, $sOrders->toKeyValue ( 'PrimaryKey', 'Id' ), $comparison );
		} else {
	...
	public function filterBySOrderStatuses($sOrderStatuses, $comparison = null) {
		if ($sOrderStatuses instanceof SOrderStatuses) {
			return $this->addUsingAlias ( SOrderStatusHistoryPeer::STATUS_ID, $sOrderStatuses->getId (), $comparison );
		} elseif ($sOrderStatuses instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this->addUsingAlias ( SOrderStatusHistoryPeer::STATUS_ID, $sOrderStatuses->toKeyValue ( 'PrimaryKey', 'Id' ), $comparison );
		} else {
