...
abstract class BaseShopDeliveryMethodsSystemsQuery extends ModelCriteria {
	 ...
	 * </code>
	 *
	 * @param array[$delivery_method_id, $payment_method_id]
	...
	public function findPk($key, $con = null) {
		...
		if ((null !== ($obj = ShopDeliveryMethodsSystemsPeer::getInstanceFromPool ( serialize ( array (
				( string ) $key [0],
				( string ) $key [1] 
		) ) ))) && ! $this->formatter) {
	 ...
	 *        	A connection object
	 *        	
	 * @return ShopDeliveryMethodsSystems A model object, or null if the key is not found
	 */
	protected function findPkSimple($key, $con) {
		...
		if ($row = $stmt->fetch ( PDO::FETCH_NUM )) {
			$obj = new ShopDeliveryMethodsSystems ();
			$obj->hydrate ( $row );
			ShopDeliveryMethodsSystemsPeer::addInstanceToPool ( $obj, serialize ( array (
					( string ) $key [0],
					( string ) $key [1] 
			) ) );
		}
	...
	public function filterBySDeliveryMethods($sDeliveryMethods, $comparison = null) {
		if ($sDeliveryMethods instanceof SDeliveryMethods) {
			return $this->addUsingAlias ( ShopDeliveryMethodsSystemsPeer::DELIVERY_METHOD_ID, $sDeliveryMethods->getId (), $comparison );
		} elseif ($sDeliveryMethods instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this->addUsingAlias ( ShopDeliveryMethodsSystemsPeer::DELIVERY_METHOD_ID, $sDeliveryMethods->toKeyValue ( 'PrimaryKey', 'Id' ), $comparison );
		} else {
	...
	public function filterByPaymentMethods($sPaymentMethods, $comparison = null) {
		if ($sPaymentMethods instanceof SPaymentMethods) {
			return $this->addUsingAlias ( ShopDeliveryMethodsSystemsPeer::PAYMENT_METHOD_ID, $sPaymentMethods->getId (), $comparison );
		} elseif ($sPaymentMethods instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this->addUsingAlias ( ShopDeliveryMethodsSystemsPeer::PAYMENT_METHOD_ID, $sPaymentMethods->toKeyValue ( 'PrimaryKey', 'Id' ), $comparison );
		} else {
	...
	public function usePaymentMethodsQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN) {
		return $this->joinPaymentMethods ( $relationAlias, $joinType )->useQuery ( $relationAlias ? $relationAlias : 'PaymentMethods', 'SPaymentMethodsQuery' );
	}
