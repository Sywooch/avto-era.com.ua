...
abstract class BaseSCallbacksQuery extends ModelCriteria
{
	 ...
	 *
	 * @param     SCallbackStatuses|PropelCollection $sCallbackStatuses The related object(s) to use as filter
	 * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
	 *
	 * @return    SCallbacksQuery The current query, for fluid interface
	 */
	public function filterBySCallbackStatuses($sCallbackStatuses, $comparison = null)
	{
		if ($sCallbackStatuses instanceof SCallbackStatuses) {
			return $this
				->addUsingAlias(SCallbacksPeer::STATUS_ID, $sCallbackStatuses->getId(), $comparison);
		} elseif ($sCallbackStatuses instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this
				->addUsingAlias(SCallbacksPeer::STATUS_ID, $sCallbackStatuses->toKeyValue('PrimaryKey', 'Id'), $comparison);
		} else {
	...
	public function filterBySCallbackThemes($sCallbackThemes, $comparison = null)
	{
		if ($sCallbackThemes instanceof SCallbackThemes) {
			return $this
				->addUsingAlias(SCallbacksPeer::THEME_ID, $sCallbackThemes->getId(), $comparison);
		} elseif ($sCallbackThemes instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this
				->addUsingAlias(SCallbacksPeer::THEME_ID, $sCallbackThemes->toKeyValue('PrimaryKey', 'Id'), $comparison);
		} else {
	 ...
	 *
	 * @param     SCallbacks $sCallbacks Object to remove from the list of results
	 *
	 * @return    SCallbacksQuery The current query, for fluid interface
	 */
