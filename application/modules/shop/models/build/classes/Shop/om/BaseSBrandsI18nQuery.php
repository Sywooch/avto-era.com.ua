...
abstract class BaseSBrandsI18nQuery extends ModelCriteria {
	...
	public function findPk($key, $con = null) {
		...
		if ((null !== ($obj = SBrandsI18nPeer::getInstanceFromPool ( serialize ( array (
				( string ) $key [0],
				( string ) $key [1] 
		) ) ))) && ! $this->formatter) {
	...
	protected function findPkSimple($key, $con) {
		...
		if ($row = $stmt->fetch ( PDO::FETCH_NUM )) {
			$obj = new SBrandsI18n ();
			$obj->hydrate ( $row );
			SBrandsI18nPeer::addInstanceToPool ( $obj, serialize ( array (
					( string ) $key [0],
					( string ) $key [1] 
			) ) );
		}
	...
	public function filterBySBrands($sBrands, $comparison = null) {
		if ($sBrands instanceof SBrands) {
			return $this->addUsingAlias ( SBrandsI18nPeer::ID, $sBrands->getId (), $comparison );
		} elseif ($sBrands instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this->addUsingAlias ( SBrandsI18nPeer::ID, $sBrands->toKeyValue ( 'PrimaryKey', 'Id' ), $comparison );
		} else {
	...
	public function useSBrandsQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN) {
		return $this->joinSBrands ( $relationAlias, $joinType )->useQuery ( $relationAlias ? $relationAlias : 'SBrands', 'SBrandsQuery' );
	}
