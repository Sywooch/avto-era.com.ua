...
abstract class BaseSProductsRating extends ShopBaseObject implements Persistent {
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false) {
		try {
			...
			return $startcol + 3; // 3 = SProductsRatingPeer::NUM_HYDRATE_COLUMNS.
		} catch ( Exception $e ) {
	...
	public function delete(PropelPDO $con = null) {
		...
		try {
			$deleteQuery = SProductsRatingQuery::create ()->filterByPrimaryKey ( $this->getPrimaryKey () );
	...
	protected function doInsert(PropelPDO $con) {
		...
		if ($this->isColumnModified ( SProductsRatingPeer::PRODUCT_ID )) {
			$modifiedColumns [':p' . $index ++] = '`PRODUCT_ID`';
		}
		if ($this->isColumnModified ( SProductsRatingPeer::VOTES )) {
			$modifiedColumns [':p' . $index ++] = '`VOTES`';
		}
		if ($this->isColumnModified ( SProductsRatingPeer::RATING )) {
			$modifiedColumns [':p' . $index ++] = '`RATING`';
		}
		
		$sql = sprintf ( 'INSERT INTO `shop_products_rating` (%s) VALUES (%s)', implode ( ', ', $modifiedColumns ), implode ( ', ', array_keys ( $modifiedColumns ) ) );
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false) {
		if (isset ( $alreadyDumpedObjects ['SProductsRating'] [$this->getPrimaryKey ()] )) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects ['SProductsRating'] [$this->getPrimaryKey ()] = true;
	...
	public function buildCriteria() {
		...
		if ($this->isColumnModified ( SProductsRatingPeer::VOTES ))
			$criteria->add ( SProductsRatingPeer::VOTES, $this->votes );
		if ($this->isColumnModified ( SProductsRatingPeer::RATING ))
			$criteria->add ( SProductsRatingPeer::RATING, $this->rating );
	 ...
	 *        	An object of SProductsRating (or compatible) type.
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true) {
		...
		if ($deepCopy && ! $this->startCopy) {
			...
			$relObj = $this->getSProducts ();
			if ($relObj) {
				$copyObj->setSProducts ( $relObj->copy ( $deepCopy ) );
			}
