...
abstract class BaseSProductVariantsI18nQuery extends ModelCriteria
{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = SProductVariantsI18nPeer::getInstanceFromPool(serialize(array((string) $key[0], (string) $key[1]))))) && !$this->formatter) {
	...
	protected function findPkSimple($key, $con)
	{
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			$obj->hydrate($row);
			SProductVariantsI18nPeer::addInstanceToPool($obj, serialize(array((string) $key[0], (string) $key[1])));
		}
		$stmt->closeCursor();
	...
	public function filterBySProductVariants($sProductVariants, $comparison = null)
	{
		if ($sProductVariants instanceof SProductVariants) {
			return $this
				->addUsingAlias(SProductVariantsI18nPeer::ID, $sProductVariants->getId(), $comparison);
		} elseif ($sProductVariants instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this
				->addUsingAlias(SProductVariantsI18nPeer::ID, $sProductVariants->toKeyValue('PrimaryKey', 'Id'), $comparison);
		} else {
	 ...
	 *
	 * @param     SProductVariantsI18n $sProductVariantsI18n Object to remove from the list of results
	 *
	 * @return    SProductVariantsI18nQuery The current query, for fluid interface
	 */
