...
abstract class BaseSProducts extends ShopBaseObject  implements Persistent
{
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = SProductsQuery::create()
			...
			if ($this->sProductsRatingsScheduledForDeletion !== null) {
				if (!$this->sProductsRatingsScheduledForDeletion->isEmpty()) {
					SProductsRatingQuery::create()
						->filterByPrimaryKeys($this->sProductsRatingsScheduledForDeletion->getPrimaryKeys(false))
						->delete($con);
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[5], $arr)) $this->setHit($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setHot($arr[$keys[6]]);
		...
		if (array_key_exists($keys[12], $arr)) $this->setMainmodimage($arr[$keys[12]]);
		if (array_key_exists($keys[13], $arr)) $this->setSmallimage($arr[$keys[13]]);
		if (array_key_exists($keys[14], $arr)) $this->setSmallmodimage($arr[$keys[14]]);
		if (array_key_exists($keys[15], $arr)) $this->setOldPrice($arr[$keys[15]]);
		if (array_key_exists($keys[16], $arr)) $this->setCreated($arr[$keys[16]]);
		if (array_key_exists($keys[17], $arr)) $this->setUpdated($arr[$keys[17]]);
		if (array_key_exists($keys[18], $arr)) $this->setViews($arr[$keys[18]]);
		if (array_key_exists($keys[19], $arr)) $this->setAddedToCartCount($arr[$keys[19]]);
		if (array_key_exists($keys[20], $arr)) $this->setEnableComments($arr[$keys[20]]);
		if (array_key_exists($keys[21], $arr)) $this->setTpl($arr[$keys[21]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SProductsPeer::USER_ID)) $criteria->add(SProductsPeer::USER_ID, $this->user_id);
		if ($this->isColumnModified(SProductsPeer::EXTERNAL_ID)) $criteria->add(SProductsPeer::EXTERNAL_ID, $this->external_id);
		...
		if ($this->isColumnModified(SProductsPeer::MAINMODIMAGE)) $criteria->add(SProductsPeer::MAINMODIMAGE, $this->mainmodimage);
		if ($this->isColumnModified(SProductsPeer::SMALLIMAGE)) $criteria->add(SProductsPeer::SMALLIMAGE, $this->smallimage);
		if ($this->isColumnModified(SProductsPeer::SMALLMODIMAGE)) $criteria->add(SProductsPeer::SMALLMODIMAGE, $this->smallmodimage);
		...
		if ($this->isColumnModified(SProductsPeer::ADDED_TO_CART_COUNT)) $criteria->add(SProductsPeer::ADDED_TO_CART_COUNT, $this->added_to_cart_count);
		if ($this->isColumnModified(SProductsPeer::ENABLE_COMMENTS)) $criteria->add(SProductsPeer::ENABLE_COMMENTS, $this->enable_comments);
		if ($this->isColumnModified(SProductsPeer::TPL)) $criteria->add(SProductsPeer::TPL, $this->tpl);
	 ...
	 *
	 * @param      PropelCollection $shopKits A Propel collection.
	 * @param      PropelPDO $con Optional connection object
	 */
	...
	/**
	 * @param	ShopKit $shopKit The shopKit object to add.
	 */
	...
	public function getSProductsI18ns($criteria = null, PropelPDO $con = null)
	{
		if(null === $this->collSProductsI18ns || null !== $criteria) {
			if ($this->isNew() && null === $this->collSProductsI18ns) {
				...
				$collSProductsI18ns = SProductsI18nQuery::create(null, $criteria)
					->filterBySProducts($this)
					->find($con);
				if (null !== $criteria) {
	 ...
	 * and new objects from the given Propel collection.
	 *
	 * @param      PropelCollection $sProductsI18ns A Propel collection.
	 * @param      PropelPDO $con Optional connection object
	 */
	 ...
	 *
	 * @param      SProductsI18n $l SProductsI18n
	 * @return     SProducts The current object (for fluent API support)
	 */
	...
	/**
	 * @param	SProductsI18n $sProductsI18n The sProductsI18n object to add.
	 */
	 ...
	 * and new objects from the given Propel collection.
	 *
	 * @param      PropelCollection $sProductImagess A Propel collection.
	 * @param      PropelPDO $con Optional connection object
	 */
	...
	/**
	 * @param	SProductImages $sProductImages The sProductImages object to add.
	 */
	 ...
	 * and new objects from the given Propel collection.
	 *
	 * @param      PropelCollection $productVariants A Propel collection.
	 * @param      PropelPDO $con Optional connection object
	 */
	...
	/**
	 * @param	ProductVariant $productVariant The productVariant object to add.
	 */
	 ...
	 * and new objects from the given Propel collection.
	 *
	 * @param      PropelCollection $sWarehouseDatas A Propel collection.
	 * @param      PropelPDO $con Optional connection object
	 */
	...
	/**
	 * @param	SWarehouseData $sWarehouseData The sWarehouseData object to add.
	 */
	...
	public function getTranslation($locale = 'ru', PropelPDO $con = null)
	{
		if (!isset($this->currentTranslations[$locale])) {
			...
			if ($this->isNew()) {
				...
				$translation = SProductsI18nQuery::create()
