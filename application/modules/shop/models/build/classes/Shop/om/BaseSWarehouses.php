...
abstract class BaseSWarehouses extends ShopBaseObject implements Persistent {
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false) {
		try {
			...
			return $startcol + 5; // 5 = SWarehousesPeer::NUM_HYDRATE_COLUMNS.
		} catch ( Exception $e ) {
	...
	public function delete(PropelPDO $con = null) {
		...
		try {
			$deleteQuery = SWarehousesQuery::create ()->filterByPrimaryKey ( $this->getPrimaryKey () );
	...
	protected function doInsert(PropelPDO $con) {
		...
		if ($this->isColumnModified ( SWarehousesPeer::ID )) {
			$modifiedColumns [':p' . $index ++] = '`ID`';
		}
		if ($this->isColumnModified ( SWarehousesPeer::NAME )) {
			$modifiedColumns [':p' . $index ++] = '`NAME`';
		}
		if ($this->isColumnModified ( SWarehousesPeer::ADDRESS )) {
			$modifiedColumns [':p' . $index ++] = '`ADDRESS`';
		}
		if ($this->isColumnModified ( SWarehousesPeer::PHONE )) {
			$modifiedColumns [':p' . $index ++] = '`PHONE`';
		}
		if ($this->isColumnModified ( SWarehousesPeer::DESCRIPTION )) {
			$modifiedColumns [':p' . $index ++] = '`DESCRIPTION`';
		}
		
		$sql = sprintf ( 'INSERT INTO `shop_warehouse` (%s) VALUES (%s)', implode ( ', ', $modifiedColumns ), implode ( ', ', array_keys ( $modifiedColumns ) ) );
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false) {
		if (isset ( $alreadyDumpedObjects ['SWarehouses'] [$this->getPrimaryKey ()] )) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects ['SWarehouses'] [$this->getPrimaryKey ()] = true;
	...
	public function buildCriteria() {
		...
		if ($this->isColumnModified ( SWarehousesPeer::NAME ))
			$criteria->add ( SWarehousesPeer::NAME, $this->name );
		if ($this->isColumnModified ( SWarehousesPeer::ADDRESS ))
			$criteria->add ( SWarehousesPeer::ADDRESS, $this->address );
		if ($this->isColumnModified ( SWarehousesPeer::PHONE ))
			$criteria->add ( SWarehousesPeer::PHONE, $this->phone );
		if ($this->isColumnModified ( SWarehousesPeer::DESCRIPTION ))
			$criteria->add ( SWarehousesPeer::DESCRIPTION, $this->description );
	 ...
	 *        	An object of SWarehouses (or compatible) type.
	...
	public function getSWarehouseDatas($criteria = null, PropelPDO $con = null) {
		if (null === $this->collSWarehouseDatas || null !== $criteria) {
			if ($this->isNew () && null === $this->collSWarehouseDatas) {
				...
				$collSWarehouseDatas = SWarehouseDataQuery::create ( null, $criteria )->filterBySWarehouses ( $this )->find ( $con );
				if (null !== $criteria) {
	...
	public function countSWarehouseDatas(Criteria $criteria = null, $distinct = false, PropelPDO $con = null) {
		if (null === $this->collSWarehouseDatas || null !== $criteria) {
			if ($this->isNew () && null === $this->collSWarehouseDatas) {
				...
				return $query->filterBySWarehouses ( $this )->count ( $con );
			}
		} else {
