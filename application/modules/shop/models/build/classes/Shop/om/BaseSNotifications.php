...
abstract class BaseSNotifications extends ShopBaseObject  implements Persistent
{
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = SNotificationsQuery::create()
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[6], $arr)) $this->setUserComment($arr[$keys[6]]);
		if (array_key_exists($keys[7], $arr)) $this->setStatus($arr[$keys[7]]);
		if (array_key_exists($keys[8], $arr)) $this->setDateCreated($arr[$keys[8]]);
		if (array_key_exists($keys[9], $arr)) $this->setActiveTo($arr[$keys[9]]);
		if (array_key_exists($keys[10], $arr)) $this->setManagerId($arr[$keys[10]]);
		if (array_key_exists($keys[11], $arr)) $this->setNotifiedByEmail($arr[$keys[11]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SNotificationsPeer::ID)) $criteria->add(SNotificationsPeer::ID, $this->id);
		if ($this->isColumnModified(SNotificationsPeer::PRODUCT_ID)) $criteria->add(SNotificationsPeer::PRODUCT_ID, $this->product_id);
		if ($this->isColumnModified(SNotificationsPeer::VARIANT_ID)) $criteria->add(SNotificationsPeer::VARIANT_ID, $this->variant_id);
		if ($this->isColumnModified(SNotificationsPeer::USER_NAME)) $criteria->add(SNotificationsPeer::USER_NAME, $this->user_name);
		if ($this->isColumnModified(SNotificationsPeer::USER_EMAIL)) $criteria->add(SNotificationsPeer::USER_EMAIL, $this->user_email);
		if ($this->isColumnModified(SNotificationsPeer::USER_PHONE)) $criteria->add(SNotificationsPeer::USER_PHONE, $this->user_phone);
		if ($this->isColumnModified(SNotificationsPeer::USER_COMMENT)) $criteria->add(SNotificationsPeer::USER_COMMENT, $this->user_comment);
		if ($this->isColumnModified(SNotificationsPeer::STATUS)) $criteria->add(SNotificationsPeer::STATUS, $this->status);
		if ($this->isColumnModified(SNotificationsPeer::DATE_CREATED)) $criteria->add(SNotificationsPeer::DATE_CREATED, $this->date_created);
		if ($this->isColumnModified(SNotificationsPeer::ACTIVE_TO)) $criteria->add(SNotificationsPeer::ACTIVE_TO, $this->active_to);
		if ($this->isColumnModified(SNotificationsPeer::MANAGER_ID)) $criteria->add(SNotificationsPeer::MANAGER_ID, $this->manager_id);
		if ($this->isColumnModified(SNotificationsPeer::NOTIFIED_BY_EMAIL)) $criteria->add(SNotificationsPeer::NOTIFIED_BY_EMAIL, $this->notified_by_email);
