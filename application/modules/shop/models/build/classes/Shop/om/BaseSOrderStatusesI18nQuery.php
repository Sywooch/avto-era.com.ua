...
abstract class BaseSOrderStatusesI18nQuery extends ModelCriteria {
	...
	public function findPk($key, $con = null) {
		...
		if ((null !== ($obj = SOrderStatusesI18nPeer::getInstanceFromPool ( serialize ( array (
				( string ) $key [0],
				( string ) $key [1] 
		) ) ))) && ! $this->formatter) {
	...
	protected function findPkSimple($key, $con) {
		...
		if ($row = $stmt->fetch ( PDO::FETCH_NUM )) {
			...
			$obj->hydrate ( $row );
			SOrderStatusesI18nPeer::addInstanceToPool ( $obj, serialize ( array (
					( string ) $key [0],
					( string ) $key [1] 
			) ) );
		}
	...
	public function filterBySOrderStatuses($sOrderStatuses, $comparison = null) {
		if ($sOrderStatuses instanceof SOrderStatuses) {
			return $this->addUsingAlias ( SOrderStatusesI18nPeer::ID, $sOrderStatuses->getId (), $comparison );
		} elseif ($sOrderStatuses instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this->addUsingAlias ( SOrderStatusesI18nPeer::ID, $sOrderStatuses->toKeyValue ( 'PrimaryKey', 'Id' ), $comparison );
		} else {
