...
abstract class BaseSProductsI18n extends ShopBaseObject implements Persistent {
	...
	public function delete(PropelPDO $con = null) {
		...
		try {
			$deleteQuery = SProductsI18nQuery::create ()->filterByPrimaryKey ( $this->getPrimaryKey () );
	...
	protected function doInsert(PropelPDO $con) {
		...
		$sql = sprintf ( 'INSERT INTO `shop_products_i18n` (%s) VALUES (%s)', implode ( ', ', $modifiedColumns ), implode ( ', ', array_keys ( $modifiedColumns ) ) );
	...
	public function buildCriteria() {
		...
		if ($this->isColumnModified ( SProductsI18nPeer::NAME ))
			$criteria->add ( SProductsI18nPeer::NAME, $this->name );
		if ($this->isColumnModified ( SProductsI18nPeer::SHORT_DESCRIPTION ))
			$criteria->add ( SProductsI18nPeer::SHORT_DESCRIPTION, $this->short_description );
		if ($this->isColumnModified ( SProductsI18nPeer::FULL_DESCRIPTION ))
			$criteria->add ( SProductsI18nPeer::FULL_DESCRIPTION, $this->full_description );
		if ($this->isColumnModified ( SProductsI18nPeer::META_TITLE ))
			$criteria->add ( SProductsI18nPeer::META_TITLE, $this->meta_title );
		if ($this->isColumnModified ( SProductsI18nPeer::META_DESCRIPTION ))
			$criteria->add ( SProductsI18nPeer::META_DESCRIPTION, $this->meta_description );
		if ($this->isColumnModified ( SProductsI18nPeer::META_KEYWORDS ))
			$criteria->add ( SProductsI18nPeer::META_KEYWORDS, $this->meta_keywords );
	 ...
	 *        	An object of SProductsI18n (or compatible) type.
