...
abstract class BaseSOrderProducts extends ShopBaseObject  implements Persistent
{
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = SOrderProductsQuery::create()
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[2], $arr)) $this->setKitId($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setIsMain($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setProductId($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setVariantId($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setProductName($arr[$keys[6]]);
		if (array_key_exists($keys[7], $arr)) $this->setVariantName($arr[$keys[7]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SOrderProductsPeer::KIT_ID)) $criteria->add(SOrderProductsPeer::KIT_ID, $this->kit_id);
		if ($this->isColumnModified(SOrderProductsPeer::IS_MAIN)) $criteria->add(SOrderProductsPeer::IS_MAIN, $this->is_main);
