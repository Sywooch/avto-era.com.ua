...
abstract class BaseSCurrenciesQuery extends ModelCriteria {
	...
	public function filterByMain($main = null, $comparison = null) {
		if (is_string ( $main )) {
			$main = in_array ( strtolower ( $main ), array (
					'false',
					'off',
					'-',
					'no',
	...
	public function filterByCurrency($sProductVariants, $comparison = null) {
		if ($sProductVariants instanceof SProductVariants) {
			return $this->addUsingAlias ( SCurrenciesPeer::ID, $sProductVariants->getCurrency (), $comparison );
		} elseif ($sProductVariants instanceof PropelCollection) {
			return $this->useCurrencyQuery ()->filterByPrimaryKeys ( $sProductVariants->getPrimaryKeys () )->endUse ();
		} else {
	...
	public function useCurrencyQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN) {
		return $this->joinCurrency ( $relationAlias, $joinType )->useQuery ( $relationAlias ? $relationAlias : 'Currency', 'SProductVariantsQuery' );
	}
	...
	public function filterBySPaymentMethods($sPaymentMethods, $comparison = null) {
		if ($sPaymentMethods instanceof SPaymentMethods) {
			return $this->addUsingAlias ( SCurrenciesPeer::ID, $sPaymentMethods->getCurrencyId (), $comparison );
		} elseif ($sPaymentMethods instanceof PropelCollection) {
			return $this->useSPaymentMethodsQuery ()->filterByPrimaryKeys ( $sPaymentMethods->getPrimaryKeys () )->endUse ();
		} else {
