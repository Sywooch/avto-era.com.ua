...
abstract class BaseSProductPropertiesDataQuery extends ModelCriteria {
	...
	public function filterBySProperties($sProperties, $comparison = null) {
		if ($sProperties instanceof SProperties) {
			return $this->addUsingAlias ( SProductPropertiesDataPeer::PROPERTY_ID, $sProperties->getId (), $comparison );
		} elseif ($sProperties instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this->addUsingAlias ( SProductPropertiesDataPeer::PROPERTY_ID, $sProperties->toKeyValue ( 'PrimaryKey', 'Id' ), $comparison );
		} else {
	...
	public function useSPropertiesQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN) {
		return $this->joinSProperties ( $relationAlias, $joinType )->useQuery ( $relationAlias ? $relationAlias : 'SProperties', 'SPropertiesQuery' );
	}
	...
	public function filterByProduct($sProducts, $comparison = null) {
		if ($sProducts instanceof SProducts) {
			return $this->addUsingAlias ( SProductPropertiesDataPeer::PRODUCT_ID, $sProducts->getId (), $comparison );
		} elseif ($sProducts instanceof PropelCollection) {
			if (null === $comparison) {
				$comparison = Criteria::IN;
			}
			return $this->addUsingAlias ( SProductPropertiesDataPeer::PRODUCT_ID, $sProducts->toKeyValue ( 'PrimaryKey', 'Id' ), $comparison );
		} else {
