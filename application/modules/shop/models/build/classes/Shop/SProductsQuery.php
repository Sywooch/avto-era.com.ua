...
// 		);
		...
		$need = array (
				...
				'shop_product_properties_data__value1',
				'shop_product_properties_data__value2',
	...
	public function mostViewed($limit = 6) {
		$this->orderByViews ( Criteria::DESC )->filterByActive ( true )->limit ( $limit );
		return $this;
	}
	public function getSimilarProducts(SProducts $model) {
		$properties = SProductPropertiesDataQuery::create ()->findByProductId ( $model->getId () );
		
		foreach ( $properties as $property )
			$data [] [$property->getPropertyId ()] = array (
					$property->getValue () 
			);
		
		shuffle ( $data );
		return $this->leftJoin ( 'ProductVariant' )->where ( 'SProducts.Id != ?', $model->getId () )->filterByActive ( true )->combinator ( $data [0] );
	}
	...
	public function globalSort($order_method) {
		switch ($order_method) {
			...
			case 'rating' :
				$this->useSProductsRatingQuery ( 'TRating', 'left join' )->withColumn ( "TRating.Rating / TRating.Votes", 'Midrate' )->endUse ()->orderBy ( 'Midrate', 'Desc' )->orderBy ( 'TRating.Votes', 'Desc' );
				break;
