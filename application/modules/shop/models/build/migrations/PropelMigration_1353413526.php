 ...
 * up to version 1353413526.
 * Generated on 2012-11-20 14:12:06 by koloda
 */
class PropelMigration_1353413526 {
...
ALTER TABLE `shop_product_variants` CHANGE `price_in_main` `price_in_main` FLOAT (10,5) NOT NULL;
...
DROP INDEX `banned` ON `users`;

DROP INDEX `password` ON `users`;

ALTER TABLE `users` CHANGE `role_id` `role_id` INTEGER;
...
ALTER TABLE `users` CHANGE `banned` `banned` TINYINT(1);

ALTER TABLE `users` CHANGE `newpass_time` `newpass_time` INTEGER;

ALTER TABLE `users` CHANGE `last_ip` `last_ip` VARCHAR(40);

ALTER TABLE `users` CHANGE `last_login` `last_login` INTEGER;

ALTER TABLE `users` CHANGE `created` `created` INTEGER;

ALTER TABLE `users` CHANGE `modified` `modified` DATETIME;
...
CREATE INDEX `users_I_1` ON `users` (`key`);

ALTER TABLE `users` ADD CONSTRAINT `users_FK_1`
	FOREIGN KEY (`role_id`)
	REFERENCES `shop_rbac_roles` (`id`)
	ON UPDATE CASCADE
	ON DELETE SET NULL;
...
ALTER TABLE `users` CHANGE `role_id` `role_id` INTEGER DEFAULT 1 NOT NULL;
...
ALTER TABLE `users` CHANGE `banned` `banned` TINYINT(1) DEFAULT 0 NOT NULL;

ALTER TABLE `users` CHANGE `newpass_time` `newpass_time` DATETIME;

ALTER TABLE `users` CHANGE `last_ip` `last_ip` VARCHAR(40) NOT NULL;

ALTER TABLE `users` CHANGE `last_login` `last_login` DATETIME DEFAULT \'0000-00-00 00:00:00\' NOT NULL;

ALTER TABLE `users` CHANGE `created` `created` DATETIME DEFAULT \'0000-00-00 00:00:00\' NOT NULL;

ALTER TABLE `users` CHANGE `modified` `modified` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP;

ALTER TABLE `users` DROP `address`;

ALTER TABLE `users` DROP `cart_data`;

ALTER TABLE `users` DROP `wish_list_data`;

ALTER TABLE `users` DROP `key`;

ALTER TABLE `users` DROP `amout`;

ALTER TABLE `users` DROP `discount`;

CREATE UNIQUE INDEX `username` ON `users` (`username`);

CREATE UNIQUE INDEX `email` ON `users` (`email`);

CREATE INDEX `banned` ON `users` (`banned`);
