...
class Ajax extends ShopController {
	...
	public function getCategoryAttributes($catId) {
		$model = SCategoryQuery::create ()->filterById ( ( int ) $catId )->filterByActive ( TRUE )->findOne ();
		if (! $model)
			return false;
	...
	public function getNotifyingRequest() {
		...
		if ($_POST ['notifme']) {
			$this->form_validation->set_rules ( 'UserName', ShopCore::t ( 'Ім’я' ), 'trim|xss_clean|required|max_length[50]' );
			$this->form_validation->set_rules ( 'UserEmail', ShopCore::t ( 'Email' ), 'valid_email|required|max_length[50]' );
			// $this->form_validation->set_rules('UserPhone', ShopCore::t('Мобільний телефон'), 'trim|xss_clean|required|max_length[50]');
			$this->form_validation->set_rules ( 'UserComment', ShopCore::t ( 'Додаткова інформація' ), 'trim|xss_clean|max_length[500]' );
	...
	public function getApiNotifyingRequest() {
		...
		if ($_POST ['notifme']) {
			$this->form_validation->set_rules ( 'UserName', ShopCore::t ( 'Ім’я' ), 'trim|xss_clean|required|max_length[50]' );
			$this->form_validation->set_rules ( 'UserEmail', ShopCore::t ( 'Email' ), 'valid_email|required|max_length[50]' );
			$this->form_validation->set_rules ( 'UserPhone', ShopCore::t ( 'Телефон' ), 'integer|trim|xss_clean|max_length[50]' );
			if ($this->form_validation->run ( $this ) != FALSE) {
				$notification = new SNotifications ();
				$notification->fromArray ( $_POST );
				$notification->setStatus ( SNotificationStatusesQuery::create ()->orderById ()->findOne ()->getId () );
