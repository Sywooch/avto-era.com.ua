...
class Shop_widgets extends ShopController {
	...
	private $viewProductDefaults = array (
			'subpath' => 'widgets' 
	);
	...
	public function showLastProducts() {
		...
		if (count ( $pageId ) >= 1) {
			 ...
			 */
			$model = SProductsQuery::create ()->joinWithI18n ( MY_Controller::getCurrentLocale () )->filterById ( $pageId )->limit ( 20 )->addAsColumn ( "newOrder", 'CASE
                        ...
                        ELSE 100
                        END' )->addAscendingOrderByColumn ( 'newOrder' )->filterByActive ( true )->find ();
			/**
	...
	public function view_product_configure($mode = 'install_defaults', $data = array()) {
		...
		switch ($mode) {
			...
			case 'show_settings' :
				$this->render ( 'view_product_form', array (
						'widget' => $data 
				) );
				break;
	...
	public function similar_products_configure($mode = 'install_defaults', $data = array()) {
		...
		switch ($mode) {
			...
			case 'show_settings' :
				$this->render ( 'similar_products_form', array (
						'widget' => $data 
				) );
				break;
