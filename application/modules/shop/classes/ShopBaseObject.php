...
class ShopBaseObject extends BaseObject {
				...
				foreach ( $_POST ['custom_field'] as $key_post => $value_post ) {
					foreach ( $this->customFields as $key => $value ) {
						if (( int ) $key_post == $value ['Id']) {
							...
							if ($validator_str)
								$validator->set_rules ( "custom_field[$key]", $value ['fLabel'], $validator_str );
						}
					}
				}
		}
			...
			foreach ( $_POST ['custom_field'] as $key => $value ) {
				...
				foreach ( $this->customFields as $fieldObject ) {
					if (( int ) $key == $fieldObject ['Id']) {
						...
						$objCustomData = CustomFieldsDataQuery::create ()->filterByentityId ( $this->entityId )->filterByfieldId ( $key )->filterByLocale ( $locale )->findone ();
						if ($objCustomData) {
	...
	/**
	 * Convert model attribute(by default Price).
	 * e.g. "99.99 $"
	 *
	 * @param string $attributeName
	 *        	Optional. Attribute name to convert.
	 * @access public
	 * @return string
	 */
	public function toCurrency($attributeName = 'Price', $cId = null) {
		$attributeName = strtolower ( $attributeName );
		
		$get = 'get' . $attributeName;
		if ($attributeName == 'origprice') {
			return ShopCore::app ()->SCurrencyHelper->convert ( $this->getVirtualColumn ( 'origprice' ), $cId );
		}
		return ShopCore::app ()->SCurrencyHelper->convert ( $this->$get (), $cId );
	}
