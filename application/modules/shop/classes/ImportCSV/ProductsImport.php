...
class ProductsImport extends BaseImport {
	...
	public function __construct() {
		...
		$this->mainCur = $this->db->get_where ( 'shop_currencies', array (
				'is_default' => '1'
		) )->row_array ();
	...
	private function startCoreProcess() {
		foreach ( BaseImport::create ()->content as $key => $node ) {
			...
			$result = $this->db->limit ( 1 )->select ( 'shop_product_variants.product_id as ProductId' )->select ( 'shop_products.category_id as SCategoryId' )->select ( 'shop_products_i18n.name as ProductName' )->join ( 'shop_products', 'shop_products.id = shop_product_variants.product_id', 'left outer' )->join ( 'shop_products_i18n', 'shop_products_i18n.id = shop_products.id' )->where ( 'number', $node ['num'] )->get ( 'shop_product_variants' )->row ();
				
			$mas [$key] = (! ($result instanceof \stdClass)) ? $this->runProductInsertQuery ( $node ) : $this->runProductUpdateQuery ( $result->ProductId, $node );
	...
	public function runProductUpdateQuery($productId, $arg) {
		...
		$prepareNames = array_merge ( $prepareNames, array (
				'updated'
		) );
		...
		$this->db->query ( 'UPDATE shop_products SET ' . implode ( ',', $updateData ) . ' WHERE `id`= ?', array (
				$productId
		) );
	...
	private function runProductVariantUpdateQuery(&$arg, &$productId) {
		...
		$cur = $this->db->select ( 'id' )->get_where ( 'shop_currencies', array (
				'code' => $arg ['cur']
		) )->row ()->id;
		...
		$this->db->query ( 'UPDATE shop_product_variants SET ' . implode ( ',', $updateData ) . ' WHERE `number`= ? AND `product_id` = ?', array (
				$arg ['num'],
				$productId
		) );

		$variantModel = $this->db->query ( 'SELECT id FROM shop_product_variants WHERE `number` = ? AND `product_id` = ?', array (
				$arg ['num'],
				$productId
		) )->row ();
		...
		$prepareNames = $binds = $updateData = array ();
		$productAlias = (isset ( $arg ['var'] )) ? array (
				'var' => 'name'
		) : array (
				'name' => 'name'
		);
		...
		$this->db->query ( 'UPDATE shop_product_variants_i18n SET ' . implode ( ',', $updateData ) . ' WHERE `locale`= ? AND `id` = ?', array (
				$this->languages,
				$variantModel->id
		) );
	...
	private function runProductInsertQuery($arg) {
		...
		$result = $this->db->where ( 'name', $arg ['name'] )->get ( 'shop_products_i18n' )->row ();
		...
		$this->db->query ( 'UPDATE shop_products SET `url`= ? WHERE `id`= ?', array (
				$this->urlCheck ( $arg [url], $productId, $arg ['name'] ),
				$productId
		) );
	...
	private function updateSProductsCategories(&$arg, $productId) {
		$this->db->delete ( 'shop_product_categories', array (
				'product_id' => $productId
		) );
			...
			foreach ( $result as $key => $item ) {
				...
				foreach ( $item as $k => $v ) {
					if ($v != '')
						...
						switch ($imageNames [$k]) {
							case 'main' :
								$imagename = $pId ['product_id'] . '_main.jpg';
								break;
							case 'small' :
								$imagename = $pId ['product_id'] . '_small.jpg';
								break;
							case 'mainMod' :
								$imagename = $pId ['product_id'] . '_mainMod.jpg';
								break;
						}
	...
	function urlCheck($url, $id = '', $name = '') {
		...
		$urlCheck = $this->db->select ( 'url,id' )->where ( 'url ', $url )->where ( '`id` !=' . $id )->get ( 'shop_products' )->row ();
	...
	function runAditionalImages($arg, $id) {
		$this->db->delete ( 'shop_product_images', array (
				'product_id' => $id
		) );
