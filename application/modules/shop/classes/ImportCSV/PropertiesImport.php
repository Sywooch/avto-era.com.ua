		...
		foreach ( $this->content as $key => $node ) {
			foreach ( $node as $nodeKey => $nodeElement ) {
				if (array_key_exists ( $nodeKey, $properyAlias ) && ! empty ( $nodeElement )) {
					$result = $this->db->query ( 'SELECT * FROM `shop_product_properties_data` WHERE `product_id` = ? AND `property_id` = ?', array (
							$node ['ProductId'],
							$properyAlias [$nodeKey]
					) )->row ();
					if (! ($result instanceof \stdClass))
						$this->db->insert ( 'shop_product_properties_data', array (
						...
						$this->db->update ( 'shop_product_properties_data', array (
								'value' => $nodeElement
						), array (
								'product_id' => $node ['ProductId'],
								'property_id' => $properyAlias [$nodeKey],
					...
					foreach ( $node ['CategoryIds'] as $categoryId ) {
						$result = $this->db->query ( 'SELECT * FROM `shop_product_properties_categories` WHERE `category_id` = ? AND `property_id` = ?', array (
								$categoryId,
								$properyAlias [$nodeKey]
						) )->row ();
						if (! ($result instanceof \stdClass))
							$this->db->insert ( 'shop_product_properties_categories', array (
									'property_id' => $properyAlias [$nodeKey],
					...
					if (! in_array ( $nodeElement, $data )) {
						$data [] = $nodeElement;
						$this->db->update ( 'shop_product_properties_i18n', array (
								'data' => serialize ( $data )
						), array (
								'id' => $properyAlias [$nodeKey],
								'locale' => $this->languages
						) );
					}
				}
