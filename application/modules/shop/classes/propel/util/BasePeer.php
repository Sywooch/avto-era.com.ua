...
class BasePeer {
	...
	/**
	 * num type
	 * simply the numerical array index, e.g.
	 * 4
	 */
	const TYPE_NUM = 'num';
	 ...
	 *        	The criteria to use.
	 * @param PropelPDO $con
	 *        	A PropelPDO connection object.
	 * @return int number of rows affected by last statement execution. For most
	 ...
	 * @return int number of rows affected by the statement. Note
	 ...
	 *
	 * @param $selectCriteria A
	 *        	Criteria object containing values used in where
	 *        	clause.
	 * @param $updateValues A
	 *        	Criteria object containing values used in set
	 *        	clause.
	 * @param PropelPDO $con
	 *        	The PropelPDO connection object to use.
	 * @return int number of rows affected by last update statement. For most
	...
	public static function doUpdate(Criteria $selectCriteria, Criteria $updateValues, PropelPDO $con) {
		...
		if (empty ( $tablesColumns )) {
			$tablesColumns = array (
					$selectCriteria->getPrimaryTableName () => array () 
			);
		}
	 ...
	 * @param array $columns
	 *        	Array of column names as key and column values as value.
	 */
	 ...
	 * in order to avoid doing the tests when it's not necessary (e.g.
	 * for SELECTs)
	 */
	...
	public static function createSelectSql(Criteria $criteria, &$params) {
		...
		if ($db->useQuoteIdentifier ()) {
			$fromClause = array_map ( array (
					$db,
					'quoteIdentifierTable' 
			), $fromClause );
			$joinClause = $joinClause ? $joinClause : array_map ( array (
					$db,
					'quoteIdentifierTable' 
			), $joinClause );
		}
	 ...
	 *        	The dot-path name of class (e.g. myapp.propel.MyValidator)
