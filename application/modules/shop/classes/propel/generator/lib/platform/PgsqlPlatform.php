...
";
		$ret .= sprintf ( $pattern, $this->quoteIdentifier ( $table->getName () ), implode ( $sep, $lines ) );
		
		if ($table->hasDescription ()) {
			$pattern = "
COMMENT ON TABLE %s IS %s;
";
			$ret .= sprintf ( $pattern, $this->quoteIdentifier ( $table->getName () ), $this->quote ( $table->getDescription () ) );
		}
...
%s = \$row[0];";
		$script = sprintf ( $snippet, $connectionVariableName, $this->quoteIdentifier ( $sequenceName ), $columnValueMutator );
