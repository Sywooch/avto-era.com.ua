...
class DefaultPlatform implements PropelPlatformInterface {
	 ...
	 * 
	 * @param PDO $con
	 *        	Optional database connection to use in this platform.
	 */
	 ...
	 * 
	 * @param PDO $con
	 *        	Database connection to use in this platform.
	 */
...
";
		return sprintf ( $pattern, $this->quoteIdentifier ( $table->getName () ), $this->quoteIdentifier ( $this->getPrimaryKeyName ( $table ) ) );
	}
...
";
		return sprintf ( $pattern, $this->quoteIdentifier ( $fromTableName ), $this->quoteIdentifier ( $toTableName ) );
	}
...
";
	...
	public function quoteIdentifier($text) {
		return $this->isIdentifierQuotingEnabled ? '"' . strtr ( $text, array (
				'.' => '"."' 
		) ) . '"' : $text;
	}
	 ...
	 *
	 * @param mixed $tf
	 *        	A boolean or string representation of boolean ('y', 'true').
	 * @return mixed
	 */
...
\$stmt->bindValue(%s, %s, %s);", $identifier, $columnValueAccessor, PropelTypes::getPdoTypeString ( $column->getType () ) );
	...
	public function getIdentifierPhp($columnValueMutator, $connectionVariableName = '$con', $sequenceName = '', $tab = "			") {
		return sprintf ( "
%s%s = %s->lastInsertId(%s);", $tab, $columnValueMutator, $connectionVariableName, $sequenceName ? ("'" . $sequenceName . "'") : '' );
	}
}
