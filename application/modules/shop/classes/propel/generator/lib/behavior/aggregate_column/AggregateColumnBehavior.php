...
class AggregateColumnBehavior extends Behavior {
	...
	public function modifyTable() {
		...
		if (! $foreignTable->hasBehavior ( 'concrete_inheritance_parent' )) {
			...
			$relationBehavior->addParameter ( array (
					'name' => 'update_method',
					'value' => 'update' . $this->getColumn ()->getPhpName () 
			) );
	...
	protected function addObjectCompute() {
		...
		$sql = sprintf ( 'SELECT %s FROM %s WHERE %s', $this->getParameter ( 'expression' ), $database->getPlatform ()->quoteIdentifier ( $tableName ), implode ( ' AND ', $conditions ) );
