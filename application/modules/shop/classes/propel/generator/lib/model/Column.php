...
class Column extends XMLElement {
	...
	public static $valid_visibilities = array (
			'public',
			'protected',
			'private' 
	);
	 ...
	 *
	 * @param
	 *        	columns Either a list of <code>Column</code> objects, or
	...
	protected function setupObject() {
		try {
			...
			$this->isInheritance = ($this->inheritanceType !== null && $this->inheritanceType !== "false"); // here we are only checking for 'false', so don't
			                                        // use boleanValue()
	 ...
	 *
	 * @param
	 *        	ColumnDefaultValue|scalar column default value
	 */
	...
	public function getAutoIncrementString() {
		if ($this->isAutoIncrement () && IDMethod::NATIVE === $this->getTable ()->getIdMethod ()) {
			...
			throw new EngineException ( sprintf ( 'You have specified autoIncrement for column "%s", but you have not specified idMethod="native" for table "%s".', $this->name, $this->getTable ()->getName () ) );
		}
	 ...
	 * to the propel type of this column.
