...
abstract class XMLElement {
	 ...
	 * 
	 * @param string $name
	 *        	The [case-insensitive] name of the attribute to lookup.
	 * @param mixed $defaultValue
	 *        	The default value to use in case the attribute is not set.
	...
	protected function booleanValue($val) {
		if (is_numeric ( $val )) {
			...
			return (in_array ( strtolower ( $val ), array (
					'true',
					't',
					'y',
					'yes' 
	 ...
	 *
	 * @param mixed $data
	 *        	VendorInfo object or XML attrib data (array)
	 * @return VendorInfo
	 */
	 ...
	 *
	 * @param string $bname
	 *        	behavior name, e.g. 'timestampable'
	...
	public function getConfiguredBehavior($bname) {
		...
		if (class_exists ( $class = $gen->generateName ( array (
				$bname,
