...
class PropelMigrationManager {
	...
	public function getPlatform($datasource) {
		...
		require_once sprintf ( '%s/../platform/%s.php', dirname ( __FILE__ ), $adapterClass );
	...
	public function updateLatestMigrationTimestamp($datasource, $timestamp) {
		...
		$stmt = $pdo->prepare ( $sql );
		$stmt->execute ();
		$sql = sprintf ( 'INSERT INTO %s (%s) VALUES (?)', $this->getMigrationTable (), $platform->quoteIdentifier ( 'version' ) );
		$stmt = $pdo->prepare ( $sql );
	...
	public function getMigrationObject($timestamp) {
		...
		require_once sprintf ( '%s/%s.php', $this->getMigrationDir (), $className );
