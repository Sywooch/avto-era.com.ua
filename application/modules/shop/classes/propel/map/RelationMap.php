...
class RelationMap {
	const	// types
	MANY_TO_ONE = 1, ONE_TO_MANY = 2, ONE_TO_ONE = 3, MANY_TO_MANY = 4, 
	// representations
	 ...
	 *
	 * @param integer $type
	 *        	The relation type (either self::MANY_TO_ONE, self::ONE_TO_MANY, or self::ONE_TO_ONE)
	 */
	 ...
	 *        	The local table for this relationship
	 */
	 ...
	 *
	 * @param TableMap $table
	 *        	The foreign table for this relationship
	 */
	 ...
	 *
	 * @param ColumnMap $local
	 *        	The local column
	 * @param ColumnMap $foreign
	 *        	The foreign column
	 */
	 ...
	 *
	 * @param int $direction
	 *        	How the associative array must return columns
	...
	public function getSymmetricalRelation() {
		...
		foreach ( $this->getRightTable ()->getRelations () as $relation ) {
			if ($localMapping == array (
					$relation->getRightColumns (),
					$relation->getLeftColumns () 
			)) {
				return $relation;
			}
		}
