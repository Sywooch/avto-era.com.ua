...
class ModelCriteria extends Criteria {
	 ...
	 * @param string $modelAlias
	 *        	The model alias
	 * @param boolean $useAliasInSQL
	 *        	Whether to use the alias in the SQL code (false by default)
	 *        	
	 ...
	 * </code>
	 *
	 * @param string|PropelFormatter $formatter
	 *        	a formatter class name, or a formatter instance
	 * @return ModelCriteria The current object, for fluid interface
	 */
	 ...
	 * Defaults to an instance of ModelCriteria::$defaultFormatterClass, i.e.
	 ...
	 *        	A name to store the condition for a later combination with combine()
	 * @param string $clause
	 *        	The pseudo SQL clause, e.g. 'AuthorId = ?'
	 * @param mixed $value
	 ...
	 *
	 * @param string $column
	 *        	A string representing thecolumn phpName, e.g. 'AuthorId'
	 * @param mixed $value
	 *        	A value for the condition
	 * @param string $comparison
	 *        	What to use for the column comparison, defaults to Criteria::EQUAL
	 *        	
	 * @return ModelCriteria The current object, for fluid interface
	 */
	 ...
	 *        	An array of conditions, using column phpNames as key
	 *        	
	 * @return ModelCriteria The current object, for fluid interface
	 */
	 ...
	 *
	 * @param mixed $clause
	 *        	A string representing the pseudo SQL clause, e.g. 'Book.AuthorId = ?'
	...
	public function orWhere($clause, $value = null, $bindingType = null) {
		return $this->_or ()->where ( $clause, $value, $bindingType );
	}
	 ...
	 * @param string $order
	 *        	The sorting order. Criteria::ASC by default, also accepts Criteria::DESC
	 *        	
	 * @return ModelCriteria The current object, for fluid interface
	 */
	 ...
	 *
	 * @param int $limit
	 *        	Maximum number of results to return by the query
	 *        	
	 * @return ModelCriteria The current object, for fluid interface
	 */
	 ...
	 *        	A list of column names (e.g. array('Title', 'Category.Name', 'c.Content')) or a single column name (e.g. 'Name')
	 *        	
	 * @return ModelCriteria The current object, for fluid interface
	 */
	public function select($columnArray) {
		...
		if ($columnArray == '*') {
			...
			foreach ( call_user_func ( array (
					$this->modelPeerName,
					'getFieldNames' 
			), BasePeer::TYPE_PHPNAME ) as $column ) {
	...
	protected function configureSelectColumns() {
		...
		$columnNames = is_array ( $this->select ) ? $this->select : array (
				$this->select 
		);
	 ...
	 * by default this is null, but after useQuery this is set the to the join of that use
	 *
	 * @param Join $previousJoin
	 *        	The previousJoin for this ModelCriteria
	 */
	 ...
	 *         
	 * @param string $name
	 *        	The relation name or alias on which the join was created
	 * @param string $clause
	 *        	SQL clause, may contain column and table phpNames
	 * @param mixed $value
	 *        	An optional value to bind to the clause
	 * @param string $operator
	 *        	The operator to use to add the condition. Defaults to 'AND'
	 *        	
	 * @return ModelCriteria The current object, for fluid interface
	 */
	 ...
	 *        	A Criterion object, or a condition name
	 *        	
	 * @return ModelCriteria The current object, for fluid interface
	 */
	 ...
	 *
	 * @param string $clause
	 *        	The SQL clause with object model column names
	 *        	e.g. 'UPPER(Author.FirstName)'
	 * @param string $name
	 *        	Optional alias for the added column
	 ...
	 * @param string $secondCriteriaClass
	 *        	Classname for the ModelCriteria to be used
	 *        	
	 ...
	 *        	The criteria to read properties from
	 * @param string $operator
	 ...
	 *
	 * @param Criteria $subQueryCriteria
	 *        	Criteria to build the subquery from
	 * @param string $alias
	 *        	alias for the subQuery
	 * @param boolean $addAliasAndSelectColumns
	 *        	Set to false if you want to manually add the aliased select columns
	 *        	
	 * @return ModelCriteria The current object, for fluid interface
	 */
	...
	public function addSelfSelectColumns() {
		call_user_func ( array (
				$this->modelPeerName,
				'addSelectColumns' 
		), $this, $this->useAliasInSQL ? $this->modelAlias : null );
	 ...
	 *
	 * @param string $relation
	 *        	The relation name or alias, as defined in join()
	 *        	
	 * @return ModelCriteria The current object, for fluid interface
	 */
	public function addRelationSelectColumns($relation) {
		$join = $this->joins [$relation];
		call_user_func ( array (
				$join->getTableMap ()->getPeerClassname (),
				'addSelectColumns' 
		), $this, $join->getRelationAlias () );
	 ...
	 *
	 * @param PropelPDO $con
	 *        	The connection object used by the query
	 */
	 ...
	 *
	 * @param string $column
	 *        	A string representing the column phpName, e.g. 'AuthorId'
	 * @param mixed $value
	 *        	A value for the condition
	 * @param PropelPDO $con
	 *        	An optional connection object
	 ...
	 *
	 * @param int $page
	 *        	number of the page to start the pager on. Page 1 means no offset
	 * @param int $maxPerPage
	 *        	maximum number of results per page. Determines the limit
	 * @param PropelPDO $con
	 *        	an optional connection object
	 *        	
	...
	public function doDelete($con) {
		$affectedRows = call_user_func ( array (
				$this->modelPeerName,
				'doDelete' 
		), $this, $con );
	 ...
	 *
	 * @param array $values
	 *        	The associatiove array of columns and values for the update
	 * @param PropelPDO $con
	 *        	The connection object used by the query
	 * @param boolean $forceIndividualSaves
	 *        	If false (default), the resulting call is a BasePeer::doUpdate(), ortherwise it is a series of save() calls on all the found objects
	 */
	 ...
	 *
	 * @param array $values
	 *        	Associative array of keys and values to replace
	 * @param PropelPDO $con
	 *        	an optional connection object
	 * @param boolean $forceIndividualSaves
	 *        	If false (default), the resulting call is a BasePeer::doUpdate(), ortherwise it is a series of save() calls on all the found objects
	 ...
	 *        	The list of condition names, e.g. array('cond1', 'cond2')
	 * @param string $operator
	 *        	An operator, Criteria::LOGICAL_AND (default) or Criteria::LOGICAL_OR
	 *        	
	 ...
	 * @param ColumnMap $colMap
	 *        	The ColumnMap object
	 ...
	 * by their exact Propel column fully qualified name (e.g.
	 * article.AUTHOR_ID)
	 ...
	 *
	 * @param string $clause
	 *        	SQL clause to inspect (modified by the method)
	 *        	
			...
			switch ($char) {
				...
				case "\"" :
					if ($isInString && $stringQuotes == $char) {
						...
						$parsedString .= preg_replace_callback ( "/[\w\\\]+\.\w+/", array (
								$this,
	 ...
	 *
	 * @param array $matches
	 *        	Matches found by preg_replace_callback
	 *        	
	 ...
	 *        	String representing the column name in a pseudo SQL clause, e.g. 'Book.Title'
	 *        	
	 ...
	 *
	 * @param string $columnName
	 *        	the Column phpName, without the table name
	 *        	
	 ...
	 *        	the fully qualified column name, e.g 'book.TITLE' or BookPeer::TITLE
	 *        	
	 ...
	 * @see Criteria::add()
	 * @param string $column
	 *        	The colName of column to run the condition on (e.g. BookPeer::ID)
	 * @param mixed $value        	
	 * @param string $operator
	 *        	A String, like Criteria::EQUAL.
	 *        	
