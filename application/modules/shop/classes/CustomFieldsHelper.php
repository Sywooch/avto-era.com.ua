...
class CustomFieldsHelper {
	...
	private $adminPatternDesc = '<div class="control-group">
                                            ...
                                            <div class="controls">
                                            {btn_elfinder}
                                            <div class="pull-right"><span class="popover_ref" data-original-title="">                                                                    
                                                <i class="icon-info-sign"></i>
                                            </span>
                                            <div class="d_n">{$description}</div>
                                            </div>
	 ...
	 * 
	 * @param string $entity
	 *        	Name of entity type - now support 'order' and 'user'
	 * @param integer $entityId
	 *        	Id of entity, related wit custom field
	...
	public function _getCustomFields($entity, $entityId = -1, $lookup = null) {
		...
		if ($entityId > 0) {
			try {
				...
				$custom_aux = $CI->db->select ( "*" )->distinct ()->join ( 'custom_fields_data', 'custom_fields_data.field_id = custom_fields.id', 'left' )->where ( "`custom_fields.entity` = '" . $lookup . "' " )->where ( "`custom_fields_data.entity_id` = '$entityId'" )->where ( "`custom_fields_data.locale` = '$locale'" )->get ( 'custom_fields' )->result_array ();
				...
				$fields = $CI->db->select ( "*" )->join ( 'custom_fields_i18n', 'custom_fields_i18n.id = custom_fields.id', 'left' )->where ( "`custom_fields.entity` = '$entity' " )->where ( "`custom_fields_i18n.locale` = '$locale'" )->get ( 'custom_fields' )->result_array ();
			}
			
			$this->customData = $fields;
		} else {
			$this->customData = $CI->db->select ( "*" )->join ( 'custom_fields_i18n', 'custom_fields_i18n.id = custom_fields.id', 'left' )->where ( "`custom_fields.entity` = '$entity' " )->where ( "`custom_fields_i18n.locale` = '$locale'" )->get ( 'custom_fields' )->result_array ();
		}
	 ...
	 * 
	 * @param bool $nameKey=false
	 *        	If FALSE, return array, where keys are field_id's, else - field_namme's
	 * @return array
	 */
	 ...
	 *        	If call after getCustomField() - render field with pattern (if TRUE) or clean widget (if FALSE). For multiple fields (after getCustomFields()) widgets always render with pattern.
	 * @return string
	 */
		...
		if ($this->customData)
			foreach ( $this->customData as $key => $customField )
				if ($customField ['field_type_id'] == 2 && $customField ['possible_values'] && is_array ( unserialize ( $customField ['possible_values'] ) )) {
					...
					$this->customData [$key] ['possible_values'] = array (
							'' => 'none' 
					);
	...
	private function renderWidget($widget, $usePattern = true) {
		...
		if ($widget ['field_description']) {
			...
			array_push ( $data, $widget ['field_description'] );
		}
