...
class SCart {
	...
	public function add($data = array()) {
		...
		if ($data->model instanceof SProducts) {
			...
			$itemData = array (
					...
					'quantity' => $data->quantity,
					'price' => in_array ( 'getPrice', get_class_methods ( get_class ( $data->model ) ) ) ? $data->model->getPrice () : ShopCore::app ()->SCurrencyHelper->toMain ( $data->model->getTotalPrice () ) 
			);
	...
	protected function _addToCart($data) {
		...
		if ($data ['instance'] == 'SProducts') {
			...
			if ($data ['variantId'] > 0) {
				$variant = SProductVariantsQuery::create ()->joinWithI18n ( MY_Controller::getCurrentLocale () )->filterByProductId ( $data ['productId'] )->findPk ( $data ['variantId'] );
	...
	public function recount() {
		...
		foreach ( $data as $key => $item ) {
			...
			if (isset ( $_POST ['met_del'] ) && $_POST ['met_del'] > 0) {
				$DeliveryData = SDeliveryMethodsQuery::create ()->filterById ( $_POST ['met_del'] )->findOne ();
		...
		if (empty ( $data ))
			...
			if (sizeof ( $ids ['SProducts'] ) > 0 || sizeof ( $ids ['ShopKit'] ) > 0) {
				if (sizeof ( $ids ['SProducts'] ) > 0) {
					// Load products
					$collection = SProductsQuery::create ()->joinWithI18n ( MY_Controller::getCurrentLocale (), Criteria::LEFT_JOIN )->findPks ( array_unique ( $ids ['SProducts'] ) );
				}
				if (sizeof ( $ids ['ShopKit'] ) > 0) {
					$kitCollection = ShopKitQuery::create ()->filterByActive ( TRUE )->findPks ( array_unique ( $ids ['ShopKit'] ) );
				}
			} else
				return false;
