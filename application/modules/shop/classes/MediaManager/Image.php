...
class Image extends BaseImageClass {
	 ...
	 * @param type $isVarId
	 *        	define product or variant id
	 */
	public function resizeByIdAdditional($id, $isVarId = FALSE) {
		if ($id == null)
			return $this;
		if ($isVarId) {
			$res = $this->db->select ( '*, shop_products.id as sproduct_id' )->join ( 'shop_products', 'shop_product_images.product_id=shop_products.id' )->join ( 'shop_product_variants', 'shop_product_variants.product_id=shop_products.id' )->where_in ( 'shop_product_variants.id', $id )->get ( 'shop_product_images' )->result_array ();
		} else {
