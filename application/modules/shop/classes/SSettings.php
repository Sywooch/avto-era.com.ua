...
class SSettings
{
    ...
    public function loadSettings()
    {
        ...
        {
            $model = $model->where('ShopSettings.Locale = ?', self::$curentLocale)
                    ->orWhere('ShopSettings.Locale = ?', '');
        }
        else
        {
            $model = $model->where('ShopSettings.Locale = ?', $this->defaultLanguage['identif'])
                    ->orWhere('ShopSettings.Locale = ?', '');
        }
    ...
    public function getSortingFront()
    {
        $CI = & get_instance();
        $locale = MY_Controller::getCurrentLocale();
        return $CI->db
                        ->select('shop_sorting_i18n.name_front, shop_sorting_i18n.tooltip, shop_sorting.get')
                        ->order_by('pos')
                        ->where('active', 1)
