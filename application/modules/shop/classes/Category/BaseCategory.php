...
class BaseCategory extends \ShopController {
	...
	public function __CMSCore__() {
		 ...
		 */
		$products = \SProductsQuery::create ()->addSelectModifier ( 'SQL_CALC_FOUND_ROWS' )->filterByCategory ( $this->categoryModel )->filterByActive ( true )->joinWithI18n ( $this->locale )->joinProductVariant ()->withColumn ( 'IF(sum(shop_product_variants.stock) > 0, 1, 0)', 'allstock' )->groupById ()->joinBrand ()->distinct ()->orderBy ( 'allstock', \Criteria::DESC );
		$this->db->cache_off ();
