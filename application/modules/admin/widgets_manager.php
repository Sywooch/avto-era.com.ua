...
class Widgets_manager extends BaseAdminController {
	...
	*/
	...
	public function create() {
		...
		if ($this->db->get_where ( 'widgets', array (
	...
	public function edit($id) {
		...
		if ($widget->num_rows () == 1) {
			...
			if ($widget ['type'] == 'module') {
				...
				echo modules::run ( $widget ['data'] . '/' . $subpath . $widget ['data'] . '_widgets/' . $widget ['method'] . '_configure', array (
						'show_settings',
						$widget
				) );
	...
	public function get($id) {
		return $this->db->get_where ( 'widgets', array (
				'id' => $id
		) );
	}
