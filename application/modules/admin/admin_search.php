...
class Admin_search extends BaseAdminController {
	...
	public function index($hash = '', $offset = 0) {
		...
		if (mb_strlen ( $searchText, 'UTF-8' ) >= 2) {
			...
			$usersResult = $this->db->where ( 'id =', $searchText )->or_where ( "username LIKE '%$searchText%'" )->or_where ( "email LIKE '%$searchText%'" )->get ( 'users' )->result_array ();
	...
	public function autocomplete() {
		if ($this->ajaxRequest) {
			$tokens = array ();
			$pages = $this->db->select ( 'title' )->get ( 'content' )->result_array ();
			foreach ( $pages as $p )
				$tokens [] = $p ['title'];
				
			$users = $this->db->select ( 'username, email' )->get ( 'users' )->result_array ();
