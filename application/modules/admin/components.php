...
class Components extends BaseAdminController {
	...
	function is_installed($mod_name) {
		return $this->db->get_where ( 'components', array (
				'name' => $mod_name
		), 1 )->num_rows ();
	}
	...
	function deinstall() {
		...
		foreach ( $modules as $module ) {
			...
			if (file_exists ( APPPATH . 'modules/' . $module . '/' . $module . '.php' ) and $this->is_installed ( $module ) == 1) {
				...
				$this->db->limit ( 1 );
				$this->db->delete ( 'components', array (
						'name' => $module
				) );
	...
	function find_components_for_menu_list() {
		 ...
		 */
		$components = $this->db->where ( 'in_menu', 1 )->order_by ( 'position', 'asc' )->get ( 'components' )->result_array ();
