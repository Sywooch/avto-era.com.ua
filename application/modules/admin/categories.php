...
class Categories extends BaseAdminController {
	...
	private function renderCatList($tree) {
		...
		foreach ( $tree as $item ) {
			$html .= '<div>';
			$html .= $this->template->fetch ( '_catlistitem', array (
					'item' => $item 
			) );
		...
		if ($this->form_validation->run ( $this ) == FALSE) {
			...
			switch ($action) {
				...
				case 'update' :
					...
					$this->lib_admin->log ( lang ( "Changed the category", "admin" ) . " " . '<a href="' . $BASE_URL . '/admin/categories/edit/' . $cat_id . '"> ' . $data ['name'] . '</a>' );
