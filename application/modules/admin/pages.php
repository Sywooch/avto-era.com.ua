...
;
	...
	public function add() {
		...
		if ($this->form_validation->run ( $this ) == FALSE) {
			...
			$this->lib_admin->log ( lang ( "Created a page", "admin" ) . " " . '<a href="' . site_url ( 'admin/pages/edit/' . $page_id ) . '">' . $data ['title'] . '</a>' );
	...
	function _set_page_roles($page_id, $roles) {
		...
		if ($roles [0] != '') {
			...
			if ($this->db->get_where ( 'content_permissions', array (
					'page_id' => $page_id 
			) )->num_rows () > 0) {
				$this->db->where ( 'page_id', $page_id );
				$this->db->delete ( 'content_permissions' );
	...
	function edit($page_id, $lang = 0) {
		...
		// Get page data
		$data = $this->db->get_where ( 'content', array (
				'id' => $page_id 
		) )->row_array ();
	...
	function move_pages($action) {
		...
		$page = $this->db->get_where ( 'content', array (
				'id' => substr ( $_POST ['pages'] [0], 5 ) 
		) )->row_array ();
			...
			foreach ( $ids as $k => $v ) {
				...
				switch ($action) {
					...
					case 'copy' :
						...
						$pages = $this->db->get_where ( 'content', array (
								'lang_alias' => $page_id 
						) )->result_array ();
	...
	protected function _copy_content_fields($original_id, $new_id) {
		$fields = $this->db->get_where ( 'content_fields_data', array (
				'item_id' => $original_id,
				'item_type' => 'page' 
		) )->result_array ();
