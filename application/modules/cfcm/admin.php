...
class Admin extends BaseAdminController {
	...
	public function index() {
		$this->template->add_array ( array (
				...
				'groupRels' => $this->db->select ( '*' )->join ( 'content_field_groups', 'content_field_groups.id = content_fields_groups_relations.group_id OR content_fields_groups_relations.group_id = -1' )->get ( 'content_fields_groups_relations' )->result_array ()
		) );
	...
	public function create_field() {
		...
		if ($_POST) {
			...
			if ($form->isValid ()) {
				...
				if ($this->db->get_where ( 'content_fields', array (
	...
	public function edit_field_data_type($field_name) {
		...
		$field = $this->db->get_where ( 'content_fields', array (
	...
	public function delete_field($field_name) {
		...
		$this->db->where ( 'field_name', $field_name )->delete ( 'content_fields' );

		$this->db->where ( 'field_name', $field_name )->delete ( 'content_fields_data' );

		$this->db->where ( 'field_name', $field_name )->delete ( 'content_fields_groups_relations' );
		...
		if ($field->num_rows () == 1) {
			...
			if ($_POST) {
				...
				if (count ( $groups )) {
					...
					if (count ( $toInsert ))
						$this->db->delete ( 'content_fields_groups_relations', array (
								'field_name' => $data ['field_name']
						) );
					$this->db->insert_batch ( 'content_fields_groups_relations', $toInsert );
				}
	...
	public function edit_group($id) {
		...
		$group = $this->db->get_where ( 'content_field_groups', array (
				'id' => $id
		) );
	...
	public function delete_group($id) {
		...
		$this->db->where ( 'group_id', $id )->update ( 'content_fields_groups_relations', array (
				'group_id' => '0'
		) );

		$this->db->where ( 'field_group', $id )->update ( 'category', array (
				'field_group' => '-1'
		) );

		$this->db->where ( 'category_field_group', $id )->update ( 'category', array (
				'category_field_group' => '-1'
		) );
	...
	public function form_from_category_group($category_id = FALSE, $item_id = FALSE, $item_type = FALSE) {
		...
		if ($category->field_group != '0') {
			// Get group
			$group = $this->db->get_where ( 'content_field_groups', array (
					'id' => $category->field_group
			) )->row ();
				
			// Get all fields in group
			$fg = ( int ) $category->field_group;
			$query = $this->db->select ( '*' )->from ( 'content_fields' )->join ( 'content_fields_groups_relations', 'content_fields_groups_relations.field_name = content_fields.field_name' )->where ( "content_fields_groups_relations.group_id = $category->field_group" )->order_by ( 'weight', 'ASC' )->get ();
			...
			if ($query->num_rows () > 0) {
				...
				echo '<div class="alert alert-info" style="margin-bottom: 18px; margin-top: 18px;">' . lang ( "Group without any fields", 'cfcm' ) . '</div>';
			}
		} else {
			echo '<div class="alert alert-info" style="margin-bottom: 18px; margin-top: 18px;"> ' . lang ( "For category", 'cfcm' ) . " " . $category->name . " " . lang ( "Field group has not been selected", 'cfcm' ) . ' </div>';
		}
	}
