...
class Cfcm extends MY_Controller {
	...
	public function get_group_fields($group_id = -1) {
		...
		$query = $this->db->where ( 'group_id', $group_id )->join ( 'content_fields', 'content_fields_groups_relations.field_name = content_fields.field_name' )->order_by ( 'weight', 'ASC' )->get ( 'content_fields_groups_relations' );
	...
	public function connect_fields($item_data, $item_type) {
		...
		foreach ( $result as $key => $val ) {
			...
			if (is_array ( $val ) or in_array ( $field->type, array (
					'select',
					'checkgroup',
					'radiogroup'
			) )) {
