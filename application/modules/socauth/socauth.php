...
class Socauth extends MY_Controller {
	 ...
	 *        	social service ID
	 * @param type $username
	 *        	name in social service
	 * @param type $email
	 *        	email in social service
	 * @param type $address
	 *        	address in social service
	 * @param type $key        	
	 * @param type $phone
	 *        	phone in social service
	 */
	private function socAuth($social, $id, $username, $email, $address, $key, $phone) {
		...
		if (count ( $user ) == 0) {
			...
			$data = new stdClass ();
			$userData = $this->db->join ( 'mod_social', 'users.id=mod_social.userId' )->where ( 'socialId', $id )->get ( 'users', 1 )->row ();
	...
	public function renderLogin() {
		if (! $this->dx_auth->is_logged_in ()) {
			$this->writeCookies ();
			\CMSFactory\assetManager::create ()->setData ( $this->settings )->render ( 'loginButtons', TRUE );
		}
	}
	...
	public function renderLink() {
		if ($this->dx_auth->is_logged_in ()) {
			...
			$socials = $this->db->where ( 'userId', $this->dx_auth->get_user_id () )->get ( 'mod_social' );
			...
			\CMSFactory\assetManager::create ()->setData ( $this->settings )->setData ( $social )->registerScript ( 'socauth' )->render ( 'linkButtons', TRUE );
		}
	}
