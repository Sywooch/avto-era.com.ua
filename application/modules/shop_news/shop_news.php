...
class Shop_news extends MY_Controller {
	...
	public function getShopNews($limit = 20) {
		...
		CMSFactory\assetManager::create ()->setData ( array (
				'contentShopNews' => $content 
		) )->registerStyle ( 'style' )->registerScript ( 'scripts' )->render ( 'content', true );
	}
	...
	public function prepareInterface($data, $pageId) {
		...
		return \CMSFactory\assetManager::create ()->setData ( array (
				'shopNews' => $data,
				'categories' => ShopCore::app ()->SCategoryTree->getTree (),
				'currentCategories' => $currentCategories 
		) )->registerScript ( 'scripts' )->fetchTemplate ( '/admin/adminModuleInterface' );
	}
	...
	public function _install() {
		 ...
		 */
		$this->db->where ( 'name', 'shop_news' )->update ( 'components', array (
				'autoload' => '1',
				'enabled' => '1' 
		) );
	}
