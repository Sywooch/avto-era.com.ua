...
class Admin extends BaseAdminController {
	...
	function menu_item($name = '') {
		...
		$ins_id = $this->db->get_where ( 'menus', array (
				'name' => $name 
		) )->row_array ();
	...
	function get_name_by_id($id) {
		$query = $this->db->get_where ( 'menus', array (
				'id' => $id 
		) )->row_array ();
		return $query ['name'];
	}
		...
		if (empty ( $_POST )) {
			...
			if ($this->form_validation->run ( $this ) == FALSE) {
				...
				if ($item_data ['item_type'] == 'url') {
					$item_data ['item_id'] = 0;
					$item_data ['add_data'] = serialize ( array (
							'url' => $_POST ['item_url'],
							'newpage' => ( int ) $newpage 
					) );
				}
	...
	function check_menu_data() {
		...
		if ($this->db->get_where ( 'menus', array (
	...
	function get_pages($cat_id = 0, $cur_page = 0, $referer = null) {
		...
		if ($pages->num_rows () > 0) {
			...
			$total = $this->db->get_where ( 'content', array (
					'lang_alias' => 0,
					'category' => $cat_id 
			) )->num_rows ();
	...
	function translate_window($id) {
		...
		$n = 0;
		foreach ( $langs as $l ) {
			$t = $this->db->get_where ( 'menu_translate', array (
					'item_id' => $id,
					'lang_id' => $l ['id'] 
			) );
