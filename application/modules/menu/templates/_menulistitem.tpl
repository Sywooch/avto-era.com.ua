<div class="row-category">
	...
	<div class="share_alt">
		...
		<div class="title {if $item.parent_id > 0} lev {/if}">
			...
			<button type="button" class="btn btn-small my_btn_s"
				style="display: none;" data-rel="tooltip" data-placement="top"
				data-original-title="{lang('hide menu', 'menu')}">
			...
			{else:} <span class="simple_tree">↳</span> {/if} <a
				href="/admin/components/cp/menu/edit_item/{$item.id}" class="pjax"
				data-rel="tooltip" data-placement="top"
				data-original-title="{lang('Edit menu item', 'menu')}">{truncate($item.title,
				100)}</a> <a
				href="/admin/components/cp/menu/translate_window/{$item.id}"
				class="pjax f_r" data-rel="tooltip" data-placement="top"
				data-original-title="{lang('Translate menu item', 'menu')}"><i
	...
	<div>
		<div class="frame_prod-on_off" data-rel="tooltip" data-placement="top"
			data-original-title="{if $item.hidden != 1}{lang('show')}{else:}{lang("
			don'tshow")}{/if}" >
			<span class="prod-on_off {if $item.hidden == 1 }disable_tovar{/if}"
				style="" {if $item.hidden !=1 }rel="true" {else:}rel="false"
				{/if}
