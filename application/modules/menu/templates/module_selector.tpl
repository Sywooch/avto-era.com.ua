<div style="border: 0px; font-size: 13px;">
	<table width="100%" border="0" cellpadding="2" cellspacing="2">
		<tr>
			<td width="100%" valign="top">{lang("Select module", "menu")}</td>
			<td valign="top">
		...
		<tr>
			<td valign="top">
				<div id="module_list">
					{foreach $modules as $module} <a href="#"
						onclick="set_module_data('{$module.name}');
                                    return false;"
						id="module_{$module.name}" title="{$module.description}"
						style="display: block; font-size: 13px; padding: 5px;">{$module.menu_name}</a>
					{/foreach}
				</div>
			</td>

			<td valign="top">
				<!-- Link params --> <input type="hidden" id="owner_id"
				value="{$insert_id}" />

				<div class="field_text">{lang('Type', "menu")}</div>
				<div class="field_input" id="module_type">{lang('Module',
					"menu")}</div>
				...
				<div class="field_input">
					<input type="text" class="textbox" value="" name="module_title"
						id="module_title" />
				</div>
				<div class="form_overflow"></div>
				...
				<div class="field_input">
					<input type="text" class="textbox" value="" name="module_method"
						id="module_method" /> <br /> func_name/param1/param2
				</div>
				<div class="form_overflow"></div>
				...
				<div class="field_input">
					<select name="module_parent_id" id="module_parent_id">
						...
						$menu_result as $item}
						<option value="{$item.id}">{ for $i=0; $i <
							$item['padding']; $i++ }-{/for} {$item.title}</option> {/foreach}
					</select>
				</div>
				...
				<div class="field_input">
					<select name="module_position_after" id="module_position_after">
						...
						<option value="first">{lang('First', "menu")}</option> {foreach
						$menu_result as $item}
						<option value="{$item.id}">{for $i=0; $i <
							$item['padding']; $i++}-{/for} {$item.title}</option> {/foreach}
					</select>
				...
				<div class="field_input">
					<input type="text" class="textbox" value="" name="mod_image"
						id="mod_image" /> <img width="16" height="16" align="absmiddle"
						src="{$THEME}images/images.png" title="{lang('Select Image', "
						menu")}" style="cursor: pointer;"
						onclick="tinyBrowserPopUp('image', 'mod_image');" />
				</div>
				...
				<div class="field_input">
					<select id="module_roles" name="module_roles[]" multiple="multiple">
						<option value="0">{lang('All', "menu")}</option> {foreach $roles
						as $role}
						<option value="{$role.id}">{$role.alt_name}</option> {/foreach}
					</select>
				</div>

				<div class="form_overflow"></div>

				<div class="field_text">{lang('Hide', "menu")}</div>
				<div class="field_input">
					<input type="radio" name="modhidden_v" id="mod_hidden"
						onclick="module_hidden = 1;" /> {lang('Yes', "menu")} <input
						type="radio" name="modhidden_v" id="mod_nohidden"
						onclick="module_hidden = 0;" checked="checked" /> {lang('No',
					"menu")}
				</div>

				<div class="form_overflow"></div>

				<div class="field_text">{lang("Open in the new window",
					"menu")}</div>
				<div class="field_input">
					<input type="radio" name="modnew_page" id="mod_newpage"
						onclick="mod_newpage = 1;" /> {lang('Yes', "menu")} <input
						type="radio" name="modnew_page" id="mod_nonewpage"
						onclick="mod_newpage = 0;" checked="checked" /> {lang('No',
					"menu")}
				</div>
				...
				<div class="field_input">
					<input type="button" value="{lang('Create', " menu")}" id="mod_btn"
						class="button"
						onclick="insert_module();
                                    return false;" />
					<input type="button" value="{lang('Cancel', "
						menu")}" class="button"
						onclick="MochaUI.closeWindow($('createnewlink'));
                                    return false;" />
				</div>
