...
class Admin extends BaseAdminController {
	...
	public function history($pageId) {
		...
		$total_pages = $this->documentation_model->getPageHistoryCount ( array (
				'page_id' => $pageId 
		) );
		...
		\CMSFactory\assetManager::create ()->setData ( array (
				...
				'history' => $pageHistory,
				'paginator' => $paginator 
		) )->registerStyle ( 'admin' )->registerScript ( 'dmp' )->registerScript ( 'admin' )->renderAdmin ( 'history' );
	}
	...
	public function settings($action = NULL) {
		...
		\CMSFactory\assetManager::create ()->registerScript ( 'admin' )->setData ( 'roles', $roles )->renderAdmin ( 'settings' );
	}
