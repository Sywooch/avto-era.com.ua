...
class Documentation extends \MY_Controller {
	...
	public function autoload() {
		...
		\CMSFactory\assetManager::create ()->setData ( 'hasCRUDAccess', $this->hasCRUDAccess () )->setData ( 'top_menu', $top_menu );

		if (! $this->input->is_ajax_request ()) {
				
			\CMSFactory\assetManager::create ()->setData ( 'hasCRUDAccess', $this->hasCRUDAccess () );
				
			if ($this->hasCRUDAccess ()) {
				\CMSFactory\assetManager::create ()->registerStyle ( 'documentation', TRUE )->registerScript ( 'documentation', FALSE, 'before' );
			}
		}
	}
	...
	public function create_new_page($mainPageId = 0, $langId = null) {
		...
		if ($this->dx_auth->is_admin ()) {
			\CMSFactory\assetManager::create ()->setData ( 'tree', $this->lib_category->build () )-> // Load category tree)
			setData ( 'errors', $this->errors )->setData ( 'mainPage', $mainPage )->setData ( 'params', $params )->render ( 'create_new_page' );
		} else {
			$this->core->error_404 ();
		}
	}
	...
	public function edit_page($id = null, $langId = null) {
		...
		if (! $this->documentation_model->getPageById ( $id, $langId )) {
			 ...
			 */
			if ($this->dx_auth->is_admin ()) {
				\CMSFactory\assetManager::create ()->setData ( 'langs', $this->documentation_model->getLangs () )->setData ( 'tree', $this->lib_category->build () )-> // Load category tree
				setData ( 'page', $page )->setData ( 'params', $params )->setData ( 'defaultLang', $this->defaultLang )->setData ( 'errors', $this->errors )->render ( 'edit_page' );
			} else {
				$this->core->error_404 ();
			}
		}
	...
	public function save_desc() {
		...
		$this->db->where ( 'id', $this->input->post ( 'id' ) )->set ( 'full_text', $this->input->post ( 'desc' ) )->update ( 'content' );
	}
	public function save_title() {
		$this->documentation_model->make_backup ();

		$this->db->where ( 'id', $this->input->post ( 'id' ) )->set ( 'title', $this->input->post ( 'h1' ) )->update ( 'content' );
	}
	...
	public function create_cat() {
		...
		if (! $this->form_validation->error_string ()) {
			 ...
			 * Create new Category
			 */
			\CMSFactory\Events::create ()->registerEvent ( array_merge ( $data, array (
	...
	public function load_category_menu($group = false) {
		 ...
		 */
		\CMSFactory\assetManager::create ()->setData ( 'tree', $tree )->setData ( 'group', $group )->setData ( 'categoryData', $categoryData )->render ( 'left_menu', true );
	}
	...
	public function recent_forum() {
		...
		$model = $forum->order_by ( 'last_post', 'desc' )->get ( 'topics', 4 );
