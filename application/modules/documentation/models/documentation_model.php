...
class Documentation_model extends CI_Model {
	...
	public function updatePage($id = false, $langId = false, $data = false) {
		if ($id != false && $data != false) {
			 ...
			 */
			$this->db->where ( 'lang_alias', $id )->update ( 'content', array (
					'url' => $data ['url'],
	...
	public function make_backup($id = null) {
		...
		$old_data = $this->db->where ( 'id', $id )->get ( 'content' )->row_array ();
	...
	public function getPageHistory($pageId, $perPage = 5, $offset = 0) {
		$result = $this->db->select ( 'mod_documentation_history.*,users.username' )->where ( 'page_id', $pageId )->order_by ( 'id', 'DESC' )->join ( 'users', 'users.id = mod_documentation_history.user_id' )->limit ( $perPage, $offset )->get ( 'mod_documentation_history' );
	...
	public function restoreArticleFromHistory($pageId, $historyId) {
		...
		$someOldData = $this->db->where ( 'id', $historyId )->get ( 'mod_documentation_history' )->row_array ();
	...
	public function deleteHistoryRow($historyId) {
		$this->db->delete ( 'mod_documentation_history', array (
				'id' => $historyId
		) );
	}
	...
	public function getSettings() {
		$settings = $this->db->select ( 'settings' )->where ( 'identif', 'documentation' )->get ( 'components' )->row_array ();
	...
	public function setSettings($settings) {
		return $this->db->where ( 'identif', 'documentation' )->update ( 'components', array (
				'settings' => serialize ( $settings )
		) );
	}
	public function getRoles() {
		$locale = \MY_Controller::getCurrentLocale ();
		$result = $this->db->join ( 'shop_rbac_roles_i18n', 'shop_rbac_roles_i18n.id = shop_rbac_roles.id' )->where ( 'shop_rbac_roles_i18n.locale', $locale )->get ( 'shop_rbac_roles' );
		return $result->result_array ();
	}
	...
	public function getPagesInCategory($id = null, $langId) {
		if ($id != null) {
			$res = $this->db->where ( 'category', $id )->where ( 'post_status', 'publish' )->where ( 'lang', $langId )->order_by ( 'position', 'asc' )->get ( 'content' )->result_array ();
		}
	...
	public function install() {
		 ...
		 */
		$this->db->where ( 'name', 'documentation' )->update ( 'components', array (
				'autoload' => '1',
				'enabled' => '1'
		) );
	}
