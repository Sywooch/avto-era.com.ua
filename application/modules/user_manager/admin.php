...
class Admin extends BaseAdminController {
	...
	function actions($value) {
		...
		foreach ( $query->result () as $row ) {
			...
			if ($row->banned == 0) {
				...
				$this->lib_admin->log ( lang ( 'Ban the user  or the user has been banned', 'user_manager' ) . ' <a href="' . site_url ( '/admin/components/cp/user_manager/edit_user/' . $value ) . '">' . $row->username . '</a>' );
			} else {
				
				// cp_check_perm('user_edit');
				($hook = get_hook ( 'users_unban' )) ? eval ( $hook ) : NULL;
				$this->users->unban_user ( $value );
				$this->lib_admin->log ( lang ( 'The user has been unbanned ', 'user_manager' ) . ' <a href="' . site_url ( '/admin/components/cp/user_manager/edit_user/' . $value ) . '">' . $row->username . '</a>' );
			}
		}
	}
	...
	function update_user($user_id) {
		...
		$user_data = $this->user2->get_user_field ( $user_id, array (
				'username',
				'email' 
		) )->row_array ();
		...
		if ($val->run ()) {
			...
			$this->lib_admin->log ( lang ( 'amt_updated_user' ) . ' <a href="' . site_url ( '/admin/components/cp/user_manager/edit_user/' . $user_id ) . '">' . $data ['username'] . '</a>' );
	...
	function _create() {
		...
		if ($_POST) {
			...
			if ($this->form_validation->run ( $this ) == FALSE) {
				...
				$user_info = $this->db->get_where ( 'roles', array (
						'name' => $data ['name'] 
				) );
				$row = $user_info->row ();
