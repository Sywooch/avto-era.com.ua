...
class Admin extends BaseAdminController {
	...
	public function index() {
		\CMSFactory\assetManager::create ()->setData ( 'models', $this->email->getAllTemplates () )->renderAdmin ( 'list' );
	}
	public function settings() {
		\CMSFactory\assetManager::create ()->registerScript ( 'email', TRUE )->registerStyle ( 'style' )->setData ( 'settings', $this->email->getSettings () )->renderAdmin ( 'settings' );
	}
	...
	public function addVariable($locale = null) {
		...
		if ($this->email->addVariable ( $template_id, $variable, $variableValue, $locale )) {
			\CMSFactory\assetManager::create ()->setData ( 'template_id', $template_id )->setData ( 'variable', $variable )->setData ( 'variable_value', $variableValue )->render ( 'newVariable', true );
		} else {
			return FALSE;
		}
	}
