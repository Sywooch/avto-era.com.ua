...
class Cmsemail_model extends \CI_Model {
	...
	public function getSettings() {
		$this->db->cache_on ();
		$settings = $this->db->select ( 'settings' )->where ( 'identif', 'cmsemail' )->get ( 'components' )->row_array ();
		$this->db->cache_off ();
	...
	public function getTemplateById($id, $locale) {
		return $this->db->select ( '*, mod_email_paterns.id as id' )->join ( 'mod_email_paterns_i18n', "mod_email_paterns_i18n.id = mod_email_paterns.id and mod_email_paterns_i18n.locale = '$locale'", 'left' )->where ( 'mod_email_paterns.id', $id )->get ( 'mod_email_paterns' )->row_array ();
	}
	public function getTemplateByName($name) {
		return $this->db->where ( 'name', $name )->get ( 'mod_email_paterns' )->row_array ();
	}
	public function deleteTemplateByID($ids) {
		$this->db->where_in ( 'id', $ids )->delete ( 'mod_email_paterns' );
		$this->db->where_in ( 'id', $ids )->delete ( 'mod_email_paterns_i18n' );
	}
