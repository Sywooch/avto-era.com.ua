		...
		<div class="pull-right">
			<div class="d-i_b">
				...
				<button onclick="selects()" type="button"
					...
					<i class="icon-ok icon-white"></i>{lang('Save', 'banners')}
				</button>
				<button onclick="selects()" type="button"
					class="btn btn-small action_on formSubmit"
					data-form="#image_upload_form" data-action="tomain">
					<i class="icon-check"></i>{lang('Save and exit', 'banners')}
				</button>
				{echo create_language_select($languages, $locale,
				"/admin/components/init_window/banners/edit/".$banner['id'])}
			</div>
		</div>
	</div>
	<form method="post"
		action="/admin/components/init_window/banners/edit/{echo $banner['id']}/{$locale}"
		...
		<table
			...
			<thead>
				<tr>
					<th colspan="6">{lang('Options', 'banners')}</th>
				</tr>
			</thead>
			<tbody>
				<tr>
							...
							<div class="form-horizontal">
								<div class="span9">
									<div class="control-group">
										<label class="control-label" for="Name">{lang('Name',
											'banners')} {$translatable}:</label>
										<div class="controls">
											<input type="text" name="name" id="Name"
												value="{echo $banner['name']}" />
										</div>
										<div class="controls">
											<span class="frame_label no_connection m-r_15"> <span
												class="niceCheck" style="background-position: -46px 0px;">
													<input type="checkbox" name="active" value="1"
													{if $banner['active'] == true}checked="checked"{/if}>
											</span> {lang('Active', 'banners')}
											</span>
										</div>
									</div>
								</div>
								...
								<div class="control-group">
									<label class="control-label" for="Text">{lang('Text
										banner', 'banners')} {$translatable}:</label>
									<div class="controls">
										<textarea name="description" id="Text" class="elRTE">{echo $banner['description']}</textarea>
									</div>
								</div>
								<div class="control-group">
									<label class="control-label" for="Url"> {lang('URL',
										'banners')} {$translatable}:</label>
									<div class="controls">
										<input type="text" name="url" id="Url"
											value="{if trim($banner['url'])}{echo $banner['url']}{/if}" />
									</div>
								</div>
								...
								<div class="control-group">
									<label class="control-label" for="data">{lang('Selected
										items', 'banners')}:</label>
									<div class="controls">
										...
										</select> <span class="help-block">{lang('Double click to
											deleting', 'banners')}</span>
									</div>
								</div>

								<div class="control-group">
									<label class="control-label" for="banner_type">{lang('Show
										in categories (select items)', 'banners')}:</label>
									<div class="controls">
										<select id="banner_type"
											onchange="autosearch(this, '/admin/components/init_window/banners/autosearch', '#autodrop', 'autodrop')">

											<option value="default">--{lang('select essence',
												'banners')}--</option>
											<option value="main">{lang('Main', 'banners')}</option> {if
											$is_shop}
											<option value="product">{lang('Product', 'banners')}</option>
											<option value="shop_category">{lang('Product
												category', 'banners')}</option>
											<option value="brand">{lang('Brand', 'banners')}</option>
											{/if}
											<option value="category">{lang('Pages categories',
												'banners')}</option>
								...
								<div class="control-group">
									<label class="control-label">{lang('Active until',
										'banners')}:</label>
									<div class="controls">
								...
								<div class="control-group">
									<label class="control-label" for="Img"> {lang('Image',
										'banners')}: </label>
									<div class="controls">
                        ...
                        return false;">
										...
										<div class="o_h">
											<input type="text" name="photo" id="Img"
												value="{echo $banner['photo'];}">
										</div>
										<div id="Img-preview" style="width: 400px;">
											{if $banner['photo']} <img src="{echo $banner['photo']}"
