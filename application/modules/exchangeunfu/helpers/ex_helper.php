...
if (! function_exists ( 'load_products_i18n' )) {
	function load_products_i18n() {
		$ci = & get_instance ();
		return $ci->db->select ( 'shop_products.id, shop_products.external_id, shop_product_variants.id as varId , shop_products_i18n.name' )->join ( 'shop_products_i18n', 'shop_products.id = shop_products_i18n.id' )->join ( ' shop_product_variants', 'shop_products.id =  shop_product_variants.product_id' )->get ( 'shop_products' )->result_array ();
	}
}
...
if (! function_exists ( 'get_product_category' )) {
	function get_product_category($id) {
		$ci = & get_instance ();
		$result = $ci->db->join ( 'shop_category', 'shop_category.id=shop_products.category_id' )->where ( 'shop_products.id', $id )->get ( 'shop_products' )->row_array ();
		return $result;
	}
}
