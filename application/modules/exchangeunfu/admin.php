...
class Admin extends BaseAdminController {
	...
	public function index() {
		...
		\CMSFactory\assetManager::create ()->registerScript ( 'admin' )->setData ( 'partners', $partners )->setData ( 'periods', $period )->renderAdmin ( 'partners' );
	}
	public function updatePartner() {
		...
		$this->db->where ( 'id', $partner_id )->update ( 'mod_exchangeunfu_partners', array (
				'name' => $name,
				'prefix' => $prefix,
				'region' => $region,
				'code' => $code,
	...
	public function addPartner() {
		...
		if ($result) {
			...
			\CMSFactory\assetManager::create ()->setData ( 'partner', $partners )->renderAdmin ( 'onePartnerRow', true );
		} else {
			return FALSE;
		}
	}
	...
	public function update_settings() {
		...
		if ($for_update) {
			...
			$this->db->where ( 'identif', 'exchangeunfu' )->update ( 'components', array (
					'settings' => serialize ( $config ) 
			) );
