...
class Stats_model_products extends CI_Model {
	 ...
	 * @param array $brands
	 *        	(optional) brandsIds
	 * @param boolean $uniqueProducts
	 *        	(optional, default FALSE) if TRUE, then method
	...
	public function getBrandsCountsData($brandIds = NULL, $uniqueProducts = FALSE) {
		...
		$brandsInfo = array ();
		foreach ( $brands as $id => $name ) {
			$result = $this->db->select ( 'stock' )->from ( 'shop_product_variants' )->join ( 'shop_products', 'shop_products.id = shop_product_variants.product_id' )->where ( 'brand_id', $id )->get ();
	 ...
	 *        	(optional, default all root categories)
	...
	protected function getAllCategories() {
		...
		$result = $this->db->select ( 'shop_category.id,parent_id,name,full_path_ids' )->from ( 'shop_category' )->join ( 'shop_category_i18n', 'shop_category.id = shop_category_i18n.id' )->where ( 'locale', $this->locale )->get ();
	...
	protected function getAllBrands() {
		$result = $this->db->select ( 'shop_brands.id,name' )->from ( 'shop_brands' )->join ( 'shop_brands_i18n', 'shop_brands_i18n.id = shop_brands.id' )->where ( 'locale', $this->locale )->get ();
		$brandsList = array ();
		foreach ( $result->result_array () as $row ) {
	...
	protected function getCategoryProductsCount($categoryId, $uniqueProducts = FALSE) {
		$this->db->select ( 'stock' )->from ( 'shop_product_variants' )->join ( 'shop_products', 'shop_products.id = shop_product_variants.product_id' );
