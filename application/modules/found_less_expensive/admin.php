...
class Admin extends BaseAdminController {
	...
	public function index() {
		...
		$this->template->add_array ( array (
				...
				'status' => $status,
				'countAll' => $this->found_less_expensive_model->getCountAll ( array (
						0,
						1 
				) ),
	...
	public function ajax_save_settings() {
		...
		$this->db->where ( 'name', 'found_less_expensive' )->update ( 'components', array (
				'settings' => $value 
		) );
	...
	public function ajax_delete() {
		...
		$this->db->delete ( 'mod_found_less_expensive', array (
				'id' => $id 
		) );
	...
	public function ajax_change_status() {
		...
		$this->db->where ( 'id', $id )->update ( 'mod_found_less_expensive', array (
				'status' => $status 
		) );
