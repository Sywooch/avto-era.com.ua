...
class Tags extends MY_Controller {
	...
	public function _set_page_tags($tags_str = '', $page_id) {
		...
		$this->db->delete ( 'content_tags', array (
				'page_id' => $page_id 
		) );
		...
		if (count ( $tags_arr ) > 0) {
			foreach ( $tags_arr as $k => $v ) {
				if (trim ( $v ) != '') {
					// Check if tag exits
					if ($this->db->get_where ( 'tags', array (
							'value' => trim ( $v ) 
					) )->num_rows () > 0) {
						$this->db->limit ( 1 );
					...
					if (mb_strlen ( $v, 'utf-8' ) > 1)
						$this->db->insert ( 'content_tags', array (
								'page_id' => $page_id,
								'tag_id' => $tag_id 
						) );
				}
