...
class Commentsapi extends MY_Controller {
	...
	public function renderAsArray($url) {
		...
		if ($this->enable_comments)
			$comments = \CMSFactory\assetManager::create ()->setData ( $data )->registerStyle ( 'comments', TRUE )->fetchTemplate ( $this->tpl_name );
		else {
			$comments = '';
		}
		...
		return array (
				...
				'total_comments' => $comments_count ? $comments_count . ' ' . $this->Pluralize ( $comments_count, array (
						lang ( 'comment', 'comments' ),
						lang ( 'comment', 'comments' ),
						lang ( 'comments', 'comments' ) 
				) ) : lang ( 'Leave comment', 'comments' ),
	...
	public function renderPosts() {
		...
		echo json_encode ( array (
				'comments' => $comments,
				'total_comments' => $comments_count ? $comments_count . ' ' . $this->Pluralize ( $comments_count, array (
						lang ( "review", 'comments' ),
						lang ( "reviews", 'comments' ),
						lang ( "review", 'comments' ) 
				) ) : lang ( 'Leave a comment', 'comments' ),
				'commentsCount' => $commentsCount [$item_id],
				'validation_errors' => $this->validation_errors 
		) );
	}
	...
	public function parsUrl($url) {
		...
		if (strstr ( $url, '/product/' )) {
			...
			$id = $this->db->select ( 'id, enable_comments' )->where ( 'url', $url )->get ( 'shop_products' )->row ();
		...
		if ($url == site_url ()) {
			$id = $this->db->select ( 'main_page_id, comments_status' )->join ( 'content', 'settings.main_page_id=content.id' )->get ( 'settings' )->row ();
		...
		$id = $this->db->select ( 'id, comments_status' )->where ( 'url', $paths )->get ( 'content' )->row ();
	...
	public function newPost() {
		...
		if ($this->input->post ( 'action' ) == 'newPost') {
			...
			if (! validation_errors ()) {
				...
				$sucesess = lang ( 'Спасибо за ваш Комментарий! ', 'comments' );
				if ($this->dx_auth->is_logged_in () == FALSE)
					$sucesess .= lang ( 'Он будет отправлен на модерацию.', 'comments' );
	...
	public function getTotalCommentsForProducts($ids, $module = 'shop', $status = 0) {
		...
		foreach ( $query as $q )
			$result [$q ['item_id']] = $q ['count'] . ' ' . $this->Pluralize ( ( int ) $q ['count'], array (
					lang ( "review", 'comments' ),
					lang ( "reviews", 'comments' ),
					lang ( "review", 'comments' ) 
			) );
		...
		foreach ( ( array ) $ids as $id )
			if (! $result [$id])
				$result [$id] = 0 . ' ' . $this->Pluralize ( '0', array (
						lang ( "review", 'comments' ),
						lang ( "reviews", 'comments' ),
						lang ( "comments", 'comments' ) 
				) );
