...
if (! function_exists ( 'func_truncate' )) {
	function func_truncate($var, $chars = 0, $end = '...', $strip_tags = FALSE) {
		...
		if ($chars > 0 and mb_strlen ( $var, 'utf-8' ) >= $chars) {
			...
			return extension_loaded ( 'tidy' ) ? call_user_func ( function () use($att) {
				$tidy = new tidy ();
				$tidy->parseString ( $att, array (
						'show-body-only' => TRUE,
						'indent' => TRUE,
						'output-xhtml' => TRUE,
						'wrap' => 200 
			...
			return extension_loaded ( 'tidy' ) ? call_user_func ( function () use($var) {
				$tidy = new tidy ();
				$tidy->parseString ( $var, array (
						'show-body-only' => TRUE,
						'indent' => TRUE,
						'output-xhtml' => TRUE,
						'wrap' => 200 
